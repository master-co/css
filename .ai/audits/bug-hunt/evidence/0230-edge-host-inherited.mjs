...
const report = { scenario, bundler, pure, client, mixed, packageDir, input: cases[scenario], build: null, observations: [], stylesheets: [], errors: [], browserFailures: [], requestedBrowsers: browserNames }
let server
try {
  ...
  if (code === 0 && report.build.exported) {
    ...
    for (const name of browserNames) {
      ...
      try {
        ...
        report.observations.push({ browser: name, id, ...observation, ...(cases[scenario].valid ? { pass: observation.color === 'rgb(18, 52, 86)' && observation.border === '7px' && !observation.classes.includes('undefined') && (!cases[scenario].package || client || id === 'client-probe' || observation.origin === (bundler === 'webpack' ? 'style' : cases[scenario].conditions ? 'react-server' : 'default')) } : {}) })
        }
