...
const report = { scenario, bundler, pure, packageDir, input: cases[scenario], build: null, observations: [], stylesheets: [], errors: [] }
let server
try {
  ...
  writeFileSync(join(root, 'app/other.module.css'), cases[scenario].b)
  ...
  if (code === 0 && report.build.exported) {
    ...
    for (const name of ['chromium', 'firefox', 'webkit']) {
      ...
      try {
        ...
        report.observations.push({ browser: name, ...observation })
