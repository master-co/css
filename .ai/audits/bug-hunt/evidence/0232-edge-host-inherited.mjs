...
export default tracedConfig;`)
  ...
  if (cases[scenario].inline) writeFileSync(join(root, 'transform-loader.cjs'), `const fs=require('node:fs');module.exports=function(source,map){const width=this.getOptions().width;const output=source.replace(/border-top:\\d+px/g,'border-top:'+width+'px');fs.appendFileSync(${JSON.stringify(join(root, 'inline-loader.jsonl'))},JSON.stringify({resource:this.resourcePath,query:this.resourceQuery,options:this.getOptions(),input:source,output})+'\\n');this.callback(null,output,map)};`)
  writeFileSync(join(root, 'app/card.module.css'), (pure ? '' : '@master entry;@preserve native;') + cases[scenario].a.replaceAll('__AUDIT_INLINE_LOADER__', join(root, 'transform-loader.cjs')))
