...
for mode in ['pure-full','runtime','progressive']:
 for request in ['runtime','css-runtime']:
  env={**os.environ,'BH_MODE':mode,'BH_REQUEST':request,'BH_DRAIN':'1'}
...
Path('.ai/audits/bug-hunt/evidence/0158-drained-host-matrix.json').write_text(json.dumps(rows,indent=2)+'\n')
