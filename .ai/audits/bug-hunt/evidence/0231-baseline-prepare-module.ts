...
export interface ModuleContext {
  ...
  resourcePath: string
  rootContext?: string
  loaders?: { path?: string, options?: Record<string, any> }[]
}

/** Delegate CSS Module syntax, purity and naming to Next's own processors. */
export async function prepareNextModule(context: ModuleContext, source: string, projectDir: string, sourceMap?: string, webpack = false, preparation?: { scoped: boolean, resolveICSS(specifier: string): Promise<Record<string, string>> }) {
  ...
  const result = await postcss(preparation?.scoped === false ? [] : getModulesPlugins(options, host)).process(source, {
