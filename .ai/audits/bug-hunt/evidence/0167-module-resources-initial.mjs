...
for (const mode of (process.env.BH_MODES?.split(',') ?? ['static', 'runtime', 'pre-render', 'progressive'])) {
  ...
  const branch = color => `@import "/base/external.css";.resource{${mode === 'pure' ? 'padding:2rem' : '@compose p:2rem'};color:${color};background-image:url("./${encodeURIComponent(name)}?variant=1#part")}`
        ...
        try {
          ...
          if (phase === 'initial') {
            for (const width of [500, 900]) {
              ...
              const row = { command, mode, browser: client.browserName, phase: `viewport-${width}`, state, result: 'PASS' }
              rows.push(row);console.log(JSON.stringify(row))
            }
          }
        } catch (cause) { error = String(cause) }
        const row = { command, mode, browser: client.browserName, phase, value, errors: [...errors], error, result: error ? 'FAIL' : 'PASS' }
        rows.push(row);console.log(JSON.stringify(row))
      }
    }
  } finally {
