...
for (const mode of ['pre-render', 'progressive']) {
  ...
  const entry = color => `@layer fallback,shared;@import "./styles/branch.css" layer(shared) supports(display:grid) screen and (min-width:700px);@master entry;@preserve native;@theme{--color-accent:${color}}@components{card{color:var(--color-accent)}}@layer fallback{.external{margin-left:3px;outline:none}.conditional{background:rgb(1,2,3);padding-left:0}}`
  try {
    ...
    dev = await createServer({ root, configFile: false, logLevel: 'silent', base: '/base/', plugins, server: { host: '127.0.0.1', port: 0 } });await dev.listen()
    ...
    for (const phase of ['wide', 'narrow', 'wide-again', 'theme-update']) {
      ...
      for (const client of clients) for (const item of client.pages) {
        ...
        try {
          ...
          await page.waitForFunction(({ wide, color }) => {
            ...
            return window.ready && card.color === color && card.paddingTop === '2px' && external.marginLeft === (wide ? '17px' : '3px') && external.outlineWidth === (wide ? '7px' : '0px') && conditional.backgroundColor === (wide ? 'rgb(12, 34, 56)' : 'rgb(1, 2, 3)') && (wide ? parseFloat(conditional.paddingLeft) > 0 : conditional.paddingLeft === '0px')
          }, { wide: width >= 700, color: expectedColor }, { timeout: 15000 })
          value = await page.evaluate(() => { const card = getComputedStyle(document.querySelector('#card')), external = getComputedStyle(document.querySelector('#external')), conditional = getComputedStyle(document.querySelector('#conditional'));return { color: card.color, padding: card.paddingTop, margin: external.marginLeft, outline: external.outlineWidth, background: conditional.backgroundColor, conditionalPadding: conditional.paddingLeft, resource: conditional.backgroundImage, styles: document.querySelectorAll('style#master-css').length } })
...
const summary = { builds, browserComparisons: rows.length, browserFailures: rows.filter(row => row.result === 'FAIL').length }
