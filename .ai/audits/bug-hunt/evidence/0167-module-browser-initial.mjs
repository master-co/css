...
for (const relation of relations) for (const childModule of [false, true]) {
  for (const mode of process.env.BH_MODES?.split(',') ?? ['pure', 'static', 'runtime', 'pre-render', 'progressive']) {
    ...
    try {
      ...
      writeFileSync(join(root, 'index.html'), '<!doctype html><div id="target">child</div><div id="local">local</div><script type="module" src="./client.js"></script>')
      ...
      for (const browserName of ['chromium', 'firefox', 'webkit']) {
        ...
        for (const width of [900, 500]) {
          ...
          const value = await page.evaluate(() => ({ names: window.names, className: document.querySelector('#target').className, padding: getComputedStyle(document.querySelector('#target')).paddingTop, display: getComputedStyle(document.querySelector('#local')).display }))
          let error
          try {
            assert.deepEqual(errors, []);assert.equal(value.display, 'inline-flex')
