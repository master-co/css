...
try {
  for (const test of cases) {
    const request = { graph: { entry: 'entry', files: { entry: test.entry, local: test.local || '.example{color:red}' }, edges: [{ from: 'entry', specifier: './local.css', resolved: 'local' }] }, urls: { entry: '/delivered/entry.css', local: '/delivered/local.css' }, baseManifest: { version: 1, utilities: [] }, options: { classes: ['example'] } }
...
console.log(JSON.stringify({ cases: cases.length, comparisons, failures: 0, scope: 'Public compileStylesheets; existing file/project/build/CLI entrypoints still require integration' }))
