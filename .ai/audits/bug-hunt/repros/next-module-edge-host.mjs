  ...
  if (code === 0 && report.build.exported) {
    ...
    for (const name of browserNames) {
      ...
      try {
        ...
        report.observations.push({ browser: name, id, phase, ...observation, ...(cases[scenario].valid ? { pass: observation.color === 'rgb(18, 52, 86)' && observation.border === '7px' && (!cases[scenario].expectedPadding || observation.padding === cases[scenario].expectedPadding) && (!cases[scenario].expectedAnimation || (observation.animation === cases[scenario].expectedAnimation && (cases[scenario].keyframeContext ? observation.animationFrames.some(frame => frame.opacity === '0.25') : observation.animationFrames.length > 0))) && (!cases[scenario].resourceFixture || (/^url\("?https?:/.test(observation.background) && observation.background.includes('?rev=1#shape') && report.requests.some(request => request.status === 200 && request.url.includes('.svg?rev=1')))) && (!cases[scenario].inspectImport || report.importObservations.at(-1).pass) && !observation.classes.includes('undefined') && (conditionKeys || !cases[scenario].package || client || id === 'client-probe' || observation.origin === (bundler === 'webpack' ? 'style' : cases[scenario].conditions ? 'react-server' : 'default')) ? true : false } : {}) })
        }
        }
