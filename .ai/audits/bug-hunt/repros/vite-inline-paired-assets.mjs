...
try {
  ...
  writeFileSync(join(root, 'external.css'), '@import "https://inline-external.test/paint.css";.example{border-image-source:url(pixel.svg)}')
  ...
  for (const [baseIndex, base] of bases.entries()) for (const format of ['es', 'cjs']) for (const emit of [false, true]) {
    ...
    const common = { root, base, configFile: false, logLevel: 'silent' }
    ...
    try {
      ...
      const assetPass = emit ? assets.length > 0 : assets.length === 0
      const assetRow = { id, base, format, emit, phase: 'assets', assets, css, result: assetPass && !css.includes('file:') ? 'PASS' : 'FAIL' }; rows.push(assetRow); console.log(JSON.stringify(assetRow))
      for (const name of ['chromium', 'firefox', 'webkit']) {
        ...
        try {
          ...
          await page.route('**/*', route => {
            ...
            const file = join(emit ? serverDir : clientDir, url.pathname.replace(/^\/deployed\//, ''))
          ...
          const row = { id, phase: 'browser', browser: name, value, missing, errors, result: value.color === 'rgb(0, 0, 255)' && value.imageLoaded && !missing.length && !errors.length ? 'PASS' : 'FAIL' }; rows.push(row); console.log(JSON.stringify(row))
