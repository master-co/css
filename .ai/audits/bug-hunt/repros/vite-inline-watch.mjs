...
const root = realpathSync(mkdtempSync(join(tmpdir(), 'master-css-inline-watch-'))), out = join(root, 'dist')
let watcher
...
try {
  ...
  const script = (process.env.BH_ENTRY === 'direct' ? 'import css from "./entry.scss?inline";' : 'import css from "./entry.css?inline";') + (process.env.BH_CONSTRUCT ? 'const sheet=new CSSStyleSheet();sheet.replaceSync(css);document.adoptedStyleSheets=[sheet];' : 'const style=document.createElement("style");style.textContent=css;document.head.append(style);')
  ...
  for (const [index, color] of ['red', process.env.BH_JS_ONLY ? 'red' : 'blue', 'green'].entries()) {
    ...
    if (index === 1 && process.env.BH_JS_ONLY) writeFileSync(join(root, 'entry.js'), script + '\nconsole.log("JS-only rebuild")')
    else if (index) writeFileSync(control, text(color))
