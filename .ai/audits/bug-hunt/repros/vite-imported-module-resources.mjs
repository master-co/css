...
const duplicate = process.env.BH_DUPLICATE === '1', native = process.env.BH_NATIVE === '1'
if (duplicate) assert.equal(command, 'build', 'Duplicate resource browser control is a production-build case.')
for (const mode of (process.env.BH_MODES?.split(',') ?? ['static', 'runtime', 'pre-render', 'progressive'])) {
  ...
  const branch = (color, className = 'resource') => `@import "${externalURL}";.${className}{${mode === 'pure' || native ? 'padding:2rem' : '@compose p:2rem'};color:${color};background-image:url("./${encodeURIComponent(name)}?variant=1#part")}`
  ...
  try {
    ...
    if (duplicate) {
      ...
      writeFileSync(join(root, 'nested-copy/branch.css'), branch('rgb(12,34,56)'))
      writeFileSync(join(root, 'nested-copy/bridge.css'), '@import "./branch.css";.bridge{display:block}')
      writeFileSync(join(root, 'nested-copy', name), svg('blue'))
    ...
    writeFileSync(join(root, 'client.js'), `import names from './${filename}';const apply=names=>{window.names=names;document.querySelector('#target').className=[names.local,names.resource??names.resourceNext].join(' ')};apply(names);if(import.meta.hot)import.meta.hot.accept('./${filename}',module=>apply(module.default));window.bootID=Math.random();window.ready=true`)
    const config = { root, cacheDir: join(root, '.vite'), base: '/base/', configFile: false, logLevel: 'silent', plugins: mode === 'pure' ? [] : createMasterCSSVitePlugin({ mode }), server: { host: '127.0.0.1', port: 0 } }
    ...
    for (const phase of (process.env.BH_PHASES?.split(',') ?? (command === 'build' ? ['initial'] : ['initial', 'child-update', 'resource-update', 'resource-restore', 'class-update']))) {
      ...
      if (phase === 'class-update') writeFileSync(join(root, 'nested/branch.css'), branch('rgb(56,34,12)', 'resourceNext'))
      for (const client of clients) {
        ...
        try {
          ...
          assert.deepEqual(value.pixel, duplicate || phase === 'resource-update' ? [0, 0, 255, 255] : [255, 0, 0, 255])
          ...
          assert.equal(await response.text(), svg(duplicate || phase === 'resource-update' ? 'blue' : 'red'))
          ...
          if (phase === 'initial') {
            for (const width of [500, 900]) {
              ...
              const row = { command, external, duplicate, native, mode, browser: client.browserName, phase: `viewport-${width}`, state, result: 'PASS' }
              rows.push(row);console.log(JSON.stringify(row))
            }
          }
        } catch (cause) { error = String(cause) }
        const row = { command, external, duplicate, native, mode, browser: client.browserName, phase, value, errors: [...errors], error, result: error ? 'FAIL' : 'PASS' }
        rows.push(row);console.log(JSON.stringify(row))
      }
    }
  } finally {
