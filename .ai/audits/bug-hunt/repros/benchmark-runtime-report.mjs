...
const child = spawn('pnpm', [`bench:${suite}`, '--run'], { stdio: 'inherit', env: {
  ...
  BROWSER_LIFECYCLE_SCENARIOS: process.env.BROWSER_LIFECYCLE_SCENARIOS || 'initial-load', BROWSER_LIFECYCLE_ROUNDS: '1',
...
for (const variant of report.variants) {
  ...
  for (const sample of samples) {
    if (['runtime-generated-rule-count', 'runtime-style-raw-bytes'].includes(sample.metricId) && usesRuntime) assert(sample.value > 0, variant.id + '/' + sample.metricId);
    if (sample.metricId === 'progressive-adopted') assert.equal(sample.value, variant.id.includes('master-progressive') ? 1 : 0, variant.id);
  }
        ...
        try {
          ...
          if (usesRuntime) {
            ...
            const comparison = await page.evaluate(`(() => {
              const actual = __readLifecycleState(); const snapshot = masterCSSRuntime.snapshot();
              return { actual: [actual.runtimeGeneratedRuleCount, actual.runtimeStyleRawBytes, actual.progressiveAdopted], expected: [snapshot.layers.reduce((sum, layer) => sum + layer.ruleCount, 0), new TextEncoder().encode(snapshot.cssText).length, snapshot.hydration.state === 'progressive' ? 1 : 0] };
            })()`);
