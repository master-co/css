...
const syntax = 'css', missing = 'directory', backend = process.env.BH_BACKEND ?? 'default', kinds = ['plain-root', 'entry']
...
for (const kind of kinds) for (const mode of modes) {
  const fixture = realpathSync(mkdtempSync(join(parent, 'external-directory-browser-'))), root = join(fixture, 'app'), external = join(fixture, 'external'), clients = []
  ...
  const tokens = padding => `@utilities{paint{padding:${padding}rem;background-image:url("./one/two/three/pixel.svg?v=1#icon")}}.never{border-left:99px solid red}`
  const reference = join(external, 'tokens.css'), pixel = join(external, 'one/two/three/pixel.svg')
  try {
    writeFileSync(reference, tokens(2))
    const input = 'style.css', plain = true
    writeFileSync(join(root, input), `${kind === 'entry' ? '@master entry;@preserve native;' : ''}@reference "../external/tokens.css";.target{@compose paint;}`)
    ...
    server = await createServer({ root, base: '/base/', cacheDir: join(root, '.vite'), configFile: false, logLevel: 'silent', plugins: createMasterCSSVitePlugin({ mode }), server: { host: '127.0.0.1', port: 0, fs: { allow: [fixture, workspace] }, ...(backend === 'node' ? { watch: { useFsEvents: false, usePolling: false } } : {}) } }); await server.listen()
    ...
    for (const phase of ['startup-restored', 'restored', 'resource-update']) {
      ...
      for (const client of clients) {
        ...
        const row = { syntax, missing, backend, kind, mode, browser: client.browserName, phase, value, failure, pass: !failure, expectedFailureEvidence: { responses: client.failedResponses, errors: client.errors, messages: client.messages.filter(message => ['error', 'full-reload'].includes(message.type)) } }
        rows.push(row); console.log(JSON.stringify(row))
      }
    }
  } finally { for (const { browser } of clients) await browser.close(); await server?.environments.client.waitForRequestsIdle(); await server?.close(); rmSync(fixture, { recursive: true, force: true }) }
}
console.log(JSON.stringify({ summary: true, syntax, missing, backend, observations: rows.length, failures: rows.filter(row => !row.pass).length }))
