// Batch 0249: actual `next dev --webpack` recovery control for PostCSS-introduced (late) global references
// on the owned Next copy. After the first render it deletes the referenced SVG, restores it, removes the
// theme definition the plugin references, then restores it. Output: BH_NEXT_HOST_EVIDENCE JSON.
...
const report = { scenario: 'postcss-late-resource-dev-recovery', bundler: 'webpack', pure, packageDir, steps: [], errors: [], serverLog: '' }
...
const colorDefinition = color => color ? `;--color-audit:${color}` : ''
const stylesheet = color => pure ? '.direct{color:#123456;border-top:7px solid red}' : `@master entry;@preserve native;@theme{--image-audit:url("./new.svg?rev=1#shape")${colorDefinition(color)}}.direct{color:#123456;border-top:7px solid red}`
const globals = color => `:root{--image-audit:url("./new.svg?rev=1#shape")${colorDefinition(color)}}`
...
try {
  ...
  for (const name of browserNames) {
    ...
    try {
      ...
      // The dev error overlay may replace the page tree; report a missing probe instead of waiting on it.
      const observe = () => page.evaluate(() => { const el = document.getElementById('probe');if (!el) return { missing: true, color: '', border: '', background: '', outline: '', marker: window.__AUDIT_MARKER };const s = getComputedStyle(el);return { color: s.color, border: s.borderTopWidth, background: s.backgroundImage, outline: s.outlineColor, marker: window.__AUDIT_MARKER } })
      ...
      const html = async () => { const response = await page.request.get(url);return { status: response.status(), text: await response.text() } }
      // Dev overlays can navigate the page between polls; treat a destroyed context as a transient missing probe.
      const safeObserve = () => observe().catch(error => ({ missing: true, navigating: String(error.message).slice(0, 80), color: '', border: '', background: '', outline: '', marker: undefined }))
      const settle = async (predicate, limit = 60) => { let last;for (let i = 0; i < limit; i++) { await delay(1000);last = { observation: await safeObserve(), page: await html() };if (await predicate(last)) break }return last }
      // Step 1: the late-referenced resource file disappears.
      rmSync(join(root, 'app/new.svg'))
      const deleted = await settle(async last => /Module not found|Can't resolve|not found|ENOENT|Cannot find|error/i.test(last.page.text) || last.observation.missing || !/^url\(/.test(last.observation.background), 45)
      const deletedError = deleted.page.text.match(/(Module not found[^<]{0,200}|Can't resolve[^<]{0,200}|CSS file not found[^<]{0,200}|Resource[^<]{0,120}not found[^<]{0,120}|ENOENT[^<]{0,200})/)?.[1] ?? null
      report.steps.push({ browser: name, step: 'svg-deleted', observation: deleted.observation, pageStatus: deleted.page.status, error: deletedError, serverLogTail: report.serverLog.slice(-1500), pass: Boolean(deletedError) || deleted.page.status >= 500 || Boolean(deleted.observation.missing) })
      // Step 2: the file returns with new bytes.
      writeFileSync(join(root, 'app/new.svg'), svg('blue'))
      const recovered = async last => last.page.status === 200 && !last.observation.missing && /^url\(/.test(last.observation.background) && (await fetchBackground(last.observation))?.body.includes('fill="blue"')
      let restored = await settle(recovered, 90)
      let reloaded = false
      if (!(await recovered(restored))) { reloaded = true;await page.reload({ waitUntil: 'networkidle' });await page.waitForSelector('main[data-hydrated="true"]', { timeout: 60000 }).catch(() => {});restored = await settle(recovered, 30) }
      const restoredAsset = await fetchBackground(restored.observation)
      report.steps.push({ browser: name, step: 'svg-restored', observation: restored.observation, asset: restoredAsset, pageStatus: restored.page.status, reloaded, pass: restored.page.status === 200 && restoredAsset?.status === 200 && restoredAsset.body.includes('fill="blue"') && keepsReference(restored.observation.background) })
      // Step 3: the theme definition the plugin references is removed from the authored stylesheet.
      writeStylesheets(null)
      const undefinedTheme = await settle(async last => !last.observation.missing && last.observation.outline !== 'rgb(17, 17, 17)')
      report.steps.push({ browser: name, step: 'theme-definition-removed', observation: undefinedTheme.observation, pageStatus: undefinedTheme.page.status, pass: undefinedTheme.page.status === 200 && undefinedTheme.observation.outline !== 'rgb(17, 17, 17)' && undefinedTheme.observation.color === 'rgb(18, 52, 86)' && keepsReference(undefinedTheme.observation.background) })
      // Step 4: the definition returns.
      writeStylesheets('#111111')
      const back = await settle(async last => !last.observation.missing && last.observation.outline === 'rgb(17, 17, 17)')
      report.steps.push({ browser: name, step: 'theme-definition-restored', observation: back.observation, pageStatus: back.page.status, pass: back.page.status === 200 && back.observation.outline === 'rgb(17, 17, 17)' && back.observation.color === 'rgb(18, 52, 86)' })
