...
const result = spawnSync('pnpm', ['exec', 'vitest', 'bench', 'build-diagnostics', '--run'], {
  env: { ...process.env, BENCHMARK_ROUNDS: '1' }, stdio: 'inherit', timeout: 180000
})
const path = '.results/build-diagnostics/report.json'
if (existsSync(path)) {
  const report = JSON.parse(readFileSync(path, 'utf8'))
  writeFileSync(fileURLToPath(new URL('../evidence/0070-build-report.json', import.meta.url)), JSON.stringify(report, null, 2))
