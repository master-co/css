...
const resultPath = join(auditRoot, `evidence/0117-vscode-${process.env.BH_VSCODE_BEFORE ? 'before' : 'after'}.json`)
...
const stagingRoot = await mkdtemp(join(tmpdir(), 'bh117-'))
try {
  ...
  const child = spawn('/Applications/Visual Studio Code.app/Contents/MacOS/Code', [
    ...
    `--extensionTestsPath=${fileURLToPath(new URL('../../../../packages/vscode/tests/bug-hunt-settings-host.cjs', import.meta.url))}`, workspace
