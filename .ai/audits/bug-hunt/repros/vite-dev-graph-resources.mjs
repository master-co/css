...
for (const mode of ['static', 'runtime', 'pre-render', 'progressive']) {
  const root = realpathSync(mkdtempSync(join(parent, 'dev-graph-resources-'))), clients = []
  ...
  try {
    ...
    for (const phase of ['initial', 'child-update', 'resource-update', 'resource-restore']) {
      ...
      for (const client of clients) {
        ...
        try {
          ...
          await page.waitForFunction(({ color, previous, changed }) => {
            const target = document.querySelector('#target');if (!target) return false
            const style = getComputedStyle(target)
            return window.ready && style.color === color && style.borderLeftWidth === '7px' && (!changed || style.backgroundImage !== previous)
          }, { color, previous: client.previousResource, changed: phase.startsWith('resource-') }, { timeout: 15000 })
        ...
        const row = { mode, browser: client.browserName, phase, value, errors: [...errors], error, result: error ? 'FAIL' : 'PASS' }
