...
const rows = []
const modules = process.env.BH_MODULE === '1', modulesDisabled = process.env.BH_MODULES_DISABLED === '1'
const base = process.env.BH_BASE || '/', pathMode = process.env.BH_PATH || 'relative'
for (const extension of process.env.BH_EXTENSION ? [process.env.BH_EXTENSION] : ['css', 'scss']) {
  for (const request of process.env.BH_REQUEST ? [process.env.BH_REQUEST] : ['normal', 'inline', 'raw', 'url']) {
    for (const managed of process.env.BH_MANAGED ? [process.env.BH_MANAGED === '1'] : [false, true]) {
      ...
      let server, externalRoot
      try {
        mkdirSync(join(root, 'node_modules'))
        symlinkSync(dirname(createRequire(require.resolve('vite')).resolve('sass')), join(root, 'node_modules/sass'), 'dir')
        const file = `${pathMode === 'encoded' ? '樣式 space' : 'style'}${modules ? '.module' : ''}.${extension}`
        if (pathMode === 'external') {
          externalRoot = realpathSync(mkdtempSync(join(tmpdir(), 'master-dev-style-external-')))
          mkdirSync(join(externalRoot, 'node_modules'))
          symlinkSync(dirname(createRequire(require.resolve('vite')).resolve('sass')), join(externalRoot, 'node_modules/sass'), 'dir')
        }
        const sourceFile = join(externalRoot || root, file)
        const specifier = (['alias', 'external'].includes(pathMode) ? `@audit/${file}` : `./${file}`) + (request === 'normal' ? '' : `?${request}`)
        ...
        writeFileSync(sourceFile, source('red'))
        writeFileSync(join(root, 'index.html'), `<div id="target" class="${modules && !modulesDisabled ? 'audit_example' : 'example'}">test</div><script type="module" src="./entry.js"></script>`)
        ...
        if (process.env.BH_NO_URL_TRACKING) for (const plugin of masterPlugins) {
          if (plugin.name === 'master-css:sass-source') plugin.configureServer = undefined
        }
        ...
        server = await createServer({ root, base, css: modules ? { modules: modulesDisabled ? false : { generateScopedName: 'audit_[local]' } } : undefined, resolve: { alias: { '@audit': dirname(sourceFile) } }, configFile: false, logLevel: 'silent', plugins: [
        ...
        ], server: { host: '127.0.0.1', port: 0, fs: { allow: [root, ...(externalRoot ? [externalRoot] : [])] } } })
        await server.listen()
        for (const name of ['chromium', 'firefox', 'webkit']) {
          ...
          page.on('response', async response => { if (response.status() >= 400) { const failure = { url: response.url(), status: response.status() };requests.push(failure);try { failure.body = await response.text() } catch {} } })
        ...
        for (const color of process.env.BH_REPEAT ? ['red', 'green', 'blue'] : ['red', 'green']) {
          if (color !== 'red') writeFileSync(sourceFile, source(color))
          await Promise.all(clients.map(async ({ page, name, errors, requests, bootID }) => {
            ...
            const value = await page.evaluate(async ({ request, expectedSource, color }) => {
              ...
              if (request === 'url' && color === 'red' && typeof window.payload === 'string') {
            ...
            }, { request, expectedSource: source(color), color })
            if (request === 'url') {
              ...
              value.linkCount = await page.locator('link#request-probe-style').count()
            }
            const untouched = value.before.color === 'rgb(0, 0, 0)' && value.before.background === 'rgba(0, 0, 0, 0)'
            const correct = request === 'normal' ? value.before.color === expected && value.before.background === 'rgb(0, 0, 255)' : request === 'inline' ? untouched && value.after?.color === expected && value.after?.background === 'rgb(0, 0, 255)' : request === 'raw' ? untouched && value.rawEqual : (color === 'red' ? untouched && value.fetched?.status === 200 : true) && value.linkCount === 1 && value.after?.color === expected && value.after?.background === 'rgb(0, 0, 255)'
            const row = { modules, modulesDisabled, base, pathMode, extension, request, managed, color, browser: name, expected, expectedBootID: bootID, coldStartReload: color === 'red' && value.bootID !== bootID, correct, value, errors: [...errors], requests: [...requests], waitError, result: !waitError && correct && (color === 'red' || value.bootID === bootID) && !errors.length && !requests.length ? 'PASS' : 'FAIL' }
      ...
      finally { for (const client of clients) await client.browser.close();await server?.close();rmSync(root, { recursive: true, force: true });if (externalRoot) rmSync(externalRoot, { recursive: true, force: true }) }
    }
  }
}
