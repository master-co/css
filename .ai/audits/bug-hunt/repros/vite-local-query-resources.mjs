...
import { mkdirSync, mkdtempSync, realpathSync, readFileSync, writeFileSync, rmSync, symlinkSync } from 'node:fs'
...
const query = process.env.BH_QUERY ?? 'url'
const filename = kind === 'module' ? 'style.module.css' : kind === 'scss' ? 'style.scss' : 'style.css'
for (const mode of (process.env.BH_MODES?.split(',') ?? ['static', 'runtime', 'pre-render', 'progressive'])) {
  const root = realpathSync(mkdtempSync(join(parent, 'local-query-resources-'))), clients = []
  ...
  try {
    if (kind === 'scss') { mkdirSync(join(root, 'node_modules'));symlinkSync(dirname(createRequire(require.resolve('vite')).resolve('sass')), join(root, 'node_modules/sass'), 'dir') }
    writeFileSync(join(root, filename), (kind === 'scss' ? '$layout:inline-flex;' : '') + '@import "./bridge.css" layer(guard) supports(display:grid) screen and (min-width:700px);.local{display:' + (kind === 'scss' ? '$layout' : 'inline-flex') + '}')
    writeFileSync(join(root, 'bridge.css'), '@import "./branch.css";.bridge{display:block}');
    ...
    writeFileSync(join(root, 'client.js'), (query === 'url' ? `import url from './${filename}?url';const link=document.createElement('link');link.rel='stylesheet';link.href=url;document.head.append(link);` : inline ? `import css from './${filename}?inline';const style=document.createElement('style');style.textContent=css;document.head.append(style);` : kind === 'module' ? `import names from './${filename}';document.querySelector('#target').classList.add(names.local);` : `import './${filename}?${query}';`) + 'window.bootID=Math.random();window.ready=true')
    if (multiple) {
        ...
        try {
          ...
          if (phase === 'initial') {
            for (const width of [500, 900]) {
              ...
              const row = { command, kind, query, inline, multiple, mode, browser: client.browserName, phase: `viewport-${width}`, state, result: 'PASS' }
              rows.push(row);console.log(JSON.stringify(row))
            }
          }
        } catch (cause) { error = String(cause) }
        const row = { command, kind, query, inline, multiple, mode, browser: client.browserName, phase, value, errors: [...errors], error, result: error ? 'FAIL' : 'PASS' }
        rows.push(row);console.log(JSON.stringify(row))
      }
    }
  } finally {
