...
for (const mode of ['static', 'runtime', 'pre-render', 'progressive']) {
  const root = realpathSync(mkdtempSync(join(parent, 'retained-reference-browser-'))), clients = []
  ...
  try {
    ...
    writeFileSync(join(root, 'nested/pixel.svg'), svg('red'))
    ...
    for (const phase of command === 'build' ? ['initial'] : ['initial', 'reference-update', 'resource-update']) {
      if (phase === 'reference-update') writeFileSync(join(root, 'nested/tokens.css'), tokens(7))
      if (phase === 'resource-update') writeFileSync(join(root, 'nested/pixel.svg'), svg('blue'))
      for (const client of clients) {
        ...
        try {
          ...
          }, { padding: phase === 'initial' ? '32px' : '112px', changed: phase === 'resource-update', prior: client.resource }, { timeout: 10000 })
