...
if (process.argv[2] === 'worker') {
  ...
  try {
    ...
    const resultFile = fileURLToPath(new URL('../evidence/0082-multiprocess.json', import.meta.url))
    ...
    assert(rows.filter(r => 'round' in r).every(r => ['after-one', 'after-two'].includes(r.finalText)))
