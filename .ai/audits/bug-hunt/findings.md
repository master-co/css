...
| BH-0051 | P1 | 已確認 | Next --webpack 頂層CSS rule移除原生CSS loaders，CSS被當JS解析 | 管線部分修正；default CSS/SCSS/Sass production9browser通過，未儀器化LightningCSS production矩陣與偶發timeout仍待驗；[checkpoint](evidence/0183-next-virtual-commit-validation.json) |
...
| BH-0053 | P1 | 已確認 | Next/Turbopack 在 Sass 預處理前將SCSS／縮排Sass交給CSS分類，編譯失敗 | 實際兩種來源與raw-SCSS回歸失敗；尚未修復；[證據及下一步](evidence/0183-next-virtual-findings.json) |
