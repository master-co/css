...
| BH-0051 | P1 | 已確認 | Next --webpack 中頂層CSS rule使原生CSS loaders被移除，CSS被當JS解析而無法啟動 | 相對config import排除fileURL快取警告後仍重現；尚未修復，未進入瀏覽器HMR；[0183](batches/0183-reference-host-recovery.md) / [證據與下一步](evidence/0183-runtime-inputs-next-webpack-css.json) |

51 historical confirmed findings: 48 fixed, 3 unresolved. BH-0047 pre-render manifest HMR is fixed; four original candidates, one unresolved Vite-host development shutdown native-handle limitation and blocked coverage remain unfinished. See linked batches for status history and evidence.
