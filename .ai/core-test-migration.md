...
| `packages/facade/tests/config/at/layer.test.ts` | compiler | rewritten-css-first | Layer/at-rule authoring now lowers through CSS directives and compiled variant/at-rule records; covered by compiler migration tests and engine parser parity. |
...
| `packages/facade/tests/utils/equal-at-rules.test.ts` | engine | helper-replaced | Internal equality helpers are no longer public facade API; equivalent behavior is asserted through generated at-rule CSS and parser parity tests. |
...
| `packages/facade/tests/utils/generate-at.test.ts` | engine | covered-exact | Covered by `packages/engine/tests/parser-parity.test.ts` for compiled at-rule parse/generate semantics. |
...
| `packages/facade/tests/utils/parse-at.test.ts` | engine | covered-exact | Covered by `packages/engine/tests/parser-parity.test.ts`, including logical operators, comparisons, feature aliases, container aliases, custom at-rule aliases, and generate-at output. |
