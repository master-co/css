...
| `packages/core/tests/issues/377.test.ts` | engine/tooling | covered-exact | Issue regression restored in ESLint sort-classes tests; readable sorting now preserves the old visibility-chain order. |
...
| `packages/core/tests/utils/sort-readable-classes.test.ts` | engine/tooling | covered-representative | Public sorting helper is gone from facade; priority/order behavior is covered by engine rule order tests and tooling sort-classes tests. |
