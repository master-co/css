...
- Treat runtime source, the `@master/css-binding-wasm-engine` feature set, schema runtime constants, and preset manifest loading as browser runtime bundle inputs. Keep compiler/tooling features in `@master/css-binding-wasm-compiler` or `@master/css-binding-wasm-tooling` and measure split artifact impact.
