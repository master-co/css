...
## Syntax Tutorial migration, 2026-09-08

- Getting Started now links to Syntax Tutorial immediately after Installation. The Guide overview has six sections and 34 primary teaching entries; section icons use stable IDs and the old `#syntax-tutorial` entrance resolves within Getting Started. English and Traditional Chinese navigation were checked.
- One native Save button progresses through declarations, hover and keyboard focus, responsive padding, composition, a custom `@theme` spacing token, and complete configuration/HTML/generated CSS. The same authored content and configured compiler output feed HTML, search and llms. Reference contracts remain in their original `contract.mdx` files; registry source paths are independent of learning links.
- All four retired routes and 120 rule anchors are covered across English, Traditional Chinese and canonical paths, including unknown/malformed fragment fallback. Browser checks confirmed the four English entrypoints, a Traditional Chinese rule anchor and unknown-fragment fallback. Static checks verify noindex, tutorial canonical, usable no-JavaScript links, and exclusion from the page registry and sitemap. English-prefixed development routes retain `/en`; canonical routes remain unprefixed.
- Checked actual CSS viewport widths of 390px, 768px and 1280px with no whole-page overflow. The responsive preview uses 833px/835px frame viewports derived from the current preset, visibly changes padding, and preserves hover feedback and native keyboard focus. Fixed the preview's flex-item width so its controls and button stay inside the Demo container. No new mobile document navigation or article action UI was added.
- Reference/migration tests (13), llms tests (13), syntax tests (14), documentation examples (6), static migration/output parity checks (3), type-check and source budgets passed. Site lint has no errors: 72 existing warnings plus three class-sort suggestions in sequential tutorial markup. The tutorial keeps the base declaration first when adding conditions or replacing its token.
- The scoped production build passed with the existing dynamic compiler-import warning. An incremental build omitted a referenced manifest asset; clearing the production `.next` cache while preserving `.next/dev` and rebuilding restored it. The CSS contract check verified the complete clean output.
- CSS review: global manifest and all existing generated rule text are unchanged. Only `pt:5xl` joins the global class set for the redirect fallback. Fifteen existing route contracts change (the four retired Guide routes and Introduction across three URL variants); three Syntax Tutorial routes are added. The reviewed snapshot contains 1,145 routes, 206 hydration contracts, 1,902 rules and 2,204 exact CSS segments. No parser, compiler or runtime semantics change.
