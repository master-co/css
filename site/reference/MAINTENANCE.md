...
`ConfiguredExample` accepts either a class array or trusted literal HTML, including local raw HTML specimens. It compiles with the public render session, matching iframe previews; tooling validates every expanded declaration without filtering emission. Generate CSS from the displayed source instead of maintaining cached CSS specimens; keep any configuration required by the output, such as `@mode`, explicit.
