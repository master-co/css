...
export const mcpEditorial: Record<string, ToolEditorial> = {
  mastercss_workspace_info: {
    ...
    output: 'Read `root` and `roots` for workspace scope, `packages` for resolved package locations, and `manifest.status`, `manifest.entries`, `manifest.dependencies`, and `manifest.warnings` for the loaded context. A loaded preset with no entries does not confirm the intended project configuration.',
    lifecycle: readOnly
  },
  ...
  mastercss_inspect_directives: {
    ...
    fields: { content: 'CSS buffer to inspect when `entryPath` is absent.', filePath: 'Virtual path for the CSS buffer. Defaults to `master.css`.', entryPath: 'Existing stylesheet path inside the workspace. Takes precedence over `content`.', preserveNativeCSS: 'Forward the native-CSS preservation option to compilation.' },
  ...
  mastercss_manifest_query: {
    ...
    fields: { query: 'Case-insensitive substring query. Omit for all entries in the selected kinds.', kind: 'Category to return. Defaults to `all`.', namespace: 'Exact namespace filter for tokens and utilities. Other categories are not filtered by namespace.', limit: 'Maximum entries returned per category, not across the whole response. Defaults to 50.' },
  ...
  mastercss_lint_project: {
    ...
    fields: { patterns, rules }, example: { patterns: ['src/button.html'], rules: 'sort-classes' }, exampleNote: 'Check class order in an existing file without applying a fix.',
  ...
  mastercss_preview_fixes: {
    ...
    fields: { mode: 'Preview operation. Defaults to `lint-fixes`.', patterns, rules, includeDirectiveFixes: 'Allow structural directive fixes in lint mode. Defaults to false.', outputPath: 'Target path inside the workspace. Required in `generated-css` mode; its parent directory must exist.', ttlMs },
