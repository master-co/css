...
test('design system is responsive and its controls change real layout', async ({ page }, testInfo) => {
  ...
  const measure = page.locator('.demo-measure')
  const viewport = page.viewportSize()!
  await expect(measure.locator('.demo-measure-line')).toContainText(/Container · \d+ × \d+ px/)
  const originalMeasure = await measure.locator('.demo-measure-line').textContent()
  await page.setViewportSize({ ...viewport, width: Math.max(320, Math.floor(viewport.width / 2)) })
  await expect(measure.locator('.demo-measure-line')).not.toHaveText(originalMeasure!)
