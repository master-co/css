...
import { nuxtInstallationModes } from '../nuxt-installation-examples'

for (const mode of nuxtInstallationModes) test(`Nuxt production application delivers ${mode} CSS`, async ({ page, browser }) => {
  const url = JSON.parse(process.env.NUXT_INSTALLATION_FIXTURES!)[mode]
  const errors: string[] = []
