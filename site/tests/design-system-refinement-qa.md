...
## Supplemental component review — 2026-09-24

The follow-up review covered eight remaining distinct shared UI treatments: `DemoBadge`, `DemoAsset`, `DemoCopyButton`, `DemoContainer`, `DemoPalette`, `DemoWaterfall`, `DemoCatalog`, and `DemoFeatureSupport`. Each has an original/current/adopted comparison route with a real Guide or Reference example. Pure compositions such as theme and style comparisons continue to use their already-reviewed canvas, comparison, and viewport components; the token inventory continues to use the established `doc-table` treatment.

- The Brand assets now sit on their chosen preview surface without the generic media background painting a light rectangle behind transparent SVGs. The original artwork and Guide color palette remain unchanged.
- The color chips remain plain, copy the documented CSS variable, show the result in flow, and meet a 44px target. Text copy controls add a labeled icon while keeping the native button and status text.
- Container demos keep their real query boundary and internal horizontal scrolling; Home, End, and Fit produce the expected widths without changing the page viewport.
- Recipe disclosures remain native `<details>` controls, and live support labels still use `CSS.supports` with both supported and unsupported states rendered in text.
- The shared request-order diagram stays illustrative, without a time scale or measured FCP claim. The original preload Guide retains its detailed timeline.

The adopted examples were checked at 390, 768, and 1280 CSS-pixel widths in light and dark modes, including page overflow, focus, keyboard operation, copying, disclosures, and native responsive behavior. `test:reference` (20), `test:docs-examples` (13), `test:llms` (32), type-check, `prepare-app`, `check:ai-context`, `test:css-contract`, and `pnpm build:site` passed. The CSS contract remains at 1,216 routes, 135 hydration contracts, 1,603 generated rules, and 1,822 exact CSS segments, so its baseline was not changed. The production demo dogfood run passed 42 cases with two platform-conditional skips. The 183-utility audit passed all 40 desktop/mobile light/dark batches. Design System dogfood passed all 12 desktop/tablet/mobile light/dark cases, covering 87 recipe disclosures and visible component previews. Full site lint passed with 0 errors and the same 31 pre-existing class-style warnings; changed files also passed scoped lint.
