...
const bars = [
  ['Mon', 80, 'h:2rem'], ['Tue', 130, 'h:3.25rem'], ['Wed', 100, 'h:2.5rem'],
  ['Thu', 160, 'h:4rem'], ['Fri', 120, 'h:3rem'], ['Sat', 190, 'h:4.75rem'],
] as const
...
export default function Page() {
  return <main className="min-h:100dvh p-md bg-surface-base text-body">
    <section data-workspace className="grid-cols:4 gap-sm w:100% max-w-3xl mx:auto grid-cols:8@2xs gap-md@2xs">
      ...
      <DemoSurface id="demand" className="grid-col-span:4 min-w:0 p-md grid-col-span:5@2xs">
        <figure className="m:0">
          ...
          <div className="flex items-end gap-xs h:8rem mt-sm">
            {bars.map(([day, value, height]) => <div key={day} className="flex flex-col flex:1 gap-xs min-w:0 text-center">
              <DemoLabel>{value}</DemoLabel>
              <DemoItem className={`${height} w:100% r:var(--radius-sm)|var(--radius-sm)|0|0`} aria-hidden="true" />
