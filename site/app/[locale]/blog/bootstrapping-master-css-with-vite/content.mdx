...
`@master/css.vite` brings Master CSS into that pipeline as a first-class Vite integration. Instead of choosing between several setup guides, manual runtime imports, extraction scripts, and HTML post-processing steps, you install one plugin and choose the rendering model that matches your application.
...
The original Vite integration focused on Static Extraction. It worked well for projects that wanted a compiled stylesheet, but the rest of the Master CSS rendering story still required manual setup.
...
- `extract`: imports `virtual:master.css` and runs the static extraction workflow.
...
Use extract mode when your classes are visible in source and you want a compiled CSS asset before runtime. This is the closest mode to traditional utility-first build workflows.
...
- resolves the Master CSS config as a virtual module;
...
- exposes `virtual:master.css` for extraction workflows;
- handles static extraction output;
