...
4. The generated CSS is injected as `<style id="master">`, and a hydration manifest is injected as `<script type="application/json" id="master-css-hydration-manifest">`.
...
`style#master` gives the browser the CSS it needs to paint immediately. `script#master-css-hydration-manifest` gives the Master CSS runtime a compact JSON rule IR, currently `version: 1`, that describes the generated rules inside that style block. Each manifest rule can include the class name, layer, priority, selector text, generated CSS text, and any referenced variables or animations.
...
<head>
    ...
    <script type="application/json" id="master-css-hydration-manifest">{"version":1,"rules":[{"className":"font:48px","key":"font:48px","layer":"utilities","type":0,"sortTier":0,"priority":{"selector":0},"text":".font\\:48px{font-size:48px}","selectorText":".font\\:48px"}]}</script>
</head>
<body>
...
`CSSRuntimeRegistry` loads the canonical project-level MasterCSSManifest compiled by the Next.js adapter.
