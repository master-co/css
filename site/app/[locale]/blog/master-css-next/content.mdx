...
The integration defaults to Static Rendering: it scans complete source class names and builds an ordinary CSS asset. Progressive Rendering is an explicit option. In that mode, the Next.js Adapter API supplies final static and pre-rendered HTML so Master CSS can inject each document's first-page rules into `<style id="master-css">`.

Static Rendering works with Turbopack and Webpack. Progressive Rendering remains useful when first-page HTML and later browser-generated class names both matter.
...
export default function Page() {
  return (
    <main className="min-h:100vh p:2.5rem bg-surface-base text-neutral">
      <h1 className="font-6xl font-heavy tracking-tight">
...
4. The generated CSS is injected as `<style id="master-css">`, and the page hydration manifest is written as a hashed JSON asset.
5. The style tag receives `data-master-css-hydration-manifest`, and the runtime imports that manifest during hydration to attach existing CSS rules to runtime-managed rule objects instead of regenerating them.

The result is a page that can arrive from the server already carrying the CSS needed for its first render, while still keeping the runtime behavior that makes Master CSS practical in interactive applications. If the manifest pointer is missing or the loaded rules no longer match `style#master-css`, the runtime warns and rebuilds its stylesheet from connected DOM classes. A referenced manifest that cannot be loaded or validated instead rejects startup with `INVALID_HYDRATION_MANIFEST`.
...
export default function Page() {
  return <h1 className="font-6xl">Hello</h1>
}
```

Progressive Rendering produces HTML shaped like this:

```html
<!doctype html>
<html>
<head>
  <style id="master-css" data-master-css-hydration-manifest="/_next/static/master-css/hydration/master-css-hydration.8f3a1c2d.json">@layer theme{:root,:host{--font-size-6xl:3rem}}@layer utilities{.font-6xl{font-size:var(--font-size-6xl)}}</style>
</head>
<body>
  <h1 class="font-6xl">Hello</h1>
</body>
</html>
```

The browser can paint the heading from `style#master-css` before the runtime does any work. When the runtime starts, it imports the hashed JSON manifest, adopts the generated `.font-6xl` rule, and continues handling later dynamic classes from the same managed stylesheet. If a page has no generated Master CSS rules, the adapter does not need to attach a manifest URL.

This page hydration manifest is separate from the project manifest and from the `buildReport` option in `@master/css-next`. When enabled, `buildReport` writes `.next/master-css-build-report.json` with details about rendered files; it is not the route-level JSON used to hydrate `style#master-css` in the browser.
...
For the default source-scanned setup, follow [Static Rendering in Next.js](/guide/installation/nextjs). To use the adapter handoff, follow [Progressive Rendering in Next.js](/guide/installation/nextjs/progressive).
...
Install Master CSS and the Next.js adapter:

```bash action=install
@master/css@rc @master/css-next@rc
```

Keep the root layout focused on the global stylesheet:
...
The Next.js adapter injects the runtime through the build tool and loads the canonical project-level MasterCSSManifest automatically. React apps outside build-tool integrations can use `@master/css-runtime` directly.
...
Development uses the runtime engine directly for fast refresh. Run a production build to verify that `<style id="master-css">` is pre-rendered into static and pre-rendered HTML outputs:
...
Then enable static mode in `next.config.js`:
...
Under the hood, static mode scans configured source files and writes generated CSS under `.master/`. The Next.js integration replaces `@import "@master/css"` with that CSS in both Turbopack and Webpack builds. It preserves ordinary native CSS by default; `pruneNativeCSS: true` explicitly opts project-owned source into pruning.

Managed utilities are generated for detected classes. References to `var(--*)` or manifest animation names can pull matching variables and keyframes from the project CSS entry graph. During Turbopack builds, a scanner loader also reads JavaScript and TypeScript modules that pass through the module graph, so imported files can incrementally feed additional class names into the scanner.
...
export function Badge({ tone }: {
  ...
  return (
    <span className={`px-xs py-3xs r-sm bg-${tone}/.1 text-${tone}`}>
      New
    </span>
  )
}
...
```html
<span class="px-xs py-3xs r-sm bg-blue/.1 text-blue">New</span>
```
...
If another integration already owns Next.js `adapterPath`, `@master/css-next` composes it with the Master CSS adapter. Master CSS runs first by default; set `adapterOrder: 'external-first'` when the external adapter must process each build output first.
...
- it keeps Static Rendering as the default and makes Progressive Rendering an explicit choice for pages that need runtime handoff.
