...
4. The generated CSS is injected as `<style id="master-css">`, and a hydration manifest is injected as `<script type="application/json" id="master-css-hydration-manifest">`.
...
`style#master-css` gives the browser the CSS it needs to paint immediately. `script#master-css-hydration-manifest` gives the Master CSS runtime a compact JSON rule IR, currently `version: 1`, that describes the generated rules inside that style block. Each manifest rule can include the class name, layer, priority, selector text, generated CSS text, and any referenced variables or animations.
...
<html>
<head>
    <style id="master-css">@layer utilities{.font\:48px{font-size:48px}}</style>
...
The browser can paint the heading from `style#master-css` before the runtime does any work. When the runtime starts, it reads the manifest, maps the native CSS rule back to `font:48px`, and continues handling later dynamic classes from the same managed stylesheet.

This runtime hydration manifest is separate from the `manifest` option in `@master/css.next`. When enabled, the Next adapter option writes `.next/master-css-manifest.json` as a build report for rendered files; it is not the JSON script used to hydrate `style#master-css` in the browser.
