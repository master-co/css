...
export default function Page() {
    return (
        <main className="min-h:100vh p:10x bg:surface text:strong">
            <h1 className="font:48px font:heavy tracking:tight">
...
export default function Page() {
    return <h1 className="font:48px">Hello</h1>
}
```
...
<html>
<head>
    <style id="master">@layer utilities{.font\:48px{font-size:48px}}</style>
    <script type="application/json" id="master-css-runtime-manifest">{"version":1,"rules":[{"className":"font:48px","key":"font:48px","layer":"utilities","type":0,"sortTier":0,"priority":{"selector":0},"text":".font\\:48px{font-size:48px}","selectorText":".font\\:48px"}]}</script>
</head>
<body>
    <h1 class="font:48px">Hello</h1>
</body>
</html>
```

The browser can paint the heading from `style#master` before the runtime does any work. When the runtime starts, it reads the manifest, maps the native CSS rule back to `font:48px`, and continues handling later dynamic classes from the same managed stylesheet.
...
export function Badge({ tone }: {
    ...
    return (
        <span className={`r:4px px:2x py:1x text:${tone} bg:${tone}/.1`}>
            New
        </span>
    )
}
...
```html
<span class="r:4px px:2x py:1x text:accent bg:accent/.1">New</span>
```
