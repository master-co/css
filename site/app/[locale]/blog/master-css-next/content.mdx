...
export default function Page() {
    return (
        <main className="min-h:100vh p:10x bg:surface fg:strong">
...
<html>
<head>
    <style id="master">@layer utilities{.font\:48{font-size:3rem}}</style>
    <script type="application/json" id="master-css-runtime-manifest">{"version":1,"rules":[{"className":"font:48","key":"font:48","layer":"utilities","type":-1,"sortTier":0,"priority":{"selector":0},"text":".font\\:48{font-size:3rem}","selectorText":".font\\:48"}]}</script>
</head>
<body>
    <h1 class="font:48">Hello</h1>
</body>
</html>
```

The browser can paint the heading from `style#master` before the runtime does any work. When the runtime starts, it reads the manifest, maps the native CSS rule back to `font:48`, and continues handling later dynamic classes from the same managed stylesheet.

This runtime hydration manifest is separate from the `manifest` option in `@master/css.next`. When enabled, the Next adapter option writes `.next/master-css-manifest.json` as a build report for rendered files; it is not the JSON script used to hydrate `style#master` in the browser.

---

## Adapter integration
The new adapter is registered in `next.config.mjs`, and Next.js calls it when the full build output is available:
...
`CSSRuntimeRegistry` loads the canonical project-level MasterCSSPlan compiled by the Next.js adapter.
...
Under the hood, static mode scans the configured source files with `@master/css-extractor` and writes a generated CSS file under `.master/`. A Turbopack CSS loader replaces `@import "@master/css"` with that generated CSS, and stylesheets that import `@master/css` are treated as native CSS pruning roots by default.

Ordinary class selectors in those roots are pruned against detected classes. References to `var(--*)` or plan animation names can pull matching variables and keyframes from the project CSS entry graph. During Turbopack builds, a scanner loader also reads JavaScript and TypeScript modules that pass through the module graph, so imported files can incrementally feed additional class names into the scanner.
