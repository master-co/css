...
export default function DemoInteractions() {
  ...
  return (
    <>
      <Demo title="Controls with a purpose" controls={<DemoControls label="Cross-axis alignment">
        ...
        <div className={`flex gap:sm h:40x ${alignment}`}>
          <DemoItem className="flex:1 p:md" tone="blue">01</DemoItem><DemoItem className="flex:1 h:24x p:md" tone="violet">02</DemoItem><DemoItem className="flex:1 p:md" tone="neutral">03</DemoItem>
        </div>
      </Demo>
      <Demo title="Motion" caption="Animation starts only when requested. Replay returns every animation to its starting time.">
        <DemoMotion><div className="flex justify-center p:lg"><DemoItem className="grid place-content:center size:16x animation:rotate|2s|linear|infinite">↗</DemoItem></div></DemoMotion>
      </Demo>
      <Demo title="Scroll region" caption="A keyboard-focusable, bounded region keeps scrolling local to the example.">
        <DemoScrollArea aria-label="Layer collection" className="h:40x">
