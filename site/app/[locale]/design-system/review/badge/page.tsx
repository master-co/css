...
function BadgeSpecimen({ previous = false }: { previous?: boolean }) {
  return <div className={styles.specimens}>
    {badges.map(({ label, tone, variant }) => <div key={label} className={styles.specimen}>
      <DemoBadge tone={tone} variant={variant} className={previous ? styles.previousBadge : undefined}>{label}</DemoBadge>
      <DemoItem className={styles.object}>{label} surface</DemoItem>
    </div>)}
  </div>
}
...
function OriginalSpecimen() {
  return <div className={styles.specimens}>
    {badges.map(({ label }) => <div key={label} className={styles.specimen}>
      <OriginalDemoLabel>{label}</OriginalDemoLabel>
      <div className={`app-box ${styles.originalObject}`}>{label} surface</div>
    </div>)}
  </div>
}
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 24</div>
    <h1>Demo badge</h1>
    <p className={styles.intro}>A badge identifies a state or category beside a specimen. It is static text, so an action still needs a real button or link. The adopted treatment keeps the Guide’s restrained annotation style and sharpens the shared badge.</p>
    <div className={styles.reviewNote} role="note">Adopted direction: the shared badge has a smaller radius and balanced monospace inset. The original Guide remains unchanged.</div>
    <label htmlFor="badge-review-theme" className={styles.themeControl}><span>Preview theme</span><span className={styles.themeSelect}>Light · Dark · System<ThemeSelect id="badge-review-theme" aria-label="Preview theme" /></span></label>

    <section aria-labelledby="badge-options">
      <h2 id="badge-options">State labels beside an object</h2>
      <p className={styles.sectionCopy}>All three treatments use the same subject and comparison content. Labels sit outside the objects being demonstrated.</p>
    ...
    <section aria-labelledby="badge-scale">
      ...
      <Demo>
        <div className={styles.scale}>
          ...
          <div><span>wrap</span><DemoBadge tone="violet" variant="outline" className={styles.longBadge}>comparison at a narrow viewport</DemoBadge></div>
        </div>
      </Demo>
      <p className={styles.optionNote}>The chip has no hover behavior or focus stop. The long label wraps instead of forcing horizontal overflow.</p>
    </section>
    ...
    <section aria-labelledby="badge-real-use">
      ...
      <p className={styles.optionNote}>The reference keeps its own teaching labels. A badge may name an adjacent category, but it must stay outside any measured border layout.</p>
    </section>
  </main>
}
