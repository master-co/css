...
function PreviousLabel({ className, ...props }: Parameters<typeof DemoLabel>[0]) {
  return <span {...props} className={[styles.previousLabel, className].filter(Boolean).join(' ')} />
}
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 08</div>
    <h1>Demo label</h1>
    <p className={styles.intro}>Labels identify a class, object or condition without becoming the lesson. The adopted shared label keeps the original Guide’s quiet monospace character and improves small-text readability.</p>
    <div className={styles.reviewNote} role="note">Approved direction: the shared <code>DemoLabel</code> uses the refined text treatment. The original Guide labels retain their established styling.</div>

    <section aria-labelledby="label-options">
      <h2 id="label-options">Class and metadata labels</h2>
      <p className={styles.sectionCopy}>Each treatment shows the same class, descriptive name, and long utility string above a specimen. The cards and their content are identical.</p>
    ...
    <section aria-labelledby="label-real-use">
      ...
      <ShadowScaleDemo />
      <p className={styles.optionNote}>The shared label changes text treatment only. The lesson continues to own its card geometry, alignment and spacing.</p>
    </section>
  </main>
}
