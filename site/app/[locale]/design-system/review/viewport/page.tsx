...
function Preview({ candidate = false }: { candidate?: boolean }) {
  return <Demo padding="none" background="plain">
    <div className={candidate ? styles.candidate : styles.current}>
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 14</div>
    <h1>Demo viewport</h1>
    <p className={styles.intro}>Viewport-dependent classes have to run inside a viewport whose width really changes. This comparison keeps one authored Save button, the actual preset breakpoint and an iframe for both shared treatments.</p>
    ...
    <section aria-labelledby="viewport-options">
      ...
      <p className={styles.sectionCopy}>Both shared treatments use the same iframe document and width controls. The candidate changes only the surrounding chrome.</p>
    ...
    <section aria-labelledby="viewport-real-use">
      ...
      <p className={styles.optionNote}>The Reference keeps control of its teaching geometry. Viewport decoration must not make an outer container query look like a real media query.</p>
    </section>
  </main>
}
