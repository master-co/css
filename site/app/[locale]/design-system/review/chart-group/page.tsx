...
function PreviousChartGroup() {
  return <div className={styles.previousGroup}>
    <div className={styles.previousHeading}><h4>Compressed CSS</h4><span>Brotli · 10 samples</span></div>
    <BenchmarkBars items={items} unit="kB" />
  </div>
}

const options = [
  { number: '01', title: 'Original Guide', detail: 'Chart beneath the document heading', preview: <div className={styles.original}><h4>Compressed CSS</h4><BenchmarkBars items={items} unit="kB" /></div>, note: 'The original benchmark page gives the chart its context in the document heading and figure caption. The bars, values, and colors already work well.' },
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 20</div>
    <h1>Benchmark chart group</h1>
    <p className={styles.intro}>A chart title should explain what the bars measure without competing with the data. The original Guide chart and its full figure caption remain useful; this review concerns the optional shared heading around a chart.</p>
    <div className={styles.reviewNote} role="note">Adopted shared style: <code>BenchmarkChartGroup</code> now uses the refined metric heading. The published Guide keeps its original chart and figure caption.</div>
    <label htmlFor="chart-review-theme" className={styles.themeControl}><span>Preview theme</span><span className={styles.themeSelect}>Light · Dark · System<ThemeSelect id="chart-review-theme" aria-label="Preview theme" /></span></label>

    <section aria-labelledby="chart-options">
      <h2 id="chart-options">Chart treatments</h2>
      <p className={styles.sectionCopy}>Each treatment uses the same illustrative data, including a zero value. The bars themselves are the existing Guide component.</p>
    ...
    <section aria-labelledby="chart-real-use">
      <div className={styles.sectionHeading}><div><h2 id="chart-real-use">Actual Guide use</h2><p>The real `docs` fixture chart uses committed measurements and keeps its explanation in the document.</p></div><Link href="/guide/benchmarks#static-css-output-structure-and-production-build">Open /guide/benchmarks</Link></div>
      <div className={styles.realUse}><StaticCSSOutputChart /></div>
      <p className={styles.optionNote}>This original chart is not converted into a group by the review. Its nearby figure caption, sources and limitations stay in the Guide.</p>
    </section>
  </main>
}
