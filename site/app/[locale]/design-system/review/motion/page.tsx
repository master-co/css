...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 15</div>
    <h1>Demo motion</h1>
    <p className={styles.intro}>An animated specimen needs an obvious place to play and controls that never become part of the movement. The Guide keeps its original token demonstration; both shared versions pause on load and animate the same item.</p>
    <div className={styles.reviewNote} role="note">Adopted default: the shared <code>DemoMotion</code> now separates its animation stage from the playback deck. Play, pause and replay the two shared examples. Replay restarts the actual animation timeline; the Guide continues to follow the OS motion preference.</div>

    <section aria-labelledby="motion-options">
      <h2 id="motion-options">Playback for one specimen</h2>
      <p className={styles.sectionCopy}>The candidate changes the chrome surrounding the animation, not its duration, easing or rotation.</p>
    ...
    <section aria-labelledby="motion-real-use">
      ...
      <p className={styles.optionNote}>The control in that lesson directly changes the element’s CSS play state. Shared playback controls are for explaining an animation timeline when the lesson itself does not supply a control.</p>
    </section>
  </main>
}
