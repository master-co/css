...
const options = [
  {
    ...
    preview: <OriginalDemo><div className={styles.originalStage}>
      <ResizeZone width="100%" originX="center" showRuler>
        <div className={styles.originalContent}><strong>Workspace layout</strong><p>Drag the edge to reveal the original viewport ruler.</p></div>
      </ResizeZone>
    </div></OriginalDemo>
  },
  {
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 10</div>
    <h1>Demo measure</h1>
    <p className={styles.intro}>Measurements should explain an actual box boundary. The adopted shared measure gives the dimensions clearer hierarchy while retaining the Guide’s thin ruler language.</p>
    <div className={styles.reviewNote} role="note">Approved default direction: the shared <code>DemoMeasure</code> uses the refined annotation. Use the width slider and detail toggle to test both axes. The controls sit outside the measured object.</div>

    <section aria-labelledby="measure-options">
      <h2 id="measure-options">Live content bounds</h2>
      <p className={styles.sectionCopy}>The previous and adopted treatments measure the same panel. The original Guide demonstrates its own draggable ruler, which remains unchanged.</p>
    ...
    <section aria-labelledby="measure-real-use">
      ...
      <p className={styles.optionNote}>The Reference keeps its own true browser measurements. A shared annotation must report the box it observes rather than a requested CSS width.</p>
    </section>
  </main>
}
