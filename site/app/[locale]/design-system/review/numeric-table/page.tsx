...
const options = [
  { number: '01', title: 'Original Guide', detail: 'Pink spacing specimen', className: '', preview: <SpacingTable variant="original" />, note: 'The original uses the pink diagonal field and raised blocks to make each gap visible. The table keeps the established Guide typography.' },
  { number: '02', title: 'Current', detail: 'Neutral demo pattern', className: '', preview: <SpacingTable variant="current" />, note: 'The shared table changed the gap specimen to a neutral stripe and demo blocks. Numeric values and column order stayed the same.' },
  { number: '03', title: 'Adopted', detail: 'Refined pink scale table', className: 'review-numeric-table-candidate', preview: <SpacingTable variant="candidate" />, note: 'The adopted shared style restores the pink spacing specimen, gives value columns stable widths and a quieter row rhythm, and lets a narrow table scroll with keyboard focus.' }
] as const
...
export default function Page() {
  return <main className="review-numeric-table-review">
    ...
    <p className="review-numeric-table-intro">The Spacing Guide already uses a useful pink field to reveal gaps. The numeric table should preserve that visual language while keeping token values, pixel equivalents and descriptions easy to scan.</p>
    <div className="review-numeric-table-reviewNote" role="note">Adopted shared style: <code>ThemeNumberVariableTable</code> uses the refined pink specimen for spacing. The published Guide keeps its surrounding content and layout.</div>
    ...
    <section aria-labelledby="numeric-options">
      ...
      <p className="review-numeric-table-sectionCopy">The same five manifest spacing tokens appear in each treatment. Their actual values set the gaps in the specimens.</p>
