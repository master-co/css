...
import ColorPaletteItem from 'internal/components/ColorPaletteItem'
import { getColor } from 'internal/data/color-palette'
...
const options = [
  {
    ...
    preview: <div className={styles.original}><ColorPaletteItem color={getColor('blue', 60)} colorName="blue" level={60} /><p>Click the swatch to copy its color.</p></div>
  },
  {
    ...
    preview: <div className={styles.previous}><Demo><DemoCopyButton value={token} label="Copy blue token" icon={null} className="demo-button">Copy blue token</DemoCopyButton></Demo></div>
  },
  {
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 26</div>
    <h1>Demo copy button</h1>
    <p className={styles.intro}>Copy controls should state the value they copy and report whether the browser accepted the action. The adopted control keeps the native button and makes the action and status easier to scan.</p>
    <div className={styles.reviewNote} role="note">Adopted direction: shared text copy controls now have a clear icon, refined button and readable result. The Guide palette remains unchanged.</div>
    <label htmlFor="copy-review-theme" className={styles.themeControl}><span>Preview theme</span><span className={styles.themeSelect}>Light · Dark · System<ThemeSelect id="copy-review-theme" aria-label="Preview theme" /></span></label>

    <section aria-labelledby="copy-options">
      <h2 id="copy-options">Copy one color reference</h2>
      <p className={styles.sectionCopy}>Activate the current and candidate buttons with mouse or keyboard. A status line below each button reports the value or explains clipboard denial.</p>
    ...
    <section aria-labelledby="copy-real-use">
      ...
      <div className={styles.guidePalette}><ColorPalette filterColors={['blue']} /></div>
      <p className={styles.optionNote}>The Design System palette can copy variable references; the Guide palette continues to copy literal palette values.</p>
    </section>
  </main>
}
