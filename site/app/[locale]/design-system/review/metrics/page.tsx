...
function PreviousMetrics() {
  return <dl className={styles.previousMetrics}>
    {metrics.map(metric => <div className={styles.previousMetric} key={String(metric.label)}>
      <dt>{metric.label}</dt><dd className={clsx(styles.previousValue, benchmarkToneTextClasses[metric.tone ?? 'neutral'])}>{metric.value}</dd><dd className={styles.previousDetail}>{metric.detail}</dd>
    </div>)}
  </dl>
}
...
export default function Page() {
  return <main className={styles.review}>
    <div className={styles.kicker}>Design system · Component review 21</div>
    <h1>Benchmark metrics</h1>
    <p className={styles.intro}>The benchmark Guide has a useful full-data table. The newer summary list should provide a quick read of selected metrics without replacing that table or changing the recorded numbers.</p>
    <div className={styles.reviewNote} role="note">Adopted shared style: <code>BenchmarkMetrics</code> now uses the refined measurement rows. The published Guide keeps its original tables.</div>
    <label htmlFor="metrics-review-theme" className={styles.themeControl}><span>Preview theme</span><span className={styles.themeSelect}>Light · Dark · System<ThemeSelect id="metrics-review-theme" aria-label="Preview theme" /></span></label>

    <section aria-labelledby="metrics-options">
      <h2 id="metrics-options">Metric treatments</h2>
      <p className={styles.sectionCopy}>The same illustrative values appear in all three treatments. Look at label-to-value alignment and where the detail sits as the width changes.</p>
    ...
    <section aria-labelledby="metrics-real-use">
      <div className={styles.sectionHeading}><div><h2 id="metrics-real-use">Actual Guide use</h2><p>The live static-comparison summary is still rendered from committed benchmark results.</p></div><Link href="/guide/benchmarks#static-css-output-structure-and-production-build">Open /guide/benchmarks</Link></div>
      <div className={styles.realUse}><StaticComparisonSummary /></div>
      <p className={styles.optionNote}>The candidate affects only the optional compact summary list. Guide tables and their surrounding caveats remain intact.</p>
    </section>
  </main>
}
