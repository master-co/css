...
```tsx
<DemoMedia className="w:full r:sm" aria-label="Sun above two mountain ridges" />
...
```tsx
<DemoMotion>
  <div className="grid place-items:center h:40x">
    <DemoItem className="size:16x animation:rotate|2s|linear|infinite">↗</DemoItem>
  </div>
</DemoMotion>
```
...
`InstallationModeTabs` reuses the site’s navigation treatment for sibling setup guides. Pass translated labels, canonical destinations and any existing alias routes. One matching link receives `aria-current="page"`; aliases affect selection without changing the destination. These are ordinary links with native keyboard behavior, not an in-page tab panel. See [Blazor runtime rendering](/guide/installation/blazor/runtime-rendering) for an alias that selects the canonical runtime guide.

<InstallationModeTabs items={[
  { href: '/guide/installation/blazor', aliases: ['/guide/installation/blazor/runtime-rendering'], label: 'Runtime Rendering' },
  { href: '/guide/installation/blazor/static-rendering', label: 'Static Rendering' },
]} />
...
<DocumentOptions label="Example settings" options={[
  { name: 'root-size', defaultValue: '16', description: <>Divisor for numeric lengths converted to <code>rem</code>. It does not change the browser’s root font size.</> },
...
`theme` adds a real document-class switch; define `mode-trigger: class` in the
...
<Demo title="Container query" padding="none">
  <DemoContainer title="Card">
    <section className="container">
      <article className="grid-cols:1 grid-cols:2@container(md)">
...
<DemoScrollArea aria-label="Layers" className="h:40x">
  {/* overflowing content */}
</DemoScrollArea>
<DemoMotion>
  <DemoItem className="size:16x animation:rotate|2s|linear|infinite" />
</DemoMotion>
