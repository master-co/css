...
The default is a neutral, soft, medium badge. Its font-relative padding is explicit;
...
`DemoAsset` pairs original artwork with a named surface and a native download link. Use it for logos, icons or exported diagrams; it is a presentation frame, not a measured CSS layout. Intrinsic image dimensions reserve the aspect ratio. The preview constrains images to the available width and a 6rem height without cropping.
...
`label`, native button attributes and children; its status confirms successful writes
