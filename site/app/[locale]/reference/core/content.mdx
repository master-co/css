...
```js
import { MasterCSS, createCSS, previewCSS } from '@master/css'
...
### `previewCSS()`
Generate CSS for class names without mutating a `MasterCSS` instance. Use `variableNames` and `animationNames` when previewing native CSS that references generated theme variables or keyframes.
<DocFn>
    {[
        { name: 'css', type: 'MasterCSS' },
        { name: 'classNames', type: 'readonly string[]' },
        { name: 'options?', type: 'PreviewCSSOptions' },
        { name: 'return', type: 'string' }
    ]}
</DocFn>
```ts
const text = previewCSS(css, ['text-center'], {
    variableNames: ['color-card'],
    animationNames: ['fade']
})
```
