...
const customCSS = new MasterCSS(config, customConfig)
```
<DocFn>
    {[
        { name: 'baseConfig', type: 'Config?', defaultValue: 'undefined' },
        { name: 'customConfig', type: 'Config?', defaultValue: 'undefined' },
    ]}
</DocFn>

### `css.config`
The `css.config` property is the resolved config used by the engine. `new MasterCSS(baseConfig, customConfig)` merges both configs; `createCSS(customConfig)` passes the default config as the base config.
