...
```html
<!-- @MARK w:full --><!-- @MARK max-w:screen-md --><!-- @MARK mx:auto -->
...
Use text-relative units when the maximum width is based on reading comfort instead of the screen scale.
```html
<!-- @MARK max-w:72ch -->
...
### Use viewport and screen caps
Use `max-vw` when an element should never exceed the viewport width. Use `max-w:screen-*` when the cap should follow the configured screen scale.
```html
<!-- @MARK max-vw --><!-- @MARK max-w:screen-lg@lg -->
<section class="max-vw max-w:screen-lg@lg">
    ...
</section>
```
...
```html
<div class="max-w:full max-w:screen-sm@sm max-w:72ch@print">...</div>
```
