...
```
`CSSRuntime` uses the provided config directly. Use `initCSSRuntime(config)` when the runtime should extend the default config with a project config.
...
<DocFn>
    {[
        { name: 'config?', type: 'Config | Config[]', defaultValue: 'undefined' },
