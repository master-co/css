...
Override a preset line color token when separators need a different mode-aware value.
```css name=index.css
@theme light {
    --color-line-subtle: $color-slate-60/.2; /* [!code highlight] */
}

@theme dark {
    --color-line-subtle: $color-gray-30/.2; /* [!code highlight] */
}
```
Apply the token to a border color.
```html
<div class="b:1px|solid border-color:subtle"></div>
```
