...
### `@source`
Maps to scanner `include` and broadly includes files that may contain classes.
...
### `@source not`
Maps to scanner `exclude` and removes noisy files such as tests, generated output, or vendor code.
...
### `@source required`
Maps to scanner `required` and scans mandatory files even when they would otherwise be excluded.
