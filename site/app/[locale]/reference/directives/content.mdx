...
`<color>` supports both the `~color` theme namespace and raw CSS color values. For example, `bg:<color>` behaves like `bg:<~color|color>`, so `bg:primary` can resolve `--color-primary` while `bg:#fff` remains a raw color. Put more specific color namespaces before `color` when needed, such as `text:<~color-text|~color|color>`.
...
```css
.card {
    @compose grid gap:md p:lg r:xl shadow:md bg:white;
}
...
.notice {
    ...
    @dark {
        @compose bg:gray-90 text:strong;
    }
