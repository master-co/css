...
Dynamic colon entries use `key:<source-list>` and keep the class syntax as `key:value`. Source entries can be theme namespaces with `~` or `=`, or one raw value kind: `number`, `color`, or `image`.
