...
@components {
    btn {
        @compose inline-flex align-items:center pi:md h:10x r:lg;
    }
}
```
...
`<color>` supports both the `~color` theme namespace and raw CSS color values. For example, `bg:<color>` behaves like `bg:<~color|color>`, so `bg:primary` can resolve `--color-primary` while `bg:#fff` remains a raw color. Put more specific color namespaces before `color` when needed, such as `text:<~color-text|color>`.
...
`*` is for raw-any full-value wrappers, such as paired vendor declarations. It may be used alone or with namespace refs, as in `text-decoration:<~color-text|~color|*>`, but it cannot be mixed with enum values or raw kinds. The `<color>` shorthand does not apply to `*` wrappers; list `~color` explicitly when raw-any values should also resolve color tokens.
...
```css
@safelist "dialog-open transform:translateY(-5):hover bg:blue-60@dark";
```
