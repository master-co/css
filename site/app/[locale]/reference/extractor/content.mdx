...
`cwd` is used to resolve project CSS plan entries and source patterns such as [`.include`](#include), [`.exclude`](#exclude), and [`.required`](#required).
...
### `options.plan`
Pass an explicit compiled MasterCSSPlan. When omitted, integrations and CLI flows load the project CSS entry plan from `cwd`.
<DocProp defaultValue={options.plan} types={['MasterCSSPlan']} />
