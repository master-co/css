...
### `options.shakeNative`
Prune native CSS class selector rules in linked stylesheets based on extracted class usage.
<DocProp defaultValue={options.shakeNative} types={['boolean']} />

When enabled, a stylesheet that imports the virtual Master CSS module becomes a linked native stylesheet:
...
The virtual import is removed, local relative CSS imports are expanded, and native class rules are kept only when their class names are found by the extractor. Stylesheets that do not import the virtual module are left untouched.
