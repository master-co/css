...
Local relative `.css` imports are expanded, `@master shake;` is removed from the compiled output, and native class rules are kept only when their class names are found by the extractor. The `virtual:master.css` import is optional for native shaking; use it only where the generated Master CSS should be inserted.
