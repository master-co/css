...
### CSS directives
Extractor options can also be declared in CSS. When these directives are loaded through the `master.css` config graph, they become global extractor defaults. When they appear in a stylesheet root, they only affect that root.
...
### Native CSS shaking
Prune native CSS class selector rules from a stylesheet by declaring `@master shake;` in that stylesheet:
...
Local relative `.css` imports are expanded, `@master shake;` is removed from the compiled output, and native class rules are kept only when their class names are found by the extractor. See [Shaking unused classes](/guide/shaking-unused-classes) for the full model.
