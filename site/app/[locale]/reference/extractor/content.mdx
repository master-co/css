import options from '~/packages/extractor/src/options'

## Installation [sr-only]
```bash action=install
@master/css-extractor
```

---

## API Reference
### CSSExtractor

```js
import { CSSExtractor } from '@master/css-extractor'
```
Create an instance of the Master CSS extractor:
```js
const extractor = new CSSExtractor(options, cwd)
```

`cwd` is used to resolve project CSS manifest entries and source patterns such as [`.include`](#include), [`.exclude`](#exclude), and [`.required`](#required).

`CSSExtractor` is the source-scanning engine behind Master CSS Static Rendering, which is usually used by build tools or third-party package authors.

This package is specific to the **Node.js** environment.

To start using Master CSS static rendering, check out the [guide](http://localhost:3000/guide/installation) first.

🚧 Check out the [source code](https://github.com/master-co/css/blob/dev/rc/packages/extractor/src/core.ts) temporarily.

### `options`
<Code lang="ts">{JSON.stringify(options, null, 4)}</Code>
{/* <OptionsCodes vite webpack>
    {JSON.stringify(options, null, 4)}
</OptionsCodes> */}
The above are the default compilation options, you can also introduce these default values to expand your options:
```js
import { options } from '@master/css-extractor'
// or in a side-effect-free way
import options from '@master/css-extractor/options'
```

### `options.manifest`
Pass an explicit compiled MasterCSSManifest. When omitted, integrations and CLI flows load the project CSS entry manifest from `cwd`.
<DocProp defaultValue={options.manifest} types={['MasterCSSManifest']} />

### `options.include`
Broadly specify source files/directories to be scanned by Master CSS.
<DocProp defaultValue={options.include} types={['FastGlobPattern[]']} />

### `options.exclude`
Broadly exclude source files/directories from scanning by Master CSS.
<DocProp defaultValue={options.exclude} types={['FastGlobPattern[]']} />

### `options.required`
Mandatory source files/directories are to be scanned by Master CSS.
<DocProp defaultValue={options.required} types={['FastGlobPattern[]']} />
This option is similar to [`.include`](#include) but not excluded by [`.exclude`](#exclude). Usually, you will use it to specify what is accidentally excluded by [`.exclude`](#exclude) file/directory.

### `options.safelist`
Generate fixed CSS rules based on the specified class name, regardless of source.
<DocProp defaultValue={options.safelist} types={['string[]']} />
Typically you would use it with the following limitations:
- Class names that do not exist in the source code.
- Irregular class names.
- Class names from asynchronous data.

### `options.blocklist`
Exclude class names accidentally accepted by the scanning process.
<DocProp defaultValue={options.blocklist} types={['(string | RegExp)[]']} />

### CSS directives
Extractor options can also be declared in CSS. For the full stylesheet syntax, see [CSS directives](/reference/directives).

When these directives are loaded through the project CSS entry graph, they become global extractor defaults. When they appear in a stylesheet root, they only affect that root.

```css
@source 'src/**/*.{html,js,jsx,ts,tsx,vue,svelte,astro,md,mdx}';
@source not 'src/**/*.test.tsx';
@source required 'src/generated.tsx';

@safelist 'dialog-open bg:blue-60@dark';
@blocklist 'debug-*';
```

`@source` maps to `options.include`, `@source not` maps to `options.exclude`, `@source required` maps to `options.required`, `@safelist` maps to `options.safelist`, and `@blocklist` maps to `options.blocklist`.

Bare source globs are resolved from `cwd`. Globs that start with `./` or `../` are resolved relative to the CSS file that declares the directive. `@blocklist` accepts exact strings and `*` / `?` wildcard patterns.

### Native CSS pruning
Prune native CSS class selector rules from a stylesheet by importing `@master/css` in that stylesheet:

```css
@import "@master/css";
@import "./styles/btn.css";

.card {
    color: red;
}

.unused {
    color: blue;
}
```

Local relative `.css` imports are expanded, the Master import is replaced by generated CSS, and native class rules are kept only when their class names are found by the extractor. Add `@preserve native;` to preserve native CSS in a Master-managed root. See [Native CSS pruning](/guide/native-css-pruning) for the full model.

### `extractClassCandidates()`
Extract unvalidated class candidates from source content.
<DocFn>
    {[
        { name: 'content', type: 'string', defaultValue: 'undefined' },
        { name: 'return', type: 'string[]' },
    ]}
</DocFn>
```ts
import { extractClassCandidates } from '@master/css-source'

const content: string = `
    import { setupCounter } from './counter'

    const counterElement = document.querySelector<HTMLButtonElement>('#counter')
    const syntax = 'block'
    counterElement?.classList.add('transition:transform|.3s', 'transform:translateY(-5):hover', syntax)

    setupCounter(counterElement!)
`
const result = extractClassCandidates(content)
```
Result:
```js
[
    'const',
    'counterElement',
    'syntax',
    'block',
    'transition:transform|.3s',
    'transform:translateY(-5):hover',
    'setupCounter(counterElement!)'
]
```
