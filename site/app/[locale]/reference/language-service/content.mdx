...
Enable semantic token rendering.
<DocProp defaultValue={JSON.stringify(settings.renderSemanticTokens)} types={['boolean']} />

Semantic tokens classify Master CSS spans as `class`, `property`, `variable`, `function`, `number`, `string`, `keyword`, `modifier`, or `operator`. Components defined in config are emitted as `class` tokens with the `declaration` modifier.
