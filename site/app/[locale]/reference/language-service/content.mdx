...
Semantic tokens are generated with the configured Master CSS engine, so custom variables, components, utilities, modes, and other config-dependent tokens can be classified after `master.css.*` is loaded by the caller.
