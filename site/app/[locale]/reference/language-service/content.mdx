...
The token types stay within the exported LSP legend and use CSS-native categories where possible: declaration keys use `property`, values and semantic utility names use `enumMember`, selector class names and component utilities use `class`. CSS-like punctuation and operators also carry role-derived modifiers such as `declarationTerminator`, `declarationSeparator`, `blockBrace`, `selectorCombinator`, and `functionPunctuation` so editors can map them to native CSS scopes.
