...
Semantic tokens are generated with the configured Master CSS engine, so custom variables, components, utilities, modes, and other config-dependent tokens can be classified after the project config is loaded by the caller.

Use `renderSemanticTokensAtPosition()` when a client wants active-only highlighting for the utility class or CSS directive at the current editor position.
```ts
