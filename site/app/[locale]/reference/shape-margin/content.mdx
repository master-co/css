...
<img class="float:left shape:circle(50%) shape-margin:2rem" src="/images/photo.jpg" alt="Photo">
```

### Pair with shape outside
`shape-margin:*` only has an effect when the element participates in shape layout, usually with `float:*` and `shape:*`.
```html
<img class="float:left shape:circle(50%) shape-margin:1rem" src="/images/avatar.jpg" alt="">
```
