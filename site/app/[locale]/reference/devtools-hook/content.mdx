...
<Code lang="ts">{require('~/packages/devtools-hook/src/types/event-callbacks.ts?raw').default}</Code>

### `EventNames`
The type definition for the available event names.
<Code lang="ts">{require('~/packages/devtools-hook/src/types/event-names.ts?raw').default}</Code>
