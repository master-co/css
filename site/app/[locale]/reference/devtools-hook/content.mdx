...
const onMutated = (context) => {
  ...
  console.log('Class usages:', context.classCounts)
...
<Code lang="ts">{require('!!raw-loader!~/packages/devtools-hook/src/types/event-callbacks.ts').default}</Code>

### `EventNames`
The type definition for the available event names.
<Code lang="ts">{require('!!raw-loader!~/packages/devtools-hook/src/types/event-names.ts').default}</Code>
