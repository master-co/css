...
### `settings.plan`
Specify an explicit compiled MasterCSSPlan. By default, ESLint loads the project CSS entry discovered from the current working directory.
    ...
    export default [
        {
            settings: {
                "@master/css": {
                    plan: ${JSON.stringify(settings.plan, null, 4)}
                }
            }
        }
    ]
...
If the discovered project CSS entry is not the plan you want for this ESLint run, pass an explicit compiled plan:
```js name=eslint.config.js
export default [
    {
        settings: {
