...
### `settings.config`
Specify a custom Master CSS configuration or path. By default, it reads `master.css` based on the current working directory.
...
If the `master.css` file cannot be found, you can specify the path:
```js name=eslint.config.js
