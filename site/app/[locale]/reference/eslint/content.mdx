...
### `settings.manifest`
Specify an explicit compiled MasterCSSManifest. By default, ESLint loads the project CSS entry discovered from the current working directory.
    ...
    export default [
        {
            settings: {
                "@master/css": {
                    manifest: ${JSON.stringify(settings.manifest, null, 4)}
                }
            }
        }
    ]
...
If the discovered project CSS entry is not the manifest you want for this ESLint run, pass an explicit compiled manifest:
```js name=eslint.config.js
export default [
    {
        settings: {
