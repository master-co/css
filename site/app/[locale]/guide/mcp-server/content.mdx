...
Use `migrate-to-mastercss` when a project is moving from CSS, CSS Modules, Sass, Tailwind CSS, CSS-in-JS, component-library styling, Master CSS v1, or [Master CSS v2 RC](/guide/migration/v2-rc). A v2 RC upgrade saves the original manifest and browser baseline before coordinating package, artifact, and source updates; it does not run old and new runtimes together. The prompt should make the agent inspect the workspace first, then return a plan with:
...
Every tool returns a version 3 object with `metadata`, `diagnostics` and a required `result`. Success has `result: { status: 'success', data }`; failure has `result: { status: 'error', error: { code, message } }`. Tool failures also set MCP `isError`. JSON text and `structuredContent` contain the same object. Use the tool's published `outputSchema` to validate both branches.

`metadata` includes package/language/binding versions, the context actually used, manifest fingerprint, entries and dependencies. Unavailable manifest information is null or an empty array. Project context is the default: an absent entry or failed compilation returns an error, without switching to the preset. Request `context: 'preset'` explicitly for preset-only queries.

Class inspection separates `matchStatus`, `cssSyntaxStatus`, `cssValueStatus` and `browserSupport`. Check records identify their name, version, phase and scope. A CSS grammar error is different from an unknown future capability; `var()` does not establish a computed value. Browser support stays `not-checked` unless a browser check was actually executed. Preserve diagnostics, dependency arrays and ambiguous-name alternatives when consuming reports.

Markdown/MDX extraction distinguishes live markup and code from display text, and returns original UTF-16 occurrence ranges with inclusion/exclusion reasons. The CLI, editor and MCP share Rust extraction and matching. CLI report versions are independent of the MCP envelope; read each report's declared version.

For CLI reports, `cwd` is the command root. For MCP payloads, `root` is the configured workspace root.
...
1. A preview tool such as `mastercss_preview_fixes` or `mastercss_preview_directive_format` creates a diff preview and returns a confirmation token.
