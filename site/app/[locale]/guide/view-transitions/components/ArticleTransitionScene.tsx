  ...
  return <main className="container p-md bg-surface-base text-body">
    ...
    {selected ? <article data-article-detail className="overflow:hidden demo-surface">
      <Image alt={selected.imageAlt} src={selected.image} placeholder="blur" sizes="(max-width: 600px) 100vw, 600px"
        className={clsx(selected.imageTransition, shared, 'block h:auto w:100% video object-cover')} />
      <div className="p-lg">
        ...
        <h1 ref={heading} tabIndex={-1} className={clsx(selected.titleTransition, shared, 'mb:0 mt-sm text-2xl font-semibold outline:2px|solid|var(--color-blue):focus-visible outline-offset-4xs:focus-visible')}>{selected.title}</h1>
