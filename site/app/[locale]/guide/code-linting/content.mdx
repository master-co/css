...
```css
.btn {
    @compose font:16px margin:md; /* [!code --] */
    @compose font:md m:md; /* [!code ++] */
}
```

Plain `.css` files still need an ESLint CSS parser or language setup so ESLint can parse the file before Master CSS rules run. Framework files with parsed `<style>` blocks, such as Vue and Svelte files, can lint `@compose` in those style blocks.
