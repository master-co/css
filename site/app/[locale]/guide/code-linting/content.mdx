...
| `@master/css/no-conflicting-classes` | `warn` | Yes | Report classes that target the same declarations in the same variant scope. |
...
(!) `"m:0.625rem" applies the same declarations as "m:5x".`
...
When a conflict can be fixed automatically, the rule removes the later conflicting class and keeps the earlier class. Review the fix if the authoring order was carrying intent.
