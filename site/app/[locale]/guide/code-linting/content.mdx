...
You can also restrict teams to using [configuration](/guide/configuration) and [components](/guide/components) instead of defining classes through traditional `html <style>` or CSS files.
...
```
(o) Or create the custom style `btn` using [components](/guide/components) in the project CSS entry.
```css name=index.css
@master {
    .btn {
