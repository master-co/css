...
You can also restrict teams to using [theme](/guide/theme) and [component classes](/guide/reusing-styles#components) instead of defining classes through traditional `html <style>` or CSS files.
...
```
(o) Or create the custom style `btn` using [component classes](/guide/reusing-styles#components) in the project CSS entry.
```css name=index.css
@layer components {
    .btn {
