...
```mcss
bg:white font:md flex px:4x rel w:full shadow:lg rounded z:10 gap:0.5rem <!-- [!code --] -->
rel z:10 flex gap:0.5rem w:full px:4x rounded font:md bg:white shadow:lg <!-- [!code ++] -->
```
...
```mcss
margin:16px block@dark@sm <!-- [!code --] -->
m:16px block@sm@dark <!-- [!code ++] -->
```

Canonical fixes preserve value sources and cascade priority. A literal stays a literal; `font-size:16px` is not replaced by a token that happens to equal `16px` today. Explicit `var()` references and multi-value declarations retain their identities. A shorter static utility or composition is not suggested when it changes the existing shorthand, longhand, or value-source priority.

Use the explicit [v2 RC migration command](/guide/migration/v2-rc#6-preview-and-apply-migration-proposals) for historical token syntax. Canonical linting does not guess the meaning of old colon tokens.

The same rules apply inside parsed `@compose` class lists. Native declarations and variant blocks can make the retained intent explicit:
...
| --- | --- | --- |
| `preferStaticUtilities` | `true` | Recommend equivalent named utilities only when value source and cascade priority are preserved. |
| `preferPropertyAliases` | `true` | Prefer fixed property aliases such as `m:16px` over `margin:16px`. |
| `preferCompositionUtilities` | `true` | Recommend a composition only when it preserves the existing priority and declarations. |
