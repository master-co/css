<StepSection>
    <Step $row>
        <StepL>
            ### <StepNum />Create a project
            If you don't have a Blazor project, create one first. It's recommended to refer to [Build your first Blazor app](https://dotnet.microsoft.com/en-us/learn/aspnet/blazor-tutorial/create)
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'Terminal',
                        lang: 'bash',
                        code: `
                            dotnet new blazorserver -o project --no-https -f net6.0
                            cd project
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Create a New Folder
            Add a new folder named npm_packages to root directory.
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'Terminal',
                        lang: 'bash',
                        code: `
                            mkdir npm_packages
                            cd npm_packages
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Set up NPM
            Set up NPM and install the required webpack dependencies.
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'Terminal',
                        lang: 'bash',
                        code: `
                            npm init -y
                            npm install webpack webpack-cli --save-dev
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Create a configuration file
            Run the command to create a [master.css](/guide/configuration) file.
        </StepL>
        <StepR>
            ```bash action=create
            @master/css@rc
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Install Master CSS
            Install Master CSS into your project via package managers.
        </StepL>
        <StepR>
            ```bash action=install
            @master/css-runtime@rc
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Set up CSS runtime engine
            Create a new file `npm_packages/src/index.js` and initialize the runtime engine. This manual setup uses the default runtime config; use an official build integration when application code needs to import the compiled `master.css` config object.
        </StepL>
        <StepR>
            ```js name=npm_packages/src/index.js
            …
            import { initCSSRuntime } from '@master/css-runtime'; // [!code highlight]

            initCSSRuntime() // [!code highlight]
            ```
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Link the default stylesheet
            Link the Master CSS default stylesheet from the Blazor document layout.
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'Pages/_Layout.cshtml',
                        lang: 'html',
                        code: `
                            <html>
                                <head>
                                    <link rel="stylesheet" href="https://cdn.master.co/css@rc/base.css"> <!-- [!code highlight] -->
                                </head>
                            </html>
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Add an NPM build script
            Add an NPM build script on script section of the `package.json` file.
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'package.json',
                        lang: 'js',
                        code: `
                            {
                                ...
                                scripts: {
                                    ...
                                    "build": "webpack ./src/index.js --output-path ../wwwroot/js --output-filename index.bundle.js --mode=development" // [!code highlight]
                                }
                            }
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Add prebuild step
            Add a prebuild step that will run `npm install` and `npm run build` whenever compile or build the application to the `project.csproj` file.
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'project.csproj',
                        lang: 'html',
                        code: `
                            <Project>
                                ...
                                <Target Name="PreBuild" BeforeTargets="PreBuildEvent"> <!-- [!code highlight] -->
                                    <Exec Command="npm install" WorkingDirectory="npm_packages" /> <!-- [!code highlight] -->
                                    <Exec Command="npm run build" WorkingDirectory="npm_packages" /> <!-- [!code highlight] -->
                                </Target> <!-- [!code highlight] -->
                                ...
                            </Project>
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Import bundles JavaScript file
            Webpack will create a `index.bundle.js` file in `wwwroot/js`, and import the file to entry file `Pages/_Layout.cshtml`
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'Pages/_Layout.cshtml',
                        lang: 'html',
                        code: `
                            <html>
                                ...
                                <body>
                                    ...
                                    <script src="_framework/blazor.server.js"></script>
                                    <script src="js/index.bundle.js"></script> <!-- [!code highlight] -->
                                </body>
                            </html>
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step $row>
        <StepL>
            ### <StepNum />Build and start the app
            Navigate to project directory, and build and start your app with `dotnet watch`
        </StepL>
        <StepR>
            <CodeTabs>
                {[
                    {
                        name: 'Terminal',
                        lang: 'bash',
                        code: `
                            cd project
                            dotnet watch
                        `
                    }
                ]}
            </CodeTabs>
        </StepR>
    </Step>
    <Step>
        ### <StepNum />Hello world!
        Now style your first element using Master CSS syntax!
        <CodeTabs>
            {[
                {
                    name: 'Pages/_Layout.cshtml',
                    lang: 'html',
                    code: `
                        @page "/"

                        <PageTitle>Index</PageTitle>

                        <h1 class="italic m:2xl fg:strong font:5xl font:heavy"> <!-- [!code highlight] -->
                            Hello World <!-- [!code highlight] -->
                        </h1> <!-- [!code highlight] -->

                        Welcome to your new app.

                        <SurveyPrompt Title="How is Blazor working for you?" />
                    `
                }
            ]}
        </CodeTabs>
        Open your browser to watch the changes.
        <HelloWorld url="localhost:3000" />
    </Step>
</StepSection>
