...
<StepSection>
    ...
    <Step $row>
        <StepL>
            ...
            - Define a `webpackConfig` and add a `MasterCSSPlugin` in `next.config.js`
            - Concat the `webpackConfig.plugins`

            (!) `MasterCSSPlugin` cannot be created directly in `nextConfig`, this will result in multiple instances.
        </StepL>
        <StepR>
            ```js name=next.config.js
            const MasterCSSPlugin = require('@master/css.webpack') // [!code ++]
