...
export const { Page, dynamic, revalidate, generateMetadata } = createLayout({
    metadata: {
        title: 'Set up Master CSS in ASP.NET Core',
        description: 'Guide to setting up Master CSS in ASP.NET Core Razor Pages or MVC projects.',
    ...
    content: ({ $ }) =>
        <Tabs className="mb:xl">
            <Tab href='/guide/installation/aspnet-core'>{$('Runtime Rendering')} <TabBadge>{$('Default')}</TabBadge></Tab>
