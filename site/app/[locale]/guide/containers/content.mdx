...
* Provide thresholds for `@container(<size>)` queries, so components can respond to the space they actually receive.
...
Add `container` to an ancestor whose inline size comes from the surrounding layout. Then apply `@container(<size>)` variants to its descendants. A size query cannot select the element itself as its query container.

```html
<section class="container">
  <!-- @MARK grid-cols:2@container(md) -->
  <article class="grid-cols:1 gap-lg grid-cols:2@container(md)">
    ...
  </article>
</section>
```
...
Use [comparison](/reference/rules/conditions#comparison-operators) and [logical](/reference/rules/conditions#logical-operators) operators when a component needs a bounded range.
