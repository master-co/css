...
Writing `.btn` at the top level of a stylesheet creates a normal CSS rule. Writing `.btn` in top-level `@layer components` defines an on-demand component class. Writing `.flow` in top-level `@layer utilities` defines an on-demand utility class. All three can use `@compose` and `@at`, but they serve different ownership needs.
...
Use top-level `@layer utilities` when your design system needs a reusable primitive that is still lower-level than a product component. Good utility names describe a styling capability, not a UI role.
...
CSS Modules and SFC `<style>` blocks can use `@compose` directly. They do not need `@master;` because they are local composed stylesheets, not project CSS entries.

```css name=Button.module.css
.button {
    @compose "inline-flex ai:center jc:center gap:xs px:md h:10x r:lg";
    background: var(--color-primary);
    color: white;
}
...
Local composed stylesheets share the project CSS entry and plugin inline config as context, so theme tokens, custom at tokens, custom selectors, and managed component classes remain available. Local files do not define global project config, do not insert generated Master CSS, and do not participate as native CSS shaking roots.
...
`@theme` and `@settings` blocks, top-level custom directives, top-level managed layers, top-level `@keyframes`, and native style rules with `@compose` or `@at` are consumed by the Master CSS compiler. Ordinary CSS outside those managed targets remains native CSS in your app stylesheet pipeline.
