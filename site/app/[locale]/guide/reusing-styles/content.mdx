...
Writing `.btn` at the top level of a stylesheet creates a normal CSS rule. Writing `btn` inside `@components` defines an on-demand component class. Writing `flow` inside `@utilities` defines an on-demand utility class. All three can use `@compose` and `@at`, but they serve different ownership needs.
...
`@theme` and `@settings` blocks, top-level custom directives, managed definition directives, top-level `@keyframes`, and native style rules with `@compose` or `@at` are consumed by the Master CSS compiler. Ordinary CSS outside those managed targets remains native CSS in your app stylesheet pipeline.
