...
Create a new abstract style by repeating the shared declarations and adding what differs.
```css name=master.css
@master components {
    .a {
...
```
You can see that `b` keeps the same text color as `a` and adds underline styling:
<Demo $py={0}>
    <DemoPanel>
