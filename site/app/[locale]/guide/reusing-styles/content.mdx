...
Writing `.btn` at the top level of a stylesheet creates a normal CSS rule. It is emitted by your stylesheet pipeline as native CSS, and Master CSS does not treat `btn` as a generated class. Writing `.btn` in top-level `@layer components` defines a config-backed component class instead: it is generated on demand only when `btn` is used or extracted, emitted in the `components` cascade layer, and can use Master CSS features such as `@compose`, nested selectors, `@at` conditions, modes, and variant suffixes in markup.
