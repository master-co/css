...
`@theme`, `@settings`, and `@animations` blocks, top-level custom directives, managed definition directives, and native style rules with `@compose` or `@at` are consumed by the Master CSS compiler. Ordinary CSS outside those managed targets, including top-level native `@keyframes`, remains native CSS in your app stylesheet pipeline.
