...
```html
<button class="inline-flex align-items:center justify-content:center gap:xs px:md py:xs r:lg font:sm font:medium bg:blue text:white">
    Save changes
</button>
```
...
```html
<button class="pi:action-x r:lg bg:brand text:white">
    Submit
</button>
```
...
<Demo>
    <button className="justify-center items-center inline-flex bg:black text:white font:sm font:medium h:10x pi:md r:lg outline:2px|solid|blue:focus outline-offset:0.125rem:focus">Submit</button>
</Demo>
...
.marketing-card {
    @compose flow p:lg r:xl shadow:md bg:white;
...
@components {
    yellow {
        @compose b:1px|solid|yellow-70 bg:yellow text:yellow-95;
    }
