...
@components {
    btn {
        ...
        @compose bg:brand fg:white bg:brand/.85:interactive;
    }
}
```
...
- Use `@custom-variant` for shared selector or condition names.
...
For token namespace behavior, see [Customizing your theme](/guide/theme). For directive syntax, see [CSS directives](/guide/directives). For abstraction decisions, see [Reusing styles](/guide/reusing-styles).
...
<section class="content-auto p:lg">...</section>
```
...
`@reference` makes package tokens, variants, utilities, and component classes available while compiling the local stylesheet. It does not import the referenced package's output CSS by itself, so make sure the app entry that imports the package is also loaded by the page.
...
<article class="content-auto bg:surface-raised p:lg r:lg">
    ...
</article>
```
