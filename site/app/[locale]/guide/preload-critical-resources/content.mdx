...
It may experience delays in downloading and initializing until the first contentful paint if the [runtime engine](/guide/rendering-modes#runtime-rendering) is discovered late.
...
Now the runtime engine is discovered early, so the download happens in parallel with the stylesheet:
