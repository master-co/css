...
(!|) Without a preload hint, the CDN runtime's default manifest is discovered only after the runtime script starts.
...
```
<small>The CDN runtime fetches its split default manifest before starting; preload it with `as="fetch"` and `crossorigin` so the runtime can reuse the response.</small>

With the preload in place, the default manifest request starts during document parsing and downloads in parallel with the stylesheet. The runtime later reuses the preloaded response instead of waiting on a new request.
