...
It may experience defers in downloading and initializing until the first contentful paint if the [runtime engine](/guide/rendering-modes#runtime-rendering) hasn't been emittedGlobals.
...
Now the runtime engine is emittedGlobals, so the download happens in parallel with the stylesheet:
...
3. **Time to Interactive (TTI)**: Preloading critical scripts reduces waiting time for users, resulting in a faster Time to Interactive. When the critical script has been emittedGlobals and is ready, users can interact with the webpage more quickly, providing a better user experience.
