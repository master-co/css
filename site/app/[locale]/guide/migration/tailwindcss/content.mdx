...
5. Replace reusable `@apply` rules with `@compose`, `@layer components`, `@utilities`, or native CSS.
...
Then add the framework or build integration that matches the project. Start from [Installation](/guide/installation), then choose the specific framework guide for Vite, Next.js, Nuxt, Svelte, Astro, Webpack, React, Angular, Laravel, Blazor, Lit, or the CLI.
...
```html
<article class="r-card grid-cols:3@md bg-brand p-card">
  ...
</article>
```
...
| `--spacing-*` or base spacing | `--spacing-*` | Use named spacing tokens for product decisions and explicit native units for direct values. |
...
```html
<section class="text-body bg-panel">
  ...
</section>
```
...
| --- | --- | --- |
| `p-6`, `px-4`, `mt-8` | `p:1.5rem`, `px:1rem`, `mt:2rem` | These values assume Tailwind’s default .25rem scale; check the original project configuration. Use a defined token such as `p-card` for product spacing. |
| `-mt-4` | `mt:-1rem` | Use a negative raw value, or `-mt-card` for a defined numeric token. |
| `w-full`, `h-screen`, `min-w-0` | `w:100%`, `h:100vh`, `min-w:0` | Use native values when they read better. |
...
| `grid-cols-3`, `gap-4` | `grid-cols:3`, `gap:1rem` | Grid column helpers and gap utilities are built in. |
...
```html
<section class="grid gap-lg w:100% max-w-5xl mx:auto px-md py-2xl grid-cols:3@md">
  <article class="p-lg r-lg bg-white shadow-sm">
...
| `focus-visible:outline-2` | `outline-width:2px:focus-visible` | Use native pseudo-class names when they are clearer. |
...
| `dark:bg-slate-950` | `bg-slate-95@dark` | Output follows the explicit `@mode dark` activation. |
| `motion-safe:animate-spin` | `animation:rotate|var(--duration-slowest)|linear|infinite@motion` | Built-in motion variants are `@motion` and `@reduce-motion`. |
...
```html
<button class="inline-flex items-center h:2.5rem px-md r-md bg-blue-60 fg-white bg-blue-70:hover opacity:.5:disabled">
  Save
</button>
```

Use `@custom-variant` when a Tailwind custom variant represents shared conditional product language. Keep selector states as selector suffixes or nested selectors.
...
{/* eslint-disable @master/css/no-invalid-classes -- custom variants are defined in the adjacent CSS example */}
```html
<button class="bg-blue-70:hover px-sm@compact">
  Save
</button>
```
...
.button {
  @compose inline-flex align-items:center px-md h:2.5rem r-md bg-blue-60 fg-white;
}
```
...
Tailwind `@utility` creates custom utilities that work with variants. In Master CSS, use `@utilities` for reusable primitives and `@layer components` for product roles.
...
Use `@layer components` when the name describes a product UI role rather than a low-level primitive.
...
function Progress({ value }: { value: number }) {
  return (
    <div className="overflow:hidden h:1rem r:100% bg-gray-10">
      <div
        className="h:100% w:var(--progress) bg-blue-60 transition:width|var(--duration-fast)|var(--easing-smooth)"
...
- [CSS directives](/reference#directives) for `@source`, `@safelist`, `@blocklist`, `@compose`, and `@variant`.
...
| Tailwind CSS plugin loading through `@plugin` | Master CSS does not execute Tailwind plugins. | Recreate needed output with native CSS, `@theme`, `@utilities`, or `@layer components`; keep plugin CSS temporarily when needed. |
...
| Tailwind `container` utility semantics | Master CSS `container` creates a container-query boundary. | Use `w:100% max-w:* mx:auto px:*` for page wrappers and `container` only for container queries. |
...
| Tailwind group and peer variant syntax | Master CSS does not implement `group-*` or `peer-*` variants. | Use native selectors, `:has()`, `:of()`, and combinators. |
