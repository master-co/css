...
### Add media queries
Using [at variants](/guide/theme#at-tokens) to define media queries, feature queries, and even container queries.
```js name=tailwind.config.js
/** @type {import('tailwindcss').Config} */
module.exports = {
    theme: {
...
`@compose` is not an alias for `@apply`. It only accepts a quoted Master CSS class list, and selector states should be written as nested selectors or `@variant` blocks instead of Tailwind's unquoted `@apply` syntax.
