...
| Source policy | `@source`, `@source not`, `@source inline()`, ignored paths | Use Master CSS `@source`, `@source not`, `@safelist`, and `@blocklist`. |
...
Use `@source` and `@source not` to define the scanner boundary.
