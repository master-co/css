...
**Master CSS** is a markup-driven CSS language with smart syntax that allows you to construct your user interface using structured and native-like syntax, such as `mcss font:14`, `mcss pt:16`, `mcss w:36@md`.
...
```
Not just media queries but feature queries and [more](/guide/theme#at-tokens).

---

## Configuration
### Add breakpoints
Define responsive breakpoints using theme [breakpoint tokens](/guide/theme#breakpoint-and-container-tokens).
...
```
(x) No longer need redundant breakpoint range options.
```js name=tailwind.config.js
module.exports = {
    theme: {
        screens: {
...
### Add media queries
Using [at tokens](/guide/theme#at-tokens) to define media queries, feature queries, and even container queries.
```js name=tailwind.config.js
/** @type {import('tailwindcss').Config} */
module.exports = {
    theme: {
...
```
```css name=index.css
@custom-at tall @media (min-height: 800px);
```
...
Use [component classes](/guide/reusing-styles#component-classes) and `@compose` to abstract styles instead of Tailwind's `@apply`.

`@compose` is not an alias for `@apply`. It only accepts a quoted Master CSS class list, and selector states should be written as nested selectors or `@at` blocks instead of Tailwind's unquoted `@apply` syntax.
