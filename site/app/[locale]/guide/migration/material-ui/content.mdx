...
```html
<section class="bg:surface fg:body r:control b:1px|solid|base">
    ...
</section>
```
...
export function DashboardPanel({ children }: { children: React.ReactNode }) {
    return (
        <section className="grid gap:md p:lg r:control bg:surface">
            {children}
        </section>
    )
}
...
@components {
    panel {
        @compose bg:surface fg:body r:lg b:1px|solid|base p:lg shadow:sm;
    }

    toolbar {
        @compose flex align-items:center gap:sm min-h:12x;
    }
}
```
...
```html
<section class="grid grid-cols:1 grid-cols:3@md gap:md">
    ...
</section>
```
...
- [Reusing styles](/guide/reusing-styles) for markup utilities, component classes, and custom utilities.
