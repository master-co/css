...
```
To learn more, check out [theme tokens](/guide/theme#theme-tokens).

### Using new custom
In v2, you can apply the [default](/guide/theme#motion) out of the box:
```html
...
```
To customize keyframes, check out [Customizing theme](/guide/theme#motion).
...
Deprecate extended `breakpoints` and `queries` in favor of breakpoint variables and custom variants in the project CSS entry.
```js name=config.js
import { Style } from '@master/css' /* [!code --] */
...
```css name=index.css
@theme {
    --breakpoint-laptop: 1024; /* [!code ++] */
}

@custom-variant @watch { /* [!code ++] */
    @media (min-width: 38mm) and (max-width: 42mm) { /* [!code ++] */
        @slot; /* [!code ++] */
    } /* [!code ++] */
} /* [!code ++] */
```
To learn more, check out [at variants](/guide/theme#at-tokens).
