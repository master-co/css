...
| Config and theme | Legacy config files, theme values, breakpoints, modes, aliases, and component definitions | Move project-owned values into `@theme`, `@settings`, `@custom-variant`, `@layer components`, and `@utilities`. |
...
```html
<section class="grid gap-lg r-card fg-body grid-cols:3@dashboard p-card">
  ...
</section>
```
...
If the v1 project had reusable component abstractions, migrate them to `@layer components` when the name describes a product UI role.

```css name=app.css
@layer components {
  .card {
    @compose grid gap-md b:1px|solid|var(--color-line-base) r-card bg-white shadow-sm p-card;
  }

  .btn-primary {
    @compose inline-flex items-center justify-center gap-xs h:2.5rem px-md r-md bg-primary fg-white;
...
```html
<button class="inline-flex items-center gap-xs h:2.5rem px-md r-md bg-blue-60 fg-white bg-blue-70:hover opacity:.5:disabled">
  Save
</button>
```
...
function Progress({ value }: { value: number }) {
  return (
    <div className="overflow:hidden h:0.5rem r:100% bg-gray-10">
      <div
...
- [Style declarations](/reference/rules/declarations) for current class syntax.
- [Theme Tokens](/guide/theme) for `@theme` tokens, [Variables and Modes](/guide/variables-and-modes) for `@settings` and modes, and [Conditional Queries](/reference/rules/conditions) for custom variants.
- [CSS directives](/reference#directives) for `@layer components`, `@utilities`, `@compose`, `@reference`, and source exceptions.
