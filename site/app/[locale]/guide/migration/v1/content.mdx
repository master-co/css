...
import plan from 'virtual:master-css-plan' /* [!code ++] */
import preloaded from 'virtual:master-css-preloaded' /* [!code ++] */

init() /* [!code --] */
initCSSRuntime({ plan, preloaded }) /* [!code ++] */
```
...
```
Animation presets are built into the default preset plan, so remove separate `@master/keyframes.css` imports.
```css name=main.css
