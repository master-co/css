...
Deprecate extended `breakpoints` and `queries` ​​in favor of `config.variables` and `config.atRuleAliases`.
```js name=master.css.js
import { Style } from '@master/css' /* [!code --] */
...
```
To learn more, check out the [at-rules](/guide/at) documentation.
