...
import config from 'virtual:master-css-config' /* [!code ++] */
import preloaded from 'virtual:master-css-preloaded' /* [!code ++] */

init() /* [!code --] */
initCSSRuntime({ config, preloaded }) /* [!code ++] */
```
...
Deprecate extended `colors` and `values` ​​in favor of variables in the project CSS entry.
```js name=config.js
...
```
To learn more, check out [theme tokens](/guide/theme#define-theme-tokens).

### Using new custom
In v2, you can apply the [default](/guide/theme#customize-motion) out of the box:
```html
<!-- @MARK animation:ping|1s|infinite -->
<div class="animation:ping|1s|infinite"></div>
```
Animation presets are built into the current config, so remove separate `@master/keyframes.css` imports.
```css name=main.css
@import '@master/keyframes.css'; /* [!code --] */
```
To customize keyframes, check out [Customizing theme](/guide/theme#customize-motion).

### Using abstract styles
Deprecate extended `classes` ​​in favor of `@master`.
```js
import { Style } from '@master/css' /* [!code --] */
...
```
To learn more, check out [component classes](/guide/reusing-styles#components).

### Merging media queries
Deprecate extended `breakpoints` and `queries` ​​in favor of screen variables and custom at tokens in the project CSS entry.
```js name=config.js
import { Style } from '@master/css' /* [!code --] */
...
Style.extend('queries', { /* [!code --] */
   watch: '(min-width:38mm) and (max-width:42mm)' /* [!code --] */
}) /* [!code --] */
```

```css name=index.css
@theme {
    --screen-laptop: 1024; /* [!code ++] */
}

@custom-at watch @media (min-width: 38mm) and (max-width: 42mm); /* [!code ++] */
```
To learn more, check out the [at tokens](/guide/at-tokens) documentation.
...
### Using `:of()` pseudo-class
In v1, we used `<selector><combinator>{}` to style elements based on their ancestor. In v2, we introduced the `:of()` pseudo-class to achieve the same effect.
```html
<!-- @MARK active -->
<div class=".active_{hidden}">hidden</div><!-- [!code --] -->
<div class=".active>{hidden}">hidden</div><!-- [!code --] -->
<div class=".active+{hidden}">hidden</div><!-- [!code --] -->
<div class=".active~{hidden}">hidden</div><!-- [!code --] -->
<div class="hidden:of(.active)">hidden</div><!-- [!code ++] -->
<div class="hidden:of(.active>)">hidden</div><!-- [!code ++] -->
<div class="hidden:of(.active+)">hidden</div><!-- [!code ++] -->
<div class="hidden:of(.active~)">hidden</div><!-- [!code ++] -->
```
This is a selector unique to Master CSS. For more information, see [`:of()`](/guide/state-selectors#of).
...
```
In v2, `$key` accesses configured variables and falls back to `var(--key)` if not found.
```css name=index.css
@theme {
    --content-key: "`";
}
