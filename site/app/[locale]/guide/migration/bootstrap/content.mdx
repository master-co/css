...
```html
<section class="fg:body p:page r:control bg:white">
    ...
</section>
```
...
<section class="w:full max-w:6xl mx:auto px:md py:2xl">
    <div class="grid grid-cols:3@md gap:lg">
        <article class="p:lg r:lg bg:white b:1px|solid|base">
            ...
        </article>
    </div>
</section>
...
```html
<div class="display:contents@md scroll-margin-top:4rem">
    ...
</div>
```
...
@components {
    btn-primary {
        @compose inline-flex align-items:center justify-content:center gap:xs px:md h:10x r:control bg:primary fg:white;
        @compose bg:blue-70:hover opacity:.5:disabled;
    }

    panel {
        @compose bg:white fg:body r:lg b:1px|solid|base p:lg shadow:sm;
    }
}
```
...
```html
<nav class="flex align-items:center gap:md">
    <div class="min-w:64x">...</div>
</nav>
```
