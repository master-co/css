...
5. Convert reusable Sass patterns to `@layer components`, `@utilities`, `@compose`, or native CSS.
...
| Placeholders | `%button`, `%card`, `%stack`, and `@extend` usage | Convert real product roles to `@layer components`; convert primitives to `@utilities`. |
...
```html
<article class="b:1px|solid|var(--color-line-base) r-card bg-white fg-body p-card">
  ...
</article>
```
...
Use `@layer components` for product roles:
...
- [Theme Tokens](/guide/theme) for tokens, [Variables and Modes](/guide/variables-and-modes) for modes, and [Conditional Queries](/reference/rules/conditions) for custom variants.
- [CSS directives](/reference#directives) for `@theme`, `@layer components`, `@utilities`, `@reference`, and `@compose`.
