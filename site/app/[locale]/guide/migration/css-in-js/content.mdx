...
4. Convert static styles to class strings or `@layer components`.
...
export function Button(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button
      className={['inline-flex items-center gap-xs min-h:2.5rem px-md r-md bg-primary fg-white', className].filter(Boolean).join(' ')}
      type="button"
      {...buttonProps}
    />
  )
...
@layer components {
  .btn {
    @compose inline-flex items-center justify-center gap-xs min-h:2.5rem px-md r-md bg-primary fg-white;
...
```tsx
const toneClasses = {
  neutral: 'bg:surface fg-body b:1px|solid|var(--color-line-base)',
  primary: 'bg-blue-60 fg-white bg-blue-70:hover',
  danger: 'bg-red-60 fg-white bg-red-70:hover'
}

const sizeClasses = {
  sm: 'h:2rem px-sm font-sm',
  md: 'h:2.5rem px-md font-sm',
  lg: 'h:3rem px-lg font-md'
}
...
export function Button({ tone = 'primary', size = 'md', className, ...props }: ButtonProps) {
  return (
    <button
      className={['inline-flex items-center r-md', sizeClasses[size], toneClasses[tone], className].filter(Boolean).join(' ')}
      type="button"
      {...props}
    />
  )
...
export function Meter({ value }: { value: number }) {
  return (
    <div className="overflow:hidden h:0.5rem r:100% bg-gray-10">
      <div
        className="h:100% w:var(--value) bg-green-60 transition:width|var(--duration-fast)|var(--easing-smooth)"
