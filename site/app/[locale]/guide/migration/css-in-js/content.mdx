...
export function Button(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
    return (
        <button
            className="inline-flex align-items:center gap:xs min-h:10x px:md r:md bg:primary fg:white"
            {...props}
        />
    )
}
...
@components {
    btn {
        @compose inline-flex align-items:center justify-content:center gap:xs min-h:10x px:md r:md bg:primary fg:white;
...
export function Button({ tone = 'primary', size = 'md', className, ...props }: ButtonProps) {
    return (
        <button
            className={`${toneClasses[tone]} ${sizeClasses[size]} inline-flex align-items:center r:md ${className ?? ''}`}
            {...props}
        />
    )
}
