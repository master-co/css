This guide upgrades both **RC before named tokens (`rc-legacy`)** and **RC with named tokens before the final semantic contract (`rc-named`)** to language v2. The first profile requires both stages below; the second already uses hyphenated tokens and begins with the final semantics stage. Check the exact installed RC versions and the project's resolved manifest before applying any mapping. RC releases did not all have identical behavior.
...
- Update native and Wasm providers together to **binding ABI 9**. Custom engine adapters must still implement the complete engine interface, including `executionState()`.
...
`matchStatus`, `cssValueStatus`, and `browserSupport`; the former `valid` or
...
Use `cssValuePolicy: 'error'` in compiler APIs or `master-css generate --strict`
...
read result version 2, full diagnostic and dependency arrays, context metadata,
