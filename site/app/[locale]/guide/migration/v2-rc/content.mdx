...
- Update native and Wasm providers together to **binding ABI 10**. Custom engine adapters must still implement the complete engine interface, including `executionState()`.
...
Upgrade bindings and tools together: ABI **10**, source result **2**, validator **3**, language result **4**, diagnostics report **3**, MCP result **3**. Manifest and hydration envelopes remain v1 with `languageVersion: 2`; regenerate hydration priorities rather than filling missing fields with RC defaults.
