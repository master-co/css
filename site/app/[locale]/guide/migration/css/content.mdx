...
.button {
  @compose inline-flex items-center justify-center gap-xs h:2.5rem px-md r-md bg-blue-60 fg-white;
}
...
export function Button(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return <button className={[styles.button, className].filter(Boolean).join(' ')} type="button" {...buttonProps} />
}
```
...
```tsx
<div className="w:var(--progress) bg-blue-60" style={{ '--progress': `${value}%` } as React.CSSProperties} />
```
...
- [CSS directives](/reference#directives) for `@reference`, `@compose`, `@source`, `@safelist`, and `@blocklist`.
