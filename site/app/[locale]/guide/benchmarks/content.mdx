...
The remaining public gap is lifecycle evidence. The browser lifecycle suite now has an internal all-scenario baseline and curated snapshot, but this page should not yet claim that runtime rendering produces better real-user interaction performance overall. Final lifecycle conclusions still need repeated review-grade runs and a stable five-minute long-session pass before becoming public results.
...
A curated internal snapshot is generated from the all-scenario `browser-lifecycle` baseline. It remains internal because the separate five-minute long-session readiness pass is still too slow for publication review, so the current data should be used for benchmark validation rather than public performance claims.
...
| 15 | Internal: implement browser lifecycle and INP-style interaction benchmarks for initial load, large DOM, append, repeated toggles, theme switching, route navigation, and long-session behavior; public conclusions wait for repeated review and a stable five-minute long-session pass. |
