...
The remaining public gap is lifecycle evidence. The browser lifecycle suite now has an internal all-scenario baseline and curated snapshot, but this page should not yet claim that runtime rendering produces better real-user interaction performance overall. A segmented five-minute long-session readiness pass still has an open blocker: the `master-static` mode exceeded the review window before producing a labeled focused report.
...
A curated internal snapshot is generated from the all-scenario `browser-lifecycle` baseline. It remains internal because segmented five-minute long-session evidence has not passed publication review: `master-static` exceeded the readiness window before producing a labeled focused report, so the current data should be used for benchmark validation rather than public performance claims.
