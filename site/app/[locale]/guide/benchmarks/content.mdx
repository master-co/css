...
The remaining public gap is lifecycle evidence. The browser lifecycle suite now has an internal all-scenario baseline and curated snapshot, but this page should not yet claim that runtime rendering produces better real-user interaction performance overall. A segmented five-minute long-session readiness pass still has an open blocker: `master-static` completed warmup, then failed during measured-round Chromium trace serialization before producing a labeled focused report.
...
A curated internal snapshot is generated from the all-scenario `browser-lifecycle` baseline. It remains internal because segmented five-minute long-session evidence has not passed publication review: `master-static` completed warmup, then failed during measured-round Chromium trace serialization before producing a labeled focused report, so the current data should be used for benchmark validation rather than public performance claims.
...
| 15 | Internal: implement browser lifecycle and INP-style interaction benchmarks for initial load, large DOM, append, repeated toggles, theme switching, route navigation, and long-session behavior; public conclusions wait for repeated review and a passing segmented five-minute long-session run. |
