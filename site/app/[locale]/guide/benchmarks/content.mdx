...
| Internal | Master CSS delivery modes | What are the initial payload, browser load, runtime bootstrap, and progressive adoption costs of static, runtime, and progressive delivery? |
| Internal | Interaction cost | How expensive are class toggles, rule generation, DOM insertion/removal, theme switches, and resize-triggered recalculation? |
| Internal | Runtime mutation diagnostics | Which MutationObserver and runtime add/remove substeps explain cleanup-cycle cost in runtime and progressive modes? |
...
The internal delivery-mode suite currently measures initial payload, local Chromium page load, runtime bootstrap, manifest loading, and progressive style adoption across Master CSS static, runtime, progressive, and Tailwind CSS static variants. It does not publish public result tables yet, and it does not measure post-load mutation; those interaction costs stay in the next suite.
...
## Interaction cost
The internal interaction-cost suite now measures changes users can feel:
...
For Master CSS runtime and progressive modes, the suite reports both DOM work and runtime stylesheet work. For static Tailwind CSS output, the comparable measurement is the browser's style/layout/paint cost after the DOM or class change. These results stay internal until the fixture coverage and repeated-sample variance are reviewed.

The internal runtime mutation diagnostics suite narrows the cleanup-cycle hotspot to `dynamic` and `stress-dom` fixtures in Master runtime and progressive modes. It records MutationObserver delivery counts, runtime `add`/`remove` call shape, temporary class cleanup, and Chromium trace timings so optimization work can target the confirmed source of cost.
...
| 6 | Internal: implement Master CSS static, runtime, and progressive delivery benchmarks with local Chromium payload/load/adoption measurements. |
| 7 | Internal: implement interaction cost suites with Playwright/browser tracing, correctness checks, and stable fixture pages. |
