...
The remaining public gap is lifecycle evidence. The browser lifecycle suite now has an internal all-scenario baseline and curated snapshot, but this page should not yet claim that runtime rendering produces better real-user interaction performance overall. Segmented `master-static` five-minute long-session evidence has passed internally after trace artifact compaction; the remaining modes still need segmented review before any public lifecycle conclusion.
...
A curated internal snapshot is generated from the all-scenario `browser-lifecycle` baseline. It remains internal because segmented five-minute long-session evidence has only passed for `master-static` after trace artifact compaction; the remaining modes still need segmented review, so the current data should be used for benchmark validation rather than public performance claims.
...
| 15 | Internal: implement browser lifecycle and INP-style interaction benchmarks for initial load, large DOM, append, repeated toggles, theme switching, route navigation, and long-session behavior; public conclusions wait for repeated review and passing segmented five-minute long-session runs across modes. |
