...
Native fallback is intentionally lower priority than Master CSS semantics. Aliases, token resolution, unit conversion, transform helpers, and shorthand utilities continue to use the Master CSS matcher before a native declaration is considered.

```html
<!-- @MARK d:block --><!-- @MARK margin:16 --><!-- @MARK bg:red-60 -->
<div class="d:block margin:16 bg:red-60">…</div>
```

<details>
    <summary>Generated CSS</summary>
    <Class2CSS>
        {['d:block', 'margin:16', 'bg:red-60']}
    </Class2CSS>
</details>

This preserves the expected syntax model:

- `d:block` uses the `d` alias and emits `display:block`.
...
- Keep Master CSS aliases, tokens, unit conversion, and shorthands ahead of native declaration fallback.
