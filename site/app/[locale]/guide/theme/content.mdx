...
## Named conditions and selector aliases
Named conditions belong in the theme when they become shared product vocabulary. Built-in selector aliases stay available to markup and CSS-authored styles without adding them to the theme.
...
Inside CSS-defined style rules, write nested selectors directly. `@custom-variant` and `@variant` only name and apply condition variants.
...
Selector aliases use `:` or `::` suffixes in class strings and nested CSS selectors. They are built into the engine, not defined with `@custom-variant`.
