...
## Overview [sr-only]
Your theme is the shared design vocabulary for a Master CSS project. It defines the values your markup reaches for every day: colors, spacing, type, radius, shadows, motion, breakpoints, container sizes, mode-specific values, and named conditions.

In Master CSS, that vocabulary usually lives in the project CSS entry: `@theme` for tokens and modes, `@settings` for project settings, and `@custom-variant` for reusable condition variants. The file still feels like CSS, while markup keeps using short semantic classes and built-in selector aliases.
...
```html
<section class="p:lg r:lg bg:white text:neutral shadow:sm grid-cols:2@md">
  <button class="px:md py:xs r:md bg:blue fg:white bg:blue/.85:hover">Save</button>
</section>
```
...
The important part is the graph, not a special file name. Tokens, modes, motion, and named conditions can live wherever the project CSS entry already reaches them.
...
```html
<pre class="font-feature-settings:tabular transition-delay:fast animation-duration:fast content:empty leading:body">...</pre>
```
...
<article class="p:md r:lg font:sans bg:white text:neutral shadow:sm">
...
```css name=index.css
@theme {
  --stripe: 0 / 7.5px 7.5px linear-gradient(135deg, $color-line-muted 4.5%, $color-surface-raised 0) $color-surface-raised;
}
```
...
Use `mode-trigger: class` active app class manually. mode modes on Put switches the when your `<html>`; Master CSS keeps native `color-scheme` in sync for the built-in `light` and `dark` modes.
...
Named conditions and selectors belong in the theme when they become shared product vocabulary. Built-in selector aliases stay available to markup and CSS-authored styles without adding them to the theme.
...
The token is written bare in the definition and in CSS `@variant` blocks. In markup, use the token name as an `@` suffix:
...
<div class="bg:white/.8@supports-backdrop animation:fade-in|fast@motion-safe">
  ...
</div>
```
{/* eslint-enable @master/css/no-invalid-classes */}

### Condition variants in CSS rules
...
`@variant` accepts custom condition variants, breakpoint tokens such as `md`, mode names such as `dark`, and raw Master CSS condition syntax such as `<sm`.

### Built-in selector aliases
Selector aliases name common selectors in class suffixes. The default set is built into the engine, so they are available without `@custom-variant`.

```html
<!-- @MARK :first --><!-- @MARK :odd --><!-- @MARK ::scrollbar-thumb -->
<ul class="mt:0:first bg:neutral-5:odd">
  <li class="bg:blue-60::scrollbar-thumb">...</li>
</ul>
```

Use native selector syntax directly when a selector is not a built-in alias:

```html
<!-- @MARK :is(h1,h2,h3,h4,h5,h6) --><!-- @MARK :has(img) -->
<article class="font:semibold_:is(h1,h2,h3,h4,h5,h6) shadow:md:has(img)">
  ...
</article>
```

### Custom selector variants
Use `@custom-variant` with a `:` or `::` token when a selector becomes shared vocabulary.
...
The same selector variant works in class suffixes:

```html
<!-- @MARK :interactive -->
<button class="fg:blue:interactive">...</button>
```

### Selectors in CSS rules
Inside CSS-defined style rules, write nested selectors directly for one-off selectors. Use `@variant` when the selector is a reusable custom variant.
...
### Naming rules
Breakpoint variables automatically create condition variants such as `@md`, and modes automatically create condition variants such as `@dark`. Custom condition token names cannot start with `@`, `:`, or `::`, and cannot conflict with breakpoint or mode names.

Selector variants use `:` or `::`, while condition variants use `@`. `@custom-variant` can define either shape.
...
```html
<div id="app">
  <button class="p:md font:md">Save</button>
</div>
```
...
@components {
  btn {
    @compose inline-flex items-center justify-center gap:xs px:md py:xs r:md font:medium font:sm;
...
Use [Global styles](/guide/global-styles#shared-vocabulary) for abstraction strategy, [component classes](/guide/global-styles#component-classes) for semantic UI classes, and [custom utilities](/guide/global-styles#custom-utilities) for low-level project primitives.
