...
<section class="bg:surface fg:strong p:card r:card shadow:card grid-cols:2@md">
    <button class="bg:accent fg:on-accent px:md py:xs r:md bg:accent-hover:interactive">Save</button>
</section>
```
...
* Some non-static, non-function utility names are also namespaces when the property name is already the clearest token namespace. This is why `tab-size-code` can be used as `tab-size:4`.
...
```html
<pre class="animation-duration:fast transition-delay:short video font-feature-settings:tabular leading:body tab-size:4">...</pre>
```
...
<section class="bg:surface fg:strong">
    <button class="bg:accent fg:on-accent">Save</button>
</section>
```
...
The default entry imports focused public files. Use `@master/css/theme.css` for theme tokens and managed keyframes, `@master/css/variants.css` for preset variants, and `@master/css/utilities.css` for preset static utilities when you need a smaller entry.
