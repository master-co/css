...
Inside CSS-defined style rules, write nested selectors directly instead of putting selector suffixes inside `@compose` or `@variant`.
...
Selector suffixes use `:` or `::`, while conditional variants use `@`. `@custom-variant` only defines condition variants.
