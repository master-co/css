...
After your project defines UI tokens, the same short token can resolve differently by context. `accent` is a color in `bg:accent`, `card` is spacing in `p:card`, radius in `r:card`, and shadow in `shadow:card`. If the shadow token is mode-specific, the same `shadow:card` class follows the active light or dark mode.
