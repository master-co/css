...
export function Button({ destructive }: { destructive?: boolean }) {
  return (
    <button className={`btn px:md r:md ${destructive ? toneClasses.destructive : toneClasses.neutral}`}>
      Delete
    </button>
  )
}
...
function Badge({ tone }: { tone: 'red' | 'green' | 'gray' }) {
  return <span className={`px:xs r:full fg:white bg:${tone}`}>Status</span>
}
```
...
function Badge({ tone }: { tone: keyof typeof toneClasses }) {
  return <span className={`px:xs r:full ${toneClasses[tone]}`}>Status</span>
}
```
...
function Headline({ size }: { size: string }) {
  return (
    <h1
      className="font-size:$headline-size leading:1.1"
