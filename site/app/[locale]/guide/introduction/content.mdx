...
It can attach [state selectors](/guide/state-selectors) and [selector tokens](/guide/theme#selector-tokens) to the same declaration:
...
The `@theme` and `@master` blocks, top-level custom directives, and managed layers are configuration, not regular stylesheet output. Master CSS compiles them into config, then generates only the classes used in markup, discovered by source scanners, or composed by another CSS-defined style. Native rules such as `.notice` lower to CSS for the stylesheet that contains them.
