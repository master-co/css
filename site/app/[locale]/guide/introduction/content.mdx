...
For one-off interface work, you can stay in markup. When a value, selector, or repeated pattern becomes part of your product vocabulary, move it into the [project CSS entry](/guide/configuration) as a token, selector token, at token, utility, or [component](/guide/components).
...
## The CSS framework
Master CSS is also a standalone CSS framework. It includes the core rule engine, browser runtime rendering, server rendering, static extraction, framework integrations, [code linting](/guide/code-linting), [language service](/guide/language-service), and CSS-first [configuration](/guide/configuration).
...
The project CSS entry is the primary configuration source. Tokens are native custom properties, selector shortcuts use `@custom-selector`, conditional shortcuts use `@custom-at`, low-level custom utilities live in `@layer general`, and semantic component classes live directly inside `@master`.
...
The `@master` block is configuration, not regular stylesheet output. Master CSS compiles it into config, then generates only the classes used in markup or discovered by extractors.
...
Master CSS provides rich [configuration](/guide/configuration) for scalable design systems. Use [design tokens](/guide/design-tokens) for color, spacing, typography, radius, elevation, and motion; use [screen sizes](/guide/screen-sizes), [sizing](/guide/sizing), and the [layout system](/guide/layout-system) to keep page structure aligned with those foundations.
...
Assuming the card has become reusable, define it as a main style using [components](/guide/components) in the project CSS entry:
...
When the styling decision is local, markup is the clearest place for it. When the decision becomes shared vocabulary, promote it into tokens, utilities, or components.
...
5. [Design tokens](/guide/design-tokens), [configuration](/guide/configuration), and [components](/guide/components) when your project needs shared vocabulary.
