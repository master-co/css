...
- Start with the declaration you want, such as `fg:accent`, `font:3xl`, or `text-center`.
...
```html
<!-- @MARK fg:accent --><!-- @MARK :hover --><!-- @MARK @sm -->
<h1 class="fg:accent fg:danger:hover font:3xl font:5xl@sm font:heavy m:2xl text-center">
    Hello World
</h1>
```

<Demo $py={0}>
    <h1 className="fg:accent fg:danger:hover font:3xl font:5xl@sm font:heavy m:2xl text-center">Hello World</h1>
</Demo>

<details>
    <summary>Generated CSS</summary>
    <Class2CSS>
        {'fg:accent fg:danger:hover font:3xl font:5xl@sm font:heavy m:2xl text-center'}
    </Class2CSS>
</details>
...
<div class="bg:accent-hover:hover">
...
<div class="bg:accent_:where(p)">

<!-- Use :first instead of :nth-child(...) -->
<div class="bg:accent:first">
```
...
### CSS-first plan authoring
The project CSS entry is the primary plan source. Theme tokens live in `@theme`, reusable selector and conditional variants use `@custom-variant`, low-level custom utilities live in `@utilities`, semantic component classes live in `@components`, and native CSS rules can use `@compose` when they should lower to ordinary stylesheet output.
...
The `@theme`, `@settings`, top-level custom directives, and managed definition directives are plan inputs, not regular stylesheet output. Master CSS compiles them into a `MasterCSSPlan`, then generates only the classes used in markup, discovered by source scanners, or composed by another CSS-defined style. Native rules such as `.notice` lower to CSS for the stylesheet that contains them.
