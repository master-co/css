...
(x) <span className="text:danger">Invalid value for `font` property.</span>
```html
<!-- @MARK font: -->
<div class="font:">…</div>
```
...
@components {
    btn {
        ...
        @variant :interactive {
            @compose outline:2px|solid|blue-60 outline-offset:0.125rem;
        }
    }
}
...
The [benchmarks](/guide/benchmarks) compare page-level CSS weight and delivery tradeoffs with reproducible inputs. On this documentation site, Master CSS keeps page CSS around **~6 kB per page** with brotli compression because it emits only the rules required by that page.
...
Master CSS provides a rich [theme layer](/guide/theme) for scalable design systems. Use theme tokens for color, spacing, typography, radius, elevation, and motion; use [breakpoints](/guide/breakpoints), [containers](/guide/containers), [sizing](/guide/sizing), and the [layout system](/guide/layout-system) to keep page structure aligned with those foundations.
...
@components {
    card {
        @compose flex transition:transform|fast|smooth overflow:hidden surface:raised b:1px|solid|base r:xl;

        &:hover {
            @compose transform:translateY(-5px);
        }
    }
...
5. [Theme](/guide/theme) and [component classes](/guide/reusing-styles#component-classes) when your project needs shared vocabulary.
