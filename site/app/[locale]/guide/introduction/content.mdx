...
```html
<h1 class="m:0 font:4xl font:heavy fg:blue-60">
    Hello World
</h1>
```
...
- `m:0` sets margin to zero.
- `font:4xl` uses a font-size token.
- `font:heavy` sets font weight.
- `fg:blue-60` sets the text color.
...
When a value or pattern becomes shared across the product, promote it later. Repeated values usually belong in [theme tokens](/guide/theme), and repeated product roles can become [component classes](/guide/reusing-styles#component-classes).
...
```html
<!-- @MARK :hover --><!-- @MARK @md -->
<button class="px:md py:xs r:md bg:blue-60 bg:blue-70:hover grid-cols:2@md fg:white">
    Save
</button>
```
...
```html
<article class="p:card bg:brand fg:white">
    ...
</article>
```
