...
`base` normalizes the platform, `theme` emits variables, `preset` holds broad defaults, `main` holds project vocabulary from `@master` component definitions, and `general` holds utility classes. Because `general` comes last, local utility decisions can override reusable project styles without reaching for `!important`.
