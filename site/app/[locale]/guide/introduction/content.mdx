...
The project CSS entry is the primary configuration source. Tokens are native custom properties, selector shortcuts use `@custom-selector`, conditional shortcuts use `@custom-at`, low-level custom utilities live in `@layer utilities`, and semantic component classes live directly inside `@master`.
...
The component definitions stay on-demand. They are emitted in the `components` layer only when `card`, `card-image`, and the other component classes appear in markup or are discovered by extraction.
