...
The project CSS entry is the primary configuration source. Theme tokens live in `@theme`, reusable selector and conditional variants use `@custom-variant`, low-level custom utilities live in `@utilities`, semantic component classes live in `@components`, and native CSS rules can use `@compose` when they should lower to ordinary stylesheet output.
