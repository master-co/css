...
<!-- Use a configured breakpoint token -->
...
<Image className="aspect:2/1 r:lg max-w:container-3xl w:full" src={require('~/site/public/syntax-error-checks.webp').default} alt="Syntax error checks" unoptimized />
...
Master CSS provides a rich [theme layer](/guide/theme) for scalable design systems. Use [design tokens](/guide/design-tokens) for color, spacing, typography, radius, elevation, and motion; use [breakpoints and containers](/guide/breakpoints-and-containers), [sizing](/guide/sizing), and the [layout system](/guide/layout-system) to keep page structure aligned with those foundations.
