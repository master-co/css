...
Use [Visual Studio Code setup](/guide/installation/vscode) to install the official extension. Use it with the VS Code ESLint extension when you need team rules, diagnostics for committed markup, and automated class sorting.
...
Optional basic class syntax diagnostics can be enabled with `masterCSS.diagnoseClassSyntax`, but class-string policy and sort-on-save diagnostics in markup and JavaScript are owned by [code linting](/guide/code-linting). The language server does not duplicate `sort-classes`, `prefer-canonical-classes`, `no-conflicting-classes`, or `no-unapproved-raw-values` warnings by default.
...
| `masterCSS.formatDirectives` | `true` | Enables Master CSS directive formatting in CSS-family documents and style blocks. |
| `masterCSS.diagnoseClassSyntax` | `false` | Enables basic class syntax diagnostics without enabling ESLint-owned class policy warnings. |
