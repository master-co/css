...
```html
<span class="bg:blue-60/.1 fg:#4f46e5">Primary</span>
```
...
Class-string syntax, policy, and sort-on-save diagnostics in markup and JavaScript are owned by [code linting](/guide/code-linting). The language server does not duplicate `sort-classes`, `prefer-canonical-classes`, `no-conflicting-classes`, `no-invalid-classes`, or `no-unapproved-raw-values` warnings.
