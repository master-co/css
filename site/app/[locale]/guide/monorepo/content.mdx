## Foreword [sr-only]
The [Language Service](/guide/language-service) and [code linting](/guide/code-linting) both support the monorepo.

By default, language server forms independent workspaces from project CSS entries that contain `@master entry;` or `@import '@master/css'`, or from `package.json` files that declare Master CSS package dependencies, independent of the package manager's workspace. The same goes for ESLint, which forms a new working directory based on the location of its configuration file.
...
@components {
    app-shell {
        @compose min-h:100vh bg:surface-base;
    }
}
```
