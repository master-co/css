...
## Exact pixel values
Use `0` for resets, the `px` alias for 1px hairlines, and explicit `px` units when a value needs exact pixel geometry. Bare numbers are emitted as written, so `p:1` becomes `padding: 1` and is not a valid CSS length.

```html
<!-- @MARK m:0 -->
<div class="m:0">0px, margin: 0rem</div>
<!-- @MARK p:px -->
<div class="p:px">1px, padding: 1px</div>
<!-- @MARK w:2px -->
...
Numeric keys are easy to confuse with low-level CSS values. Prefer semantic token keys like `sm` or `card`, then use `p:px` for a 1px value or `p:1x` for the default 4px base unit.
...
* Use `0`, `px`, or explicit pixel units for resets and exact pixel adjustments.
