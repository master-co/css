...
```html
<!-- @MARK :hover@md -->
<button class="bg:blue:hover@md">Save</button>
```

For readability, Master CSS can use [selector tokens](/guide/theme#selector-tokens) to name selectors that appear often.
```html
...
```
You can also create project-specific selector tokens for longer selectors.
```html
...
<Demo>
    <button className="bg:indigo bg:pink:hover bg:purple:active px:md h:10x font:sm font:medium fg:white r:sm inline-flex align-items:center">Submit</button>
</Demo>
```html
<!-- @MARK :hover --><!-- @MARK :active -->
    ...
    <DemoPanel $p={0} className="w:full max-w:90x">
        <ul className="list-style:none py:lg px:0 m:0">
            {furnitures.map((furniture, index) =>
                <li className="bb:0:last bg:slate-5:even@light bg:gray-90:even@dark bb:1|lightest px:lg flex align-items:center my:0 gap:sm" key={index}>
                    ...
                    <div className="flex flex-col flex:1 py:xs">
                        ...
                        <div className="text:xs m:0 fg:neutral">{furniture.intro}</div>
                    </div>
                </li>
            )}
        </ul>
...
<Demo>
    <input type="email" className="b:red:invalid b:green:valid b:1|lightest outline:0 h:12x font:sm px:md bg:panel r:md max-w:75x" defaultValue="aron@master." />
</Demo>
```html
...
<Demo>
    <input type="checkbox" className="appearance:none bg:indigo:checked b:1|light r:sm size:5x" defaultChecked />
</Demo>
```html
<!-- @MARK :checked -->
...
<Demo $py={0} $px={0}>
    <div className="size:6::scrollbar bg:gray/.2::scrollbar-thumb overflow-x:auto p:xl">
