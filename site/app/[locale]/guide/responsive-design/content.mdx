...
```
Master CSS allows for the conditional application of styles at different viewport and container sizes. These queries use shared [screen sizes](/guide/screen-sizes), which you can use as viewport breakpoints, screen variables, and container query sizes.
...
The syntax is `@<breakpoint>`, where `<breakpoint>` can be any configured [screen size token](/guide/screen-sizes), such as `xs`, `sm`, `md`, `lg`, etc.
```html
<!-- @MARK @xs @sm @md -->
<div class="grid-cols:2 grid-cols:3@xs grid-cols:4@sm grid-cols:5@md …">…</div>
```
...
Container queries and media queries share the same [screen sizes](/guide/screen-sizes).
