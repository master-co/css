...
Use [comparison](/reference/rules/conditions#comparison-operators) and [logical](/reference/rules/conditions#logical-operators) operators to create more complex viewport range queries.
```html
<!-- @MARK @sm&<=md -->
  ...
  <Demo>
    <div className="overflow:hidden container w:100% b:1px|solid|var(--color-line-base) r-lg surface-raised">
      <div className="flex flex-col@container(<=2xs)">
        <Image src="/images/blur.png" className="w-3xs aspect-ratio:3/2 object-cover {w:100%;aspect-ratio:16/9}@container(<=2xs)" width={300} height={200} alt="container left" />
...
Container queries use the [container scale](/guide/containers), which can use different values from viewport breakpoints.

### Querying container ranges
Use [comparison](/reference/rules/conditions#comparison-operators) and [logical](/reference/rules/conditions#logical-operators) operators to create more complex container range queries.
```html
<!-- @MARK @sm&<=md -->
