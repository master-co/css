...
### `@starting-style`
The `@starting-style` defines the starting values for properties to enable smooth transitions when an element first appears.

(click) Try opening the popover target to see the zoom in effect.
<StartingStyleExample />
```html
<!-- @MARK scale(0)@starting-style -->
<div id="target" popover="auto" class="~transform|.3s scale(0)@starting-style">
    ...
