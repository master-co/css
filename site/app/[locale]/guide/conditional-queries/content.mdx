...
From [comparison operators](#comparison-operators) and [logical operators](#logical-operators) to advanced [condition grouping](#grouping-conditions), you’ll learn how to harness the full potential of CSS at-rules—without the boilerplate.
...
## Common at tokens
These tokens provide a simplified syntax for commonly used `@` suffix expressions. They allow you to apply styles based on media types, feature support, container queries, and layer management in a more concise and readable way.
...
To learn more about the [responsive design](/guide/responsive-design).
...
You can [create a token](/guide/at-tokens) to simpify the class name.
...
For any custom layer, it is wrapped in the utilities layer.
...
To learn more about the [container queries](/guide/responsive-design#based-on-container-sizes).
...
Any shorthand not listed as a screen, mode, or `@custom-at` token will be implicitly treated as a `@container`, falling back to the default behavior of being eligible for container queries.
...
(i) Use `@<name?><operator><number>` to specify the feature query.
```html
<!-- @MARK @sm -->
<div class="hidden@sm">
<!-- @MARK @h<sm -->
...
<!-- @MARK @h>=sm&h<=sm -->
<div class="hidden@h>=sm&h<=sm">
```
<details>
    <summary>Generated CSS</summary>
    <Class2CSS>
        {['hidden@sm','hidden@h<sm', 'hidden@h>=sm&h<=sm']}
    </Class2CSS>
</details>

(x) Feature names after or between operators are not supported yet.
```html
