...
This applies `bg:blue` only when the button matches `:hover` and the `@md` condition is true.
...
## Common at tokens
At tokens are named conditions. Some are built in, while screen tokens and mode tokens come from your project configuration.
...
### `sm`, `md`, `lg`, and more
Use `@sm`, `@md`, `@lg`, and other screen size tokens to apply styles based on viewport width. A plain breakpoint token means "at or above this width."
```html
<!-- @MARK @sm -->
<div class="hidden@sm">
```
...
See [responsive design](/guide/responsive-design) and [screen sizes](/guide/screen-sizes) for the full breakpoint model.

### `light`, `dark`, and custom modes
Mode names also become at tokens. `@dark` and `@light` are available by default, and custom modes are added when you define them in the project CSS entry.
...
### `base`, `preset`
Use `@base` and `@preset` to place generated rules in those cascade layers.
...
```html
<!-- @MARK scale(0)@start -->
<div id="target" popover="auto" class="transition:transform|slow|smooth scale(0)@start">
    ...
...
<details>
    <summary>Generated CSS</summary>
    <Class2CSS>
        {['transition:transform|slow|smooth', 'scale(0)@starting-style']}
    </Class2CSS>
</details>
...
When a suffix name is not a screen token, mode token, or custom at token, Master CSS treats it as a named container query.

---

## Comparison operators
Comparison operators turn screen and container tokens into ranges.
...
Use them when a style should apply below, above, or between configured sizes.
