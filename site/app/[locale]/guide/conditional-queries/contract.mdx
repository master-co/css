...
hidden@!screen
opacity:0@start
backdrop-filter:blur(5px)@supports(backdrop-filter(0px))
```
...
<details>
  ...
  <Class2CSS>{['hidden@md', 'hidden@print', 'hidden@default', 'hidden@container(xs)', 'hidden@sidebar(xs)', 'hidden@md&<lg', 'hidden@!screen', 'opacity:0@start', 'backdrop-filter:blur(5px)@supports(backdrop-filter(0px))']}</Class2CSS>
</details>
...
Condition variants are named conditions. Some are built in, while breakpoint tokens and mode tokens come from your project manifest.
...
The generated CSS depends on [`mode-trigger`](/guide/variables-and-modes#mode-triggers). A mode can become a media query, a class wrapper, or a shadow host wrapper.
...
### `only` — Exact match \{#only--exact-match\}
...
Class names cannot contain whitespace, so condition arguments that would normally contain spaces are written with parentheses.
