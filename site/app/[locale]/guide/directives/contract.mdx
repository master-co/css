...
Static utilities accept native declarations, `@compose`, nested selectors, native conditional at-rules, and `@variant`. Enum, token and dynamic patterns accept declarations and nested rules; `@compose` inside a pattern body is unsupported and reports an error. The existing pattern grammar is unchanged.
...
Enum patterns require exactly one `<...>` segment with at least two pipe-separated values. Use `token=value` when the class suffix should emit a different CSS value:
...
Use `prefix-<~namespace>` or `prefix-<=namespace>` for named tokens. A source list may contain ordered namespace references, but cannot mix token sources with raw kinds, enum values, or `*`.
...
Colon patterns use `key:<source-list>` and match `key:value`. They accept one raw kind (`number`, `color`, or `image`), literal enum alternatives, or `*` according to the combinations below. They never resolve theme token names. Use `var(--name)` for explicit references, including inside functions and multi-value declarations; `$name` is removed.
...
`size:20px` emits both dimensions. `accent:#fff` accepts a raw color; `<color>` does not imply `~color`. Token patterns must be declared separately, such as `accent-<~color>`.
...
### Literal alternatives and raw values \{#literal-and-raw-values\}
...
`*` is for raw-any full-value wrappers, such as paired vendor declarations or shorthand values that should accept `var(...)`. It may be used alone or with one raw kind, as in `text-decoration:<*>`, `accent:<color|*>`, or `size:<number|*>`. It cannot be mixed with enum values or token namespaces. No raw pattern implicitly resolves tokens.
...
`--value()` accepts no arguments and is only valid inside managed enum, named-token, or dynamic colon pattern declarations. It represents a complete CSS value placeholder, so it may be reused in value slots such as `span --value()/span --value()` or inside CSS functions such as `repeat(--value(), minmax(0, 1fr))`. It cannot be joined into identifiers or function names, so forms such as `--value()-box` or `space---value()` should use enum mapping instead.
