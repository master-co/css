...
Other directive families such as `@settings`, `@theme`, `@mode`, `@custom-variant`, `@defaults`, `@components`, `@utilities`, `@source`, `@reference`, `@safelist`, `@blocklist`, `@compose`, `@variant`, `@prune native`, and `@preserve native` are not entry markers by themselves.
...
Referenced stylesheets can define `@settings`, `@theme`, `@mode`, `@custom-variant`, `@defaults`, `@components`, and `@utilities`. Their definitions are available while compiling the current stylesheet, but their native CSS, generated CSS, and managed class definitions are not emitted just because they were referenced.
...
```css
.card {
  @compose grid gap-md p-lg r-xl;
  @compose surface-raised b:1px|solid|var(--color-line-base);
}

@components {
  btn {
    @compose inline-flex px-md py-xs r-lg;
    align-items: center;
    justify-content: center;
