...
| --- | --- | --- |
| `root-size` | `16` | Divisor used when converting numeric lengths to `rem`. It does not change `html { font-size }`. |
| `base-unit` | `4` | Multiplier used by `x` values: with the defaults, `4x` becomes `1rem`. |
...
The block above sets `scope: #app`; all other shown values match the engine defaults. Token values and native CSS declarations remain distinct from these compiler settings.
...
.card {
  ...
  @compose surface:raised b:1px|solid|base;
}
