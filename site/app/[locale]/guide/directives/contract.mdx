...
| --- | --- | --- |
| `root-size` | `16` | Reference root size for conditions and tooling. It does not convert declaration values or change `html { font-size }`. |
| `default-mode` | `light` | Default mode name. Use `none` to avoid attaching a default mode. |
| `mode-trigger` | `media` | Mode activation strategy: `media`, `class`, or `host`. |
...
The block above sets `scope: #app`; all other shown values match the engine defaults. Token values and native CSS declarations remain distinct from these compiler settings. `base-unit` is removed; use CSS units for lengths. Native resolution `x` remains valid.
...
Theme directives register tokens, modes, managed animations and reusable conditions. Their definitions become compiler input; only resources required by the resulting stylesheet are emitted, unless marked `static`.
...
Defines mode-specific token values. Mode blocks can override regular theme tokens and add new mode names. With the default media trigger, the dark value is emitted inside the native color-scheme media query.
...
`@settings`, `@theme`, `@custom-variant`, `@defaults`, `@components`, and `@utilities` declare definitions for the whole stylesheet, so they must be written at the top level. Writing one inside a style rule or inside a native at-rule such as `@layer`, `@media`, or `@supports` is reported as `@<name> must be top-level`; it does not make the definition conditional. Importing a stylesheet with a qualifier follows the same rule: the import's `layer()`, `supports()`, or media query wraps the imported rules, while the imported stylesheet's definitions are kept beside that wrapper rather than inside it, so they stay global.
