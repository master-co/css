...
This enables fast page loads, minimal CSS output, and dynamic style injection/removal with full consistency. If the hydration manifest is missing, invalid, or no longer matches `style#master`, the runtime discards the pre-rendered runtime state and rebuilds the stylesheet from the connected DOM classes.
...
| Hydration manifest | Server rendering and progressive integrations | Runtime hydration | JSON rule IR with `version: 1`, used to map `style#master` back into runtime-managed rule objects. |
