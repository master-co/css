...
The runtime reads explicit `hydrationManifest` options first. Without an explicit option, `initCSSRuntimeAsync()` discovers hydration data from the document: it reads inline `script#master-css-hydration-manifest` for compatibility, then fetches the URL from `style#master-css[data-master-css-hydration-manifest]`.
