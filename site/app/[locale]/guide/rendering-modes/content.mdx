...
| `'static'`                | Scans source files, replaces `@import '@master/css'` in managed CSS entries, shakes native CSS, and generates CSS from the project CSS entry graph. |
...
## Static Rendering
Static rendering is a zero-runtime rendering mode where the build scans source files, finds complete Master CSS class strings, validates them against the configured engine, and writes the resulting CSS before deployment.

The source scanner does not observe the DOM and does not execute application code. Its input is source text, CSS entries, scanner options, and the compiled [theme configuration](/guide/theme). Its output is a CSS asset that the browser loads like any other stylesheet.

The CSS entry point is explicit: import `@master/css` from the stylesheet that should receive generated Master CSS. In static mode, integrations replace that import with generated CSS and treat the same stylesheet as a native CSS shaking root.

<StepSection>
    <Step>
        #### <StepNum />Load the scanning context
        The integration creates a source scanner with source directives, optional fixed classes, and the project CSS entry graph. In watch mode, config and source changes reset or update this context.
...
### Native CSS entries
Regular CSS outside Master CSS directives remains native CSS. When a stylesheet imports `@master/css`, integrations treat that stylesheet as a shake root by default and prune class selectors against detected source classes.
...
For the full source scanning model, see [Scanning latent classes](/guide/scanning-latent-classes). For native CSS pruning, see [Shaking unused classes](/guide/shaking-unused-classes).
