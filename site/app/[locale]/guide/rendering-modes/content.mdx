...
const rendered = renderer.render(indexHTML, {
  hydrationManifest: {
    type: 'external',
...
The runtime reads explicit `hydrationManifest` options first. Without an explicit option, `CSSRuntime.start()` reads inline `script#master-css-hydration-manifest` or imports the URL from `style#master-css[data-master-css-hydration-manifest]` before it returns. Call `runtime.observe()` after startup completes.

Each manifest rule stores the class name, layer, priority, selector text, generated CSS text, and any referenced variables or animations. The manifest's Rust-generated `resourceOrder` preserves the variable and animation insertion order needed to reproduce the exact resource buckets without parsing CSS text. During hydration, the runtime compares that IR with the native rules already present in `style#master-css`. When they match, it attaches those native CSS rules to runtime-managed rule objects instead of regenerating them.
