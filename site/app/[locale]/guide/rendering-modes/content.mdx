...
### Native CSS entries
Regular CSS outside `@master` remains native CSS. When a stylesheet declares `@master shake;`, integrations treat that stylesheet as a shake root and prune class selectors against detected source classes.

Those native rules can also pull matching config variables referenced by `var(--*)` and configured keyframes referenced by animation declarations. Import `virtual:master.css` only where the generated Master CSS should be inserted.
