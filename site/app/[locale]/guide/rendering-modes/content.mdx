...
On the client, the [CSS runtime](#runtime-rendering) hydrates these rules into a virtual stylesheet, initializing a lifecycle-aware system that can respond to class name changes in real-time. Progressive hydration depends on two pieces of HTML: `style#master-css` contains the pre-rendered CSS, and `script#master-css-hydration-manifest` describes the generated rules that style block contains.
<figure>
...
This enables fast page loads, minimal CSS output, and dynamic style injection/removal with full consistency. If the hydration manifest is missing, invalid, or no longer matches `style#master-css`, the runtime discards the pre-rendered runtime state and rebuilds the stylesheet from the connected DOM classes.
...
const { html } = render(indexHTML, manifest, { hydrationManifest: 'inject' })
```
The server renderer parses the HTML and generates the internal style sheet plus a hydration manifest:
```html
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <script type="application/json" id="master-css-hydration-manifest">...</script> <!-- [!code ++] -->
</head>
<body>
    <!-- @MARK font:5xl -->
