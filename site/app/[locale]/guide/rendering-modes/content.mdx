...
const rendered = renderer.renderHTML(indexHTML, {
  hydrationManifest: {
    type: 'external',
    source: '/_master-css/hydration/master-css-hydration.<hash>.json'
  }
})
const { html, hydrationManifest } = rendered

renderer.dispose()
```
The manifest-scoped server renderer reuses generated class rules across pages while producing an isolated style sheet and hydration manifest for each page. Recreate it when the manifest changes and dispose it when the build ends. Official static and pre-render integrations write `hydrationManifest` to a hashed JSON asset and attach the URL to `style#master-css`:
```html
<!DOCTYPE html>
<html lang="en">
<head>
