...
### CSS config and generation
`master.css` is the default entry for the CSS config graph. It can hold `@master` definitions directly or import CSS files that contain them. `@master` definitions are converted into config and removed from regular CSS output; they do not generate rules by themselves.

```css name=master.css
@import './src/buttons.css';
```

```css name=src/buttons.css
@master {
    .btn {
...
Static extraction still emits `.btn` only when `btn` appears in scanned source:
...
Source stylesheets choose where the generated stylesheet is imported. If they also contain `@master {}` blocks, import those stylesheets from `master.css` so the config graph can read them.
...
When a framework file needs the config object at runtime, import the CSS config through the integration query:

```ts
import config from './master.css?master-css-config'
```

Without the query, a `.css` import follows the normal stylesheet pipeline.
...
### CSS imports
In extract mode, the generated stylesheet is exposed as a virtual CSS module. JavaScript imports are supported, but the preferred location is often the framework's existing stylesheet entry.
...
The generated Master CSS contains `.native-card` and `.btn` when both classes are scanned. `.unused-card` is removed because no scanned source uses it.
