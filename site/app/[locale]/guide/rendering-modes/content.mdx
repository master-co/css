...
`master.css` is the root CSS config source. `@master` definitions are converted into config and removed from regular CSS output; they do not generate rules by themselves.
...
Source stylesheets can also define local `@master` config fragments. Integrations compile these fragments before the root `master.css`, so the root config remains the final override point.
...
### Native CSS pruning
Regular CSS outside `@master` remains native CSS. Class selectors in that native CSS are scanned and emitted on demand, so non-utility classes can live beside Master CSS config without shipping unused rules.
...
```html
<!-- @MARK native-card btn -->
<article class="native-card btn">Card</article>
```

The generated CSS keeps `.native-card` and `.btn`, while `.unused-card` is removed because no scanned source uses it.
...
| `'extract'`               | Scans source files, supports JavaScript or CSS imports of the generated virtual stylesheet, and compiles `@master` blocks from source stylesheets.                   |
...
| `'progressive'`           | Combines `runtime` and `pre-render` modes. Framework adapters can also prune native CSS from source stylesheets against the rendered classes.                       |
