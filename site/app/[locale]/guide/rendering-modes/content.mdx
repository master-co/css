...
<StepSection>
    ...
    <Step>
        ...
        With this project plan, `btn` is valid because the project defines it, and `text:center` is valid because it is a built-in utility. Plan-defined classes are still on-demand: defining `btn` does not emit anything until `btn` appears in scanned source or in the scanner's fixed class list.
    </Step>
    <Step>
...
Those native rules can also pull matching theme variables referenced by `var(--*)` and managed `@animations` keyframes referenced by animation declarations. Add `@preserve native;` when native CSS must be preserved.
