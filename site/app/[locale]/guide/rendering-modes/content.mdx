...
Master CSS integrations choose how generated rules reach the browser. The same `master.css` config graph can be used by runtime rendering, static extraction, server rendering, and progressive hydration.
...
| `'extract'`               | Scans source files, replaces `@import '@master/css'` in managed CSS entries, shakes native CSS, and generates CSS from the config graph rooted at `master.css`. |
...
Every mode uses the same cascade layer model. The default `@master/css/base.css` stylesheet declares `@layer base, theme, preset, main, general;`, while generated rules are emitted into layer blocks without redeclaring the global order.
...
When runtime code needs project tokens, modes, or component definitions, load the compiled config object from the integration. `virtual:master-css-config` resolves the default `master.css`; `./master.css?master-css-config` compiles a specific CSS config file.
...
<StepSection>
    <Step>
        ...
        The integration creates an extractor with source directives, optional fixed classes, and the config graph rooted at `master.css`. In watch mode, config and source changes reset or update this context.
