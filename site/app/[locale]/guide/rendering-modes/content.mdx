...
When runtime code needs project tokens, modes, or component definitions, load the compiled runtime inputs from the integration. `virtual:master-css-plan.json` resolves the canonical project-level plan discovered from the CSS entry, and `virtual:master-css-preloaded` records variables and keyframes that the entry already emitted.
...
```tsx
const className = error ? 'bg:danger' : 'bg:success'
```
...
| `virtual:master-css-plan.json` | Official build integrations | Application runtime code and framework providers | ESM module for the canonical project-level plan discovered from the CSS entry graph. |
