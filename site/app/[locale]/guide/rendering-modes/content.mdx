...
Master CSS uses integrations like Vite to further break down the rendering modes, allowing you to start the project with zero configuration.
...
| `'extract'`               | Scans source files, supports JavaScript or CSS imports of the generated virtual stylesheet, and generates CSS from the config graph rooted at `master.css`.          |
...
| `'progressive'`           | Combines `runtime` and `pre-render` modes for first-render CSS plus client-side updates.                                                                            |
...
<StepSection>
    <Step>
        ...
        The integration creates an extractor with source globs, exclude globs, optional fixed classes, and the config graph rooted at `master.css`. In watch mode, config and source changes reset or update this context.
...
In extract mode, the generated stylesheet is exposed as a virtual CSS module. Import it from the stylesheet your framework already bundles, or from JavaScript when that is the integration's preferred entry.
...
Webpack-based integrations also support the shorter CSS import:

```css name=src/style.css
@import 'master.css';
```

Use one import path for the generated stylesheet. If both JavaScript and CSS import the virtual CSS module in Webpack, the integration keeps the CSS output on the stylesheet import path.

### Native CSS entries
Regular CSS outside `@master` remains native CSS. When a stylesheet imports the generated virtual CSS module, integrations treat that stylesheet as an extractable CSS entry and prune class selectors against detected source classes.

Those native rules can also pull matching config variables referenced by `var(--*)` and configured keyframes referenced by animation declarations.
...
Static extraction can only generate CSS for class strings that are visible in scanned source. It cannot evaluate conditions, run helper functions, resolve remote content, or predict classes assembled by string concatenation.

(x) Don't construct truncated class names
```tsx
...
Write complete class names in source, or pass runtime values through CSS variables while keeping the Master CSS class itself complete. Use [Runtime Rendering](#runtime-rendering) when class names are created only after the page is running.

Source-glob scanning is the production correctness baseline. Some integrations also scan files as they pass through the module graph for faster incremental updates, but the final build still depends on statically discoverable class strings.
