...
| Mode                      | Description                                                                                                                                           |
|---------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| `'runtime'`               | Starts `CSSRuntime` in the browser, observes connected elements, and generates rules as class names appear. Vite can inject the runtime and default plan module automatically. |
...
When runtime code needs project tokens, modes, or component definitions, load the compiled runtime inputs from the integration. `virtual:master-css-plan` resolves the canonical project-level plan discovered from the CSS entry, and `virtual:master-css-preloaded` records variables and keyframes that the entry already emitted.
...
On the client, the [CSS runtime](#runtime-rendering) hydrates these rules into a virtual stylesheet, initializing a lifecycle-aware system that can respond to class name changes in real-time. Progressive hydration depends on two pieces of HTML: `style#master` contains the pre-rendered CSS, and `script#master-css-runtime-manifest` describes the generated rules that style block contains.
<figure>
...
This enables fast page loads, minimal CSS output, and dynamic style injection/removal with full consistency. If the runtime manifest is missing, invalid, or no longer matches `style#master`, the runtime discards the pre-rendered runtime state and rebuilds the stylesheet from the connected DOM classes.
...
const { html } = render(indexHTML, plan, { runtimeManifest: 'inject' })
```
The server renderer parses the HTML and generates the internal style sheet plus a runtime manifest:
```html
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <script type="application/json" id="master-css-runtime-manifest">...</script> <!-- [!code ++] -->
</head>
<body>
    <!-- @MARK font:5xl -->
...
The runtime reads `script#master-css-runtime-manifest` when `initCSSRuntime()` starts. Each manifest rule stores the class name, layer, priority, selector text, generated CSS text, and any referenced variables or animations. During hydration, the runtime compares that IR with the native rules already present in `style#master`. When they match, it attaches those native CSS rules to runtime-managed rule objects instead of regenerating them.
...
## Static Rendering
Static rendering is a zero-runtime rendering mode where the build scans source files, finds complete Master CSS class strings, validates them with the active engine plan, and writes the resulting CSS before deployment.

The source scanner does not observe the DOM and does not execute application code. Its input is source text, CSS entries, scanner options, and the compiled project plan. Its output is a CSS asset that the browser loads like any other stylesheet.
...
<StepSection>
    <Step>
        ...
        The integration creates a source scanner with source directives, optional fixed classes, and the project CSS entry graph. In watch mode, plan and source changes reset or update this context.
    ...
    <Step>
        ...
        With this config, `btn` is valid because the project defines it, and `text:center` is valid because it is a built-in utility. Configured classes are still on-demand: defining `btn` does not emit anything until `btn` appears in scanned source or in the scanner's fixed class list.
    </Step>
    <Step>
...
Those native rules can also pull matching config variables referenced by `var(--*)` and managed `@animations` keyframes referenced by animation declarations. Add `@preserve native;` when native CSS must be preserved.
...
@components {
    btn {
        @compose "inline-flex ai:center px:md py:xs r:md";
        background: var(--color-primary);
    }
}
```

These CSS-first directives are configuration inputs. The compiler lowers tokens, settings, variants, components, utilities, animations, and native composed rules into a `MasterCSSPlan`. Runtime rendering, static rendering, server rendering, language tooling, linting, and framework integrations all consume that same IR, so class syntax and cascade behavior stay consistent even when CSS is delivered differently.
...
| `MasterCSSPlan` | `@master/css-compiler` and `@master/css-plan` | Engine, runtime, server renderer, extractor, linting, language tooling | Shared IR that describes how class names become sorted CSS rules. |
| `virtual:master-css-plan` | Official build integrations | Application runtime code and framework providers | ESM module for the canonical project-level plan discovered from the CSS entry graph. |
| `virtual:master-css-preloaded` | Static and build integrations | Runtime code | Counts variables and keyframes already emitted by the CSS entry output, so runtime rendering does not insert duplicate global CSS. |
| Runtime manifest | Server rendering and progressive integrations | Runtime hydration | JSON rule IR with `version: 1`, used to map `style#master` back into runtime-managed rule objects. |
