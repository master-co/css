...
The compiler converts these definitions into static plan utilities with `layer: 'components'`. They remain on-demand: defining `.btn` does not emit CSS until `btn` is used or scanned from source.
...
For example, set paragraph text to `16` across an article, then override one paragraph with `text:24px`.
