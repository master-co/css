...
## Scoped scanning
By default, a pruned stylesheet uses the global scanner scope. Add `@source`, `@source not`, `@source required`, `@safelist`, and `@blocklist` inside the stylesheet graph when that root needs a narrower or wider usage graph.
...
## Config references
Pruned native CSS can reference variables and keyframes from the project CSS entry graph. When a kept native rule uses `var(--*)` or a configured animation name, Master CSS includes the required config output.
