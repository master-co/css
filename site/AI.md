...
- Prefer semantic role tokens when available, such as `bg:canvas`, `bg:surface`, `text:strong`, `text:muted`, line colors like `subtle` in border context, `accent`, `danger`, `r:<token>`, `shadow:<token>`, `animation-duration:<token>`, and `animation-timing-function:<token>`.
...
- Prefer contextual shorthand syntax when a utility already resolves a namespace, such as `b:1px|solid|subtle` instead of `b:1px|solid|line-subtle`, `text:muted` instead of `text:text-muted`, and `transition:opacity|normal|standard` instead of `transition:opacity|duration-normal|easing-standard`.
