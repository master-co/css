...
- Prefer contextual shorthand syntax when a utility already resolves a namespace, such as `b:px|solid|subtle` instead of `b:px|solid|line-subtle`, `fg:muted` instead of `fg:text-muted`, and `transition:opacity|normal|standard` instead of `transition:opacity|duration-normal|easing-standard`.
- Low-level values are acceptable when the page is teaching syntax, no token exists, the value is local measured geometry, or the value describes structural layout such as `w:1/2`, `h:100dvh`, `m:0`, `m:px`, `z:1`, or `opacity:.64`.
