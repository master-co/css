...
- Prefer semantic role tokens when available, such as `bg:canvas`, `bg:surface`, `fg:strong`, `fg:muted`, line colors like `subtle` in border context, `accent`, `danger`, `r:<token>`, `shadow:<token>`, `animation-duration:<token>`, and `animation-timing-function:<token>`.
...
- Prefer contextual shorthand syntax when a utility already resolves a namespace, such as `b:1px|solid|subtle` instead of `b:1px|solid|line-subtle`, `fg:muted` instead of `fg:text-muted`, and `transition:opacity|normal|standard` instead of `transition:opacity|duration-normal|easing-standard`.
- Low-level values are acceptable when the page is teaching syntax, no token exists, the value is local measured geometry, or the value describes structural layout such as `w:1/2`, `h:100dvh`, `m:0`, `m:1px`, `z:1`, or `opacity:.64`.
