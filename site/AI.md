...
- Prefer contextual shorthand syntax when a utility already resolves a namespace, such as `b:1|subtle` instead of `b:1|line-subtle`, `fg:muted` instead of `fg:text-muted`, and `transition:opacity|normal|standard` instead of `transition:opacity|duration-normal|easing-standard`.
