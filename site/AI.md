...
- Prefer contextual shorthand syntax when a utility already resolves a namespace, such as `b:px|subtle` instead of `b:px|line-subtle`, `fg:muted` instead of `fg:text-muted`, and `transition:opacity|normal|standard` instead of `transition:opacity|duration-normal|easing-standard`.
