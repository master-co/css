...
- In `/guide` pages, prefer CSS scanner directives such as `@source`, `@source not`, `@source required`, `@safelist`, and `@blocklist` over JavaScript `scanner` option examples when teaching source scanning, safelisting, or blocklisting. Use JS plugin options only when the section is specifically about integration configuration.
...
- Prefer preset palette and text hue aliases by default, such as `text:blue`, `text:neutral`, `bg:blue`, and `bg:blue-5`. Use semantic role tokens such as `bg:canvas`, `bg:surface`, `text:body`, `b:1px|solid|divider`, `accent`, or `danger` only when the page or project defines those tokens in `@theme`.
- Preserve typography semantics: use `font:<size>` when replacing a raw font-size-only class like `font:2.5rem`, and use `text:<size>` only when the complete type treatment is intended because it can include font size, line height, and letter spacing.
...
- Prefer contextual shorthand syntax when a utility already resolves a namespace, such as `b:1px|solid|divider` instead of `b:1px|solid|line-divider` after defining `--color-line-divider`, `text:body` instead of `text:text-body` after defining `--color-text-body`, and `transition:opacity|normal|standard` instead of `transition:opacity|duration-normal|easing-standard`.
- Low-level values are acceptable when the page is teaching syntax, no token exists, the value is local measured geometry, or the value describes structural layout such as `w:50%`, `h:100dvh`, `m:0`, `m:1px`, `z:1`, or `opacity:.64`.
