...
test('core grammar does not hard-code key, value, selector, or at-rule names', () => {
    const raw = readFileSync(resolve(syntaxesDir, 'master-css.json'), 'utf8')
    assert.equal(raw.includes('lowercase'), false)
    assert.equal(raw.includes('uppercase'), false)
    assert.equal(raw.includes('contents'), false)
    assert.equal(raw.includes('overflow'), false)
    assert.equal(raw.includes('flex\\\\-col'), false)
    assert.equal(raw.includes('inline\\\\-flex'), false)
    assert.equal(raw.includes('table\\\\-column'), false)
    ...
    assert.equal(raw.includes('color\\\\-profile'), false)
    assert.equal(raw.includes('font\\\\-face'), false)
