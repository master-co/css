...
- `CSSRuntimeRegistry` depends on `virtual:master-css-plan.json` and should remain the quick-start integration entry.
- `src/lib/runtime-provider.ts` must remain free of `virtual:master-css-plan.json` imports for users that provide a plan manually.
