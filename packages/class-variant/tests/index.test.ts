...
test('normal function', () => {
    const normalBtn = cv<{ intent?: string, size?: string, color: string }>(
        'inline-flex rounded',
        ['top:30', 'left:40'],
        {
            intent: {
    ...
    expect(normalBtn()).toBe('inline-flex rounded top:30 left:40 font:semibold bg:blue fg:white bg:blue-55:hover text:14 p:5|15')
    expect(normalBtn({ intent: 'secondary', size: 'sm', color: 'red' })).toBe('inline-flex rounded top:30 left:40 font:semibold color:red bg:white fg:slate-30 bg:slate-90:hover text:14 p:5|15')
    expect(normalBtn({ intent: 'primary', size: 'md', color: 'red' })).toBe('inline-flex rounded top:30 left:40 font:semibold color:red uppercase bg:blue fg:white bg:blue-55:hover text:16 p:10|25')
})
