...
test.each(['native', 'wasm'] as const)('%s skips extraction, validation and snapshots for cached sources', async (binding) => {
  ...
  try {
    ...
    expect(await scanner.scanSource('b.html', content)).toEqual({ ...first, changed: false, sourceChanged: true })
...
test.each(['html', 'tsx', 'vue', 'svelte'])('caches %s extraction including complete candidates and empty results', async (extension) => {
  ...
  try {
    ...
    expect(await scanner.scanSource(source, ' ')).toEqual({ changed: false, sourceChanged: false, candidates: [] })
    expect(resolve).toHaveBeenCalledTimes(2)
    expect(await scanner.scanSource(source, '')).toEqual({ changed: false, sourceChanged: true, candidates: [] })
    expect(scanner.state.cachedSources).toBe(1)
