...
figma.showUI(__html__, { themeColors: true, width: 240, height: 214 })

figma.ui.onmessage = async ({ type, data }) => {
    switch (type) {
        case 'get-collection-variables':
            figma.ui.postMessage({ type, data: await getCollectionVariables(data.id, data.selectedVarDefaultMode) }, { origin: '*' })
            return
