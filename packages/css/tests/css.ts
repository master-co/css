...
export const testProp = (className: string | string[], expected: string, config?: Config): void => {
    if (typeof className === 'string') {
        className = [className]
    }
    className.forEach((eachClassName) => {
        const selector = CSS.escape(eachClassName)
        expect(generateFromClasses([eachClassName], config)).toBe(`.${selector}{${expected}}`)
    })
}
