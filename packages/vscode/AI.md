...
- The extension contributes the Master CSS TextMate injection grammar from `@master/css-language/syntaxes/master-css.tmLanguage.json`. Do not keep a VS Code-local copy; package staging copies that shared asset into `dist/node_modules`.
- The TextMate grammar is the authoritative lexical layer for CSS directive syntax in CSS-family documents. Semantic tokens are the precision layer for Master CSS class-list spans and manifest-aware class, variable, and component classification.
