  ...
  for (const qualifier of ['', ' layer', ' layer(cards)', ' supports(display:grid)', ' screen', ' layer(cards) supports(display:grid) screen']) {
    test(`BH-0004 filesystem project ${explicit ? 'explicit entries' : 'discovery'} ${qualifier || 'unqualified'}`, async () => {
      ...
      try {
        ...
        writeFileSync(entry, `@import './styles/child.css'${qualifier};@master entry;`)
        writeFileSync(child, "@import 'https://invalid.invalid/external.css';@reference '../tokens.css';@source './views/*.html';@components{button{@compose paint;}}.card{@compose paint;}.ordinary{color:blue}")
        writeFileSync(tokens, '@utilities{paint{color:red}}')
        writeFileSync(view, '<div class="button card"></div>')
        const result = binding.loadProjectManifest(root, { version: 1, utilities: [] }, explicit ? [entry] : undefined)
