...
- Output: `MasterCSSManifest`, shared directive data for lower-level consumers, style definitions, native CSS with consumed Master directives removed, native class names, warnings, standalone directive metadata, and CSS import dependencies.
- CSS manifest directives such as `@settings`, `@theme`, and `@custom-variant` define manifest input. Defining a component, utility, variable, variant, or animation does not emit CSS by itself; the class still needs to be used or extracted.
...
- `compileProjectManifest()` compiles project entry CSS files into the canonical project-level `MasterCSSManifest`.
...
- The compiler records mode declarations as written. Manifest settings and engine execution decide which modes are defaults.
...
- Top-level native `@keyframes` remain native CSS. Put direct `@keyframes` inside a top-level non-mode, non-inline `@theme` block when keyframes should become managed manifest animations.
...
- The compiler package does not scan unrelated `.css` files for class usage. Pair CSS manifest entries with static mode or pass extracted classes through compiler options when filtering native CSS.
