...
export function resolveMasterCSSWorkspacePackages(workspaceDir = process.cwd()): MasterCSSWorkspacePackageResolution {
  ...
  try {
    result.languageServer = resolvePackage('@master/css-language-server/server', workspaceRequire)
  } catch {
    // The workspace language server is optional; callers fall back to their bundled server.
  }
