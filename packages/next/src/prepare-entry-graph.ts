...
export async function prepareNextEntryGraph(context: ModuleContext, projectDir: string, options: NextStylesheetLoaderOptions,
  ...
  return { graph, entry: await graph.prepareEntry(rawEntry.source, rawEntry.sourceMap), manifest: { version: 1 as const, languageVersion: 3 as const, utilities: [] }, postcss: true }
}
