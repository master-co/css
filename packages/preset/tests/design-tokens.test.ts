...
describe.concurrent('@master/css-preset design token parity', () => {
  ...
  test('executes built-in registry records with foundation role tokens only', () => {
    ...
    expect(declarationsCSS.createRule('bg:line')?.text).toContain('background:line')
    expect(declarationsCSS.createRule('bg:base')?.text).toContain('background:base')
    ...
    expect(css.createRule('outline:1px|solid|var(--color-line-base)')?.text).toBe('.outline\\:1px\\|solid\\|var\\(--color-line-base\\){outline:1px solid var(--color-line-base)}')
    ...
    expect(declarationsCSS.createRule('bg:blue-surface')?.text).toContain('background:blue-surface')
