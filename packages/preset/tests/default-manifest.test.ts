...
describe('@master/css-preset defaultManifest', () => {
    ...
    it('executes CSS-authored semantic and enum pattern utilities', () => {
        ...
        const orderedCSS = createCSS(defaultManifest)
        ...
        const nativeFallbackCSS = createCSS(defaultManifest, undefined, {
    ...
    it('removes fixed keyword value aliases while preserving raw ambiguous matches', () => {
        ...
        expect(css.create('b:line')?.text).toBe('.b\\:line{border:line}')
        expect(css.create('bt:1px')?.text).toBe('.bt\\:1px{border-top-width:1px}')
        expect(css.create('bl:line')?.text).toBe('.bl\\:line{border-left:line}')
        expect(css.create('bx:1px')?.text).toBe('.bx\\:1px{border-inline-width:1px}')
        expect(css.create('by:line')?.text).toBe('.by\\:line{border-block:line}')
        expect(css.create('b:1px|solid|line')?.text).toBe('.b\\:1px\\|solid\\|line{border:1px solid line}')
        expect(css.create('bt:1px|solid|line')?.text).toBe('.bt\\:1px\\|solid\\|line{border-top:1px solid line}')
        expect(css.create('b:1px|line')?.text).toBe('.b\\:1px\\|line{border:1px line}')
        expect(css.create('b:gray-20')?.text).toBe('.b\\:gray-20{border-color:var(--color-gray-20)}')
        expect(css.create('bl:gray-20')?.text).toBe('.bl\\:gray-20{border-left-color:var(--color-gray-20)}')
        expect(css.create('by:gray-20')?.text).toBe('.by\\:gray-20{border-block-color:var(--color-gray-20)}')
        expect(css.create('b:1px|solid|gray-20')?.text).toBe('.b\\:1px\\|solid\\|gray-20{border:1px solid var(--color-gray-20)}')
        expect(css.create('bt:1px|solid|gray-20')?.text).toBe('.bt\\:1px\\|solid\\|gray-20{border-top:1px solid var(--color-gray-20)}')
        expect(css.create('b:1px|gray-20')?.text).toBe('.b\\:1px\\|gray-20{border:1px var(--color-gray-20)}')
