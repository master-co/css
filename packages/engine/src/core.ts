...
export default class MasterCSS {
    ...
    normalizeNumericValue(value: string | number): MasterCSSNormalizedNumericValue | undefined {
        ...
        switch (matches[2].toLowerCase()) {
            ...
            case 'px':
                return { kind: 'rem', value: numericValue / this.settings.rootSize }
            case 'x':
                return { kind: 'rem', value: numericValue * this.settings.baseUnit / this.settings.rootSize }
            default:
                return
        }
    }

    inspectClass(className: string, mode?: string): MasterCSSClassInspection {
        const rules = this.generate(className, mode)
        const rule = rules[0]
