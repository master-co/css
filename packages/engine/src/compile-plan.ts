...
export type EngineSettings = MasterCSSPlanSettings & {
    rootSize: number
    baseUnit: number
...
function compileUtilityDefinition(
    ...
    const definedUtility = {
        ...utility,
        matchers: utility.matchers.map((matcher: MasterCSSPlanUtilityMatcher) => ({ ...matcher }))
    } as CompiledUtility
...
export function compilePlan(plan: MasterCSSPlan): CompiledPlan {
...
export function getCompiledPlan(plan: MasterCSSPlan) {
