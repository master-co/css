...
describe.concurrent('migrated complex utility parity', () => {
    test('keeps group syntax independent from preset utilities', () => {
        expect(MasterCSS.create({ manifest: { version: 1 } }).createRule('{display:block}')?.text)
    ...
    test('keeps grouped declarations generated utilities and invalid group recovery', () => {
        ...
        expect(css.createRule('{color:black!;bb:2px|solid}')?.text)
            .toBe('.\\{color\\:black\\!\\;bb\\:2px\\|solid\\}{color:oklch(0% 0 none)!important;border-bottom:2px solid}')
        expect(css.createRule('{pt:calc(2.5em+3.75rem);mt:-3.75rem}_:where(h1,h2,h3,h4,h5,h6)')?.text)
    ...
    test('keeps grouped gradient values with custom variables and quoted separators', () => {
        ...
        const rule = css.createRule('{content:\'\';abs;inset:0;bg:linear-gradient(90deg,G-10/.1|10%,G-20/.2|20%,G-30/.3|60%,white/.4)}::after')
        ...
        expect(createDefaultCSS().createRule('{content:\'\';block}::after@light')?.text)
    ...
    test('keeps inset utilities values and priority order', () => {
        ...
        expect(css.createRule('top:5x')?.text).toBe('.top\\:5x{top:1.25rem}')
        expect(css.createRule('bottom:2.5x')?.text).toBe('.bottom\\:2\\.5x{bottom:0.625rem}')
        expect(css.createRule('inset:4x')?.text).toBe('.inset\\:4x{inset:1rem}')
        expect(css.createRule('left:1.875rem')?.text).toBe('.left\\:1\\.875rem{left:1.875rem}')
        expect(css.createRule('right:max(0px,calc(50%-45.3125rem))')?.text)
            .toBe('.right\\:max\\(0px\\,calc\\(50\\%-45\\.3125rem\\)\\){right:max(0px,calc(50% - 45.3125rem))}')
        expect(css.createRule('top:0')?.text).toBe('.top\\:0{top:0}')
        expect(css.createRule('left:0')?.text).toBe('.left\\:0{left:0}')
        expect(css.createRule('right:0')?.text).toBe('.right\\:0{right:0}')
    ...
    test('keeps size max and min single-value utility parsing', () => {
        ...
        expect(css.createRule('size:min(2.5x,calc(6.25x-2.5x))')?.declarations)
            .toStrictEqual({
    ...
    test('keeps transition multi-value syntax and rejects removed shorthand prefix', () => {
        ...
        expect(css.createRule('~transform|.1s|ease-out,width|.1s|ease-out')).toBeUndefined()
        expect(css.createRule('transition:transform|.1s|ease-out,width|.1s|ease-out')?.text)
    ...
    test('keeps font family feature weight utilities and leaves font shorthand to native fallback', () => {
        ...
        expect(css.createRule('font:italic|1.2rem|sans')).toBeUndefined()
        expect(createDefaultCSS().createRule('font:italic|small-caps|bold|1rem/1.5|serif')?.text)
        ...
        expect(css.createRule('font-bolder')?.text).toBe('.font-bolder{font-weight:bolder}')
        expect(css.createRule('font:thin')?.text).toBe('.font\\:thin{font-weight:var(--font-weight-thin)}')
        expect(css.createRule('font-feature-settings:\'cv02\',\'cv03\',\'cv04\',\'cv11\'')?.text)
            .toBe('.font-feature-settings\\:\\\'cv02\\\'\\,\\\'cv03\\\'\\,\\\'cv04\\\'\\,\\\'cv11\\\'{font-feature-settings:\'cv02\',\'cv03\',\'cv04\',\'cv11\'}')
        expect(css.createRule('font-feature-settings:tabular')?.text)
            .toBe('.font-feature-settings\\:tabular{font-feature-settings:var(--font-feature-tabular)}')
        expect(css.createRule('font-feature-settings:font-feature-tabular')?.text)
