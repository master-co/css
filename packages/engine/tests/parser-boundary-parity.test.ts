...
describe.concurrent('migrated parser boundary parity', () => {
  ...
  test('keeps selector aliases, pseudos, shorthands, and descendants intact', () => {
    ...
    expect(css.createRule('hidden:nth-child(2)')?.text).toBe('.hidden\\:nth-child\\(2\\):nth-child(2){display:none}')
    expect(css.createRule('hidden:nth(2)')?.text).toBe('.hidden\\:nth\\(2\\):nth(2){display:none}')
