...
// cooperative local lock for MCP processes of the same OS user, not a lock on
...
async function tickets(directory: string) {
  ...
  for (const id of await readdir(directory)) {
    ...
    if (typeof value.choosing !== 'boolean' || typeof value.number !== 'string' || !/^\d+$/.test(value.number)) {
      throw new Error(`Invalid MCP preview lock register: ${file}`)
    }
...
/** Serialize hash validation and writes, including across independently loaded servers. */
export async function withPreviewWriteLock<T>(work: () => Promise<T>, options: { directory?: string, timeout?: number } = {}) {
  const directory = options.directory ?? join(userInfo().homedir, '.cache', 'mastercss', 'mcp-preview-lock-v1')
  ...
  try {
    ...
    while (true) {
      if (performance.now() >= deadline) throw new Error('Timed out waiting for another MCP preview apply.')
