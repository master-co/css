...
`@master/css-lexer` owns dependency-free lexical scanning and source range contracts shared by Master CSS tooling. It may identify source ranges, directive boundaries, quoted strings, class-like source candidates, and config entrypoint statements. It must not validate classes, generate CSS rules, resolve config, compile directives, or depend on language-service rendering behavior.
...
- CSS config entry scanners from `src/css-config-entry.ts`
