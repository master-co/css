...
test.concurrent('outline', () => {
    ...
    expect(createCSS().create('outline:black')?.text).toContain('outline-color:rgb(0 0 0)')
    expect(createCSS().create('outline:2|black')?.text).toContain('outline:0.125rem rgb(0 0 0) solid')
    expect(createCSS().create('outline:1')?.text).toContain('outline-width:0.0625rem')
    expect(createCSS().create('outline:dashed|black')?.text).toContain('outline:dashed rgb(0 0 0)')
    ...
    expect(createCSS().create('outline:thick|double|black')?.text).toContain('outline:thick double rgb(0 0 0)')
...
test.concurrent('autofill solid', () => {
    expect(createCSS().create('outline:16|black')?.text).toContain('outline:1rem rgb(0 0 0) solid')
    expect(createCSS().create('outline:16|black|solid')?.text).toContain('outline:1rem rgb(0 0 0) solid')
    expect(createCSS({ variables: { line: 'solid' } }).create('outline:16|black|line')?.text).toContain('outline:1rem rgb(0 0 0) solid')
