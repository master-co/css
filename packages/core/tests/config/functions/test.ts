...
test.concurrent('functions', () => {
    ...
    expect(new MasterCSS().create('scale(.2)')?.text).toBe('.scale\\(\\.2\\){transform:scale(0.2)}')
    ...
    expect(new MasterCSS().create('box-shadow:0|2|3|rgba(0,0,0,.1)')?.text).toBe('.box-shadow\\:0\\|2\\|3\\|rgba\\(0\\,0\\,0\\,\\.1\\){box-shadow:0rem 0.125rem 0.1875rem rgba(0,0,0,0.1)}')

    expect(new MasterCSS().create('grid-template-cols:repeat(2,auto|.6|calc(3-max(2,1)))')?.text).toBe('.grid-template-cols\\:repeat\\(2\\,auto\\|\\.6\\|calc\\(3-max\\(2\\,1\\)\\)\\){grid-template-columns:repeat(2,auto 0.6 calc(0.1875rem - max(2, 1) / 16 * 1rem))}')
