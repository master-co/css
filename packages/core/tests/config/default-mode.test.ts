...
test.concurrent('default mode', () => {
    expect(new MasterCSS({ modeTrigger: 'class' }).add('bg:invert').text).toContain('.light,:root{--invert:0 0 0}')
    expect(new MasterCSS({ modeTrigger: 'class' }).add('bg:invert').text).toContain('.dark{--invert:255 255 255}')
})
