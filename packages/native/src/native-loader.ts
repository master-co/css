...
export type NativeBindingInfo = MasterCSSBackendInfo
export type {
  MasterCSSBackend,
  MasterCSSBackendFeature,
  MasterCSSBackendInfo,
  MasterCSSBackendSurface,
  ...
  for (const startDirectory of new Set(startDirectories)) {
    ...
    while (true) {
      for (const artifact of [
        resolve(directory, 'node_modules/@master/css-backend/artifacts', name),
...
export function assertNativeCLIInfo(executablePath: string): NativeBindingInfo {
  ...
  try {
    return assertMasterCSSBackendInfo(info, {
      surface: 'cli',
