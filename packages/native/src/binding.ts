...
export interface MasterCSSBackendRequirements {
  surface: MasterCSSBackendSurface
  features?: readonly MasterCSSBackendFeature[]
  packageVersion?: string
}

export class MasterCSSBackendContractError extends Error {
  readonly code = 'BINDING_MISMATCH'

  constructor(message: string, options?: ErrorOptions) {
    super(message, options)
    this.name = 'MasterCSSBackendContractError'
  }
}

function parseBackendInfo(value: unknown): MasterCSSBackendInfo {
  if (!value || typeof value !== 'object') {
    throw new MasterCSSBackendContractError('Master CSS binding metadata must be an object.')
  }
  return value as MasterCSSBackendInfo
}

export function assertMasterCSSBackendInfo(
  value: unknown,
  requirements: MasterCSSBackendRequirements
): MasterCSSBackendInfo {
  const info = parseBackendInfo(value)
  ...
  if (!versionsMatch || !packageMatches || info.surface !== requirements.surface || missingFeatures.length) {
    throw new MasterCSSBackendContractError(
