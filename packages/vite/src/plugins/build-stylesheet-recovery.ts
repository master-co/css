...
export default function BuildStylesheetRecoveryPlugin(context: MasterCSSVitePluginContext): Plugin {
  return {
    ...
    configResolved(config) {
      if (config.build?.watch) prepareBuildStylesheetRecovery(context, config.cacheDir)
    },
    buildStart() {
