...
- Entry discovery only checks top-level project markers: `@master;` and `@import "@master/css"`. Do not treat `@master shake;`, `@master no-shake;`, imported `@theme`/`@master` blocks, top-level `@custom-at`/`@custom-selector` directives, or package CSS files as independent project entries.
