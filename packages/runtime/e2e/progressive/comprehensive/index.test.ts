...
test('comprehensive', async ({ page }) => {
    ...
    expect(rules.map(({ name }) => name)).toEqual(['layer-statement', 'theme', 'fade', 'base', 'preset', 'main', 'general'])
    ...
    expect(await page.evaluate(() => globalThis.cssRuntime.mainLayer.native?.cssRules.length)).toEqual(1)
})
