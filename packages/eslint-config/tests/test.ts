...
test('ESLint Configuration is valid', async () => {
    ...
    expect(result[0].messages.map((eachMessage) => eachMessage.message)).toEqual(
        [
            '"m:10" applies the same declarations as "m:20" and "m:30".',
            '"m:20" applies the same declarations as "m:10" and "m:30".',
            '"m:30" applies the same declarations as "m:10" and "m:20".',
            '"m:40@sm" applies the same declarations as "m:50@sm".',
            '"m:50@sm" applies the same declarations as "m:40@sm".',
