...
test('syntax', async () => {
    const extractor = await new CSSExtractor({ sources: ['syntax.html'], include: [] }, __dirname).init()
    const testClasses = [
    ...
    for (const eachGeneratedClass of extractor?.css.utilitiesLayer.rules.map(({ name }) => name) || []) {
