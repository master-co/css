...
export function renderInteractionDocument(options: {
  ...
  return [
    ...
    '<head>',
    '    <meta charset="utf-8">',
    `<script>${renderRuntimeSnapshotReader()}</script>`,
...
function renderInteractionScript(options: {
  ...
  return [
    ...
    '                    computedStyleValid: input.beforeLayout.valid && input.resizedLayout.valid && input.beforeWidth !== input.resizedWidth,',
    '                    cleanupValid: true,',
    '                    beforeWidth: input.beforeWidth,',
    '                    resizedWidth: input.resizedWidth,',
    '                    beforeLayout: input.beforeLayout,',
    '                    resizedLayout: input.resizedLayout',
    '                }',
    '            });',
    '        };',
    ...
    '            const item = scratch.firstElementChild;',
    '            const minHeight = item && getComputedStyle(item).minHeight;',
    ...
    '                computedStyleValid: appendedCount === config.appendCount && minHeight === "72px" && textAlign === "center",',
    '                cleanupValid: scratch.children.length === 0,',
    '                appendedCount,',
    '                appendStyle: { minHeight, textAlign },',
    '                styleValidation: "Alignment probe and first appended element min-height:72px"',
    '            };',
    '        }',
    ...
    '                const probe = document.getElementById("interaction-style-probe");',
    '                const textAlign = probe && getComputedStyle(probe).textAlign;',
    '                const paddingAndBorder = style ? [style.paddingLeft, style.paddingRight, style.borderLeftWidth, style.borderRightWidth].reduce((sum, value) => sum + parseFloat(value), 0) : NaN;',
    '                const expectedWidth = style?.boxSizing === "border-box" ? Math.max(1, paddingAndBorder) : 1;',
    '                const width = style ? parseFloat(style.width) : NaN;',
    '                styleChecks.push({ cycle, textAlign, width, expectedWidth, valid: textAlign === "center" && Number.isFinite(width) && Math.abs(width - expectedWidth) < 0.01 });',
    '                scratch.textContent = "";',
    '                await waitFrames(1);',
    '            }',
    ...
    '                computedStyleValid: styleChecks.length > 0 && styleChecks.every(check => check.valid),',
    '                styleChecks,',
    '                styleValidation: "Per-cycle alignment probe and temporary 1px width (including border-box minimum)",',
    '                cleanupValid,',
    ...
    '            return { ...globalThis.__readBenchmarkRuntimeSnapshot(), domNodeCount: document.getElementsByTagName("*").length };',
    '        }',
