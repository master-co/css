import { renderRuntimePreparationInstaller } from './runtime-preparation'
...
export function addRuntimeHarness(html: string, options: {
  ...
  return insertBeforeHeadEnd(withVisibility, [
    ...
    '    <script>',
    '        (() => {',
    renderRuntimePreparationInstaller(),
    '            const preparationControls = new WeakMap();',
    '            const metrics = window.__interactionMetrics;',
    ...
    '            Runtime.prototype.ensureClassRules = function(classNames) {',
    '                if (!Array.isArray(classNames)) throw new TypeError("Benchmark runtime methods require a class-name array.");',
    '                const startedAt = performance.now();',
    '                const result = originalEnsureClassRules.call(this, classNames);',
    '                const elapsed = performance.now() - startedAt;',
    '                if (metrics.collectInteractionMutations) {',
    '                    metrics.runtimeMutationMs += elapsed;',
    '                    if (metrics.runtimeDiagnosticsEnabled) {',
    '                        metrics.runtimeAddCallCount++;',
    '                        metrics.runtimeAddClassCount += classNames.length;',
    '                        metrics.runtimeAddDurationMs += elapsed;',
    '                    }',
    '                }',
    '                return result;',
    '            };',
    '            Runtime.prototype.deleteClassRules = function(classNames) {',
    '                if (!Array.isArray(classNames)) throw new TypeError("Benchmark runtime methods require a class-name array.");',
    ...
    '                }',
    '                const startedAt = performance.now();',
    '                const result = originalDeleteClassRules.call(this, classNames);',
    '                const elapsed = performance.now() - startedAt;',
    '                if (metrics.collectInteractionMutations) {',
    '                    metrics.runtimeMutationMs += elapsed;',
    '                    if (metrics.runtimeDiagnosticsEnabled) {',
    ...
    '                metrics.runtimeRemoveQueue.push({ runtime, classNames: [...classNames] });',
    ...
    '                    originalDeleteClassRules.call(runtime, names);',
    '                    callCount++;',
    ...
    '                if (metrics.runtimeDiagnosticsEnabled) {',
    '                    if (!preparationControls.has(this)) preparationControls.set(this, installBenchmarkRuntimePreparation(this, metrics));',
    '                    window.__benchmarkRuntimeControls = preparationControls.get(this);',
    '                }',
    '                requestAnimationFrame(() => requestAnimationFrame(() => {',
    '                    const marker = document.getElementById("benchmark-loaded");',
    '                    marker.dataset.ready = "true";',
