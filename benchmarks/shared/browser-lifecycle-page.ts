...
export function renderLifecycleDocument(options: {
  ...
  return [
    ...
    `<script>${renderCSSOMReader()}</script>`,
...
function renderLifecycleScenarioScript(options: {
  ...
  return [
    ...
    '            const state = globalThis.__readBenchmarkRuntimeSnapshot();',
    ...
    '                runtimeGeneratedRuleCount: state.runtimeGeneratedRuleCount,',
    '                runtimeStyleRawBytes: state.runtimeStyleRawBytes,',
    '                runtimeStyleText: state.runtimeStyleText,',
    '                retainedClassCount: state.retainedClassNames.length,',
    '                retainedRuleCount: state.retainedClassRuleCount,',
    '                progressiveAdopted: state.progressiveAdopted,',
    ...
    '                try { total += globalThis.__benchmarkCSSOM.countRules(sheet.cssRules); } catch {}',
    '            }',
    '            return total;',
    '        }',
...
export function addRuntimeHarness(html: string, options: { hideUntilRuntime: boolean }) {
  ...
  return insertBeforeHeadEnd(withVisibility, [
    ...
    '                metrics.progressiveAdopted = this.snapshot().hydration.state === "progressive" ? 1 : 0;',
