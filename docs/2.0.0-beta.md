...
- [New configuration and management](#new-configuration-and-management)
  - [⚙️ `configure()` Extend and merge configuration](#️-configure-extend-and-merge-configuration)
  - [⚙️ `init()` Manually initialize Master CSS](#️-init-manually-initialize-master-css)
...
```
⚠️ Be sure to remove `"type": "module"` in `package.json` after creating the vite project
...
## ⚙️ `configure()` Extend and merge configuration
Often you want to add additional configuration in an extended way, we designed the `configure()` method to allow you to extend and merge configuration while keeping the default configuration.
...
```
The final configuration result obtained by `import config from './master.css'`:
```diff
{
    ...,
    "semantics": {
...
## ⚙️ `init()` Manually initialize Master CSS
Initialize Master CSS with custom configuration:
```js
...
Don't forget to include `./master.js` at the entry point of the app, e.g. `./main.js`
```js
import './master';
```

If you're using a CDN in `<head>` for quick enablement, initialize the Master CSS immediately after importing:
```diff
...
<head>
    <script src="https://cdn.master.co/css@2.0.0-beta.1"></script>
