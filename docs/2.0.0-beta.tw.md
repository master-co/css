...
- [內建主題切換與同步](#內建主題切換與同步)
  ...
  - [✨ 存取主題方案](#-存取主題方案)
  - [✨ 自定主題方案配置](#-自定主題方案配置)
  - [範例：在 React 實作主題切換](#範例在-react-實作主題切換)
- [其他的更動](#其他的更動)
...
我們提供了與主流建置工具的整合套件，在你的專案以零配置啟動 Master CSS 的 AOT 編譯。
...
如果你是在 `<head>` 中透過 CDN 快速啟用，引入後請立即初始化 Master CSS：
```diff
...
<head>
...
## 範例：在 React 實作主題切換
我們提供了 [useScheme](https://github.com/master-co/css.react#usescheme)，讓你無痛依賴 React 的渲染機制實作 UI。
```tsx
import { css } from '~/master';
...
2. **首頁 `css.master.co` 的改版**：過去我們將全部的精力投入在 Master CSS 核心功能研發，許多經典特色一直埋沒在文件中，透過視覺加強優勢的傳達將會是這次改版的重點！
