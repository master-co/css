...
- **⚡️ 獨立的規則引擎容器**，大部分的 CSS-in-JS 工具像 __Styled Components__ 都依附著框架的渲染週期 ( 如 React )，在重新渲染時這類工具勢必會在執行期犧牲相當可觀的運行成本。Master CSS 的 JIT 依賴瀏覽器原生的 API，僅在類名異動時執行，與框架無關
...
```js
const { MasterCSSWebpackPlugin } = require('@master/css/build-tool-plugins')
...
官方版本的 Master UI 元件一直都在製作中，但我們最終先暫緩該系列的製作，等待 `@master/css@2` 穩定發布後在以其繼續開發元件，第一波會先釋出基本款的開源元件像是 `button`, `card`, `checkbox` 等。