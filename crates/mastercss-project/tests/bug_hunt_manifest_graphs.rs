...
fn qualified_external_imports_do_not_block_manifest_or_source_plan_compilation() {
    ...
    let entry_source = "@import './styles/child.css' layer(outer) supports(display:grid) screen;@master entry;@components{button{@compose paint;}}";
    let child_source = "@import 'https://invalid.invalid/external.css';@source './views/*.html';@utilities{paint{color:red}}.native{background:url('./missing.png')}";
