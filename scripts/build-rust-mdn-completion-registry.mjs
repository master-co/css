...
function syntaxByReference(name, property) {
  return property ? properties[name]?.syntax : syntaxes[name]?.syntax
}
...
function collectSyntaxValues(syntax, seen = new Set()) {
  ...
  for (const match of syntax.matchAll(referenceRE)) {
    ...
    if (name.endsWith('()')) {
      ...
      values.push(...collectSyntaxValues(syntaxByReference(name, !!match[2]), seen))
    }
...
const nativePropertiesPath = path.resolve('crates/mastercss-schema/src/native-properties.rs')
const nativeProperties = `// Generated by scripts/build-rust-mdn-completion-registry.mjs. Do not edit.\n\npub const NATIVE_CSS_PROPERTIES: &[&str] = &[\n${Object.keys(properties).sort().map((name) => `    ${JSON.stringify(name)},`).join('\n')}\n];\n`

if (process.argv.includes('--check')) {
  assert.equal(readFileSync(outputPath, 'utf8'), content, `${path.relative(process.cwd(), outputPath)} is stale.`)
  assert.equal(readFileSync(nativePropertiesPath, 'utf8'), nativeProperties, 'Native CSS property registry is stale.')
  console.log(`Validated ${path.relative(process.cwd(), outputPath)}.`)
} else {
  writeFileSync(outputPath, content)
  writeFileSync(nativePropertiesPath, nativeProperties)
