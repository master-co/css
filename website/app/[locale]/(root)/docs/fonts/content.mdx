...
```
By default, `font.family` variables are inherited by the `font-family` and `font` rules.

### Apply custom fonts
Apply the defined font variables using the [font family](/docs/font-family) syntax. For example, presetting the global fonts in `html <body>`:
```html
<body class="**font:sans** **font:mono_:where(code,pre)** font-feature:'salt'">
    …
</body>
```
